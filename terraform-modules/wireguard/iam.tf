...
#
# The boundary is null by decision rather than by omission - the iam-role module requires the
# argument (Stage 2 step 7). This is an EC2 service role authored by the identity that authors
# boundaries, the same case Stage 3's flow-log role recorded (Lesson 18): no non-administrator
# can create or influence it, and the boundary object itself does not exist yet (Stage 2
# decision 4).
...
module "role" {
  ...
  inline_policies = {
    ...
    #
    # logs:CreateLogGroup is absent. Terraform owns that group because Terraform owns its
    # retention (observability.tf); an agent allowed to create it would recreate it without
    # retention after a manual delete, and an unbounded log group is a bill nobody reads. The
    # agent finds the group already there and only opens a stream in it.
