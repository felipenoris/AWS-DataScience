# Who may create an environment from each blueprint (Stage 6) - the blueprint-level twin of
# data-governance/governance/grants.tf, one authorization layer down.
#
...
#
# The cause is the profile gap's: the console's "enable blueprint" flow creates the
# configuration and fills "Authorized domain units", which emits this grant, while
# PutEnvironmentBlueprintConfiguration - what the resource in blueprints.tf calls - does only
# the first. Enabling a blueprint and authorizing its use are distinct objects, and step 1.4
...
# at project creation - our failure mode - so its principal is a measurement where a choice of
# our own would be a guess (Lesson 38). The one deliberate divergence: the sample sets
# include_child_domain_units = true, this file says false - the same argument grants.tf
# already carries, that this domain is not subdivided, so the flag describes today's shape and
# is re-read the day a domain unit is created, not inherited.
#
