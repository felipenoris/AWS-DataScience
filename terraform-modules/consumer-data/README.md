...
| `result_configuration.encryption_configuration` | `SSE_KMS` under the account's zone key, **stated rather than inherited** from the bucket default: the workgroup writes the object, so the encryption choice is visible where somebody reads it. Same key, so the two cannot disagree |
