...
(`plan/architecture.md` §4.2) built with them.
...
`REFERENCES.md`, [Stage 7](stage-07-gitlab-runners-ecr.md) and [Stage 10](stage-10-orchestration-promotion.md);
`step 6` from D14; `step 8` from `plan/architecture.md`, `plan/cost-model.md` and `plan/open-questions.md`;
`step 9` from `plan/architecture.md`. They do not change. The sequence to work in is **three passes**, and the
...
Three views of the same target, none of which exists yet. Layers are `plan/conventions.md` §5.1.
...
lifecycles (`plan/conventions.md` §5.1).
...
`plan/conventions.md` §6 places it in `production/foundation/`, and because its associations (4.4) are
...
also exactly what `plan/cost-model.md`'s floor already assumes.
...
**4.5 — The ordering trap.** The authorization is deleted once the association exists, so re-creating the
association after a VPC rebuild needs a fresh authorization. Both zones are `[P]`, which makes this a
once-per-account operation — and is the argument for putting the association in `foundation/` rather than
anywhere `make down` can reach.
...
**7.2 — It is conditional, not assumed.** Under egress design B (`plan/architecture.md` §4.3) the SageMaker
...
`plan/conventions.md` §6 applies early (before Stage 6) precisely so this works when the comparison runs.
...
(`aws:PrincipalOrgID` / `aws:ResourceOrgID`), which is the third axis of `plan/architecture.md` §4.2 and is
...
Each is written so its output differs between working and broken (Lesson 13). Two throwaway `t4g.nano`
...
- **The module applied N + 3 times:** four VPCs from one module, with the Sandbox range taken from the
  allocation table of 1.2 rather than from a literal.
...
- **The peerings are reachable in the intended direction only:** the Sandbox probe reaches the Production
  ...
  missing peering is missing on purpose.
...
1. Compare the `foundation/` resource IDs before and after a `make down`/`make up` cycle, by reading the
   plan output rather than by trusting the target list.
2. Confirm no route table in any account carries a destination inside `10.40.0.0/16`.
3. **Destroy both probes and delete the temporary DNS record** when the checks pass.
...
cents of flow-log storage. Both are already inside `plan/cost-model.md`'s floor. **The Sandbox row of the
...
| # | Question | Step |
|---|---|---|
| i | Does the `sagemaker.studio` endpoint use the non-standard `aws.sagemaker.<region>.studio` service name rather than the `com.amazonaws.*` form? | 8.3 |
...
- **A forgotten `egress/` costs ~USD 4.08 per day** at 0.170/h, and since the budget alerts were skipped by
  decision (D12) nothing detects it until the end of the month (`plan/cost-model.md`).
