...
| [D21](D21-development-account.md) | The Development account, and where experimentation ends | A Development account: Sandbox becomes pure experimentation and the promotion chain starts in Development. **Re-examined 2026-08-13 and held**: whether Development needs an *interactive* surface at all now turns on one named test — is there anything a person must do next to Development's data that they cannot do next to Sandbox's? — which Stage 5's grants are what would answer. The file carries the test, what moves if the answer is "nothing", and the larger branch where CI/CD deploys from Sandbox into Sandbox. | S1a, S1b, S3, S6, S8, S10 |
...
| [D23](D23-ou-structure.md) | OU structure — the account is the isolation boundary, the OU is the… | Six OUs plus one nested (`Identity` split out of `Security`, `Sandboxes` under `Interactive`, both by execution on 2026-08-09); the account isolates, the OU attaches policy. | S1a, S1b |
