...
- **The account where the repair actually happens is `Identity`** — D30 itself moved the SCPs to `terraform-live/identity/`, applied with the delegated-admin profile. A mis-written organization-root SCP (the region restriction is the obvious candidate: Identity Center is regional and `organizations:*` answers in `us-east-1`) denies *that* account the call that would fix the policy.
