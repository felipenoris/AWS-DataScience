...
| Stage 1c — Preventive policies | [stage-01c-preventive-policies.md](stage-01c-preventive-policies.md) | **Sitting A, in progress — step 7.0 only, all of it measurement; nothing attached and nothing changed in AWS yet.** The organization's coordinates and the `Data` OU path the `datazone` carve-out is written from; the OU tree with ids; the Control Tower guardrail attached to each OU with its policy id, and the reading of those documents — **Config already denied, CloudTrail denied nowhere, `Sandboxes` the one OU with no guardrail at all**, and `Identity` registered after all, which settles 7.6's `Security` diff; verification (x) answered *yes* and (iii) answered by reading; the quota measured as **not published**; account-level BPA `NOT SET` in all six accounts with a profile. Plus the decision that the baseline carries **no CloudTrail deny**, and the three policy documents written and rendered but not created. |
