...
- **`Interactive`** — **today**, and pending a decision. Interactive compute is allowed there because
  ...
  off the data scientist is `DataScientistAccess`, an *identity* policy. **Settled 2026-08-13** at
  [Stage 1c step 7.6](plan/stages/stage-01c-preventive-policies.md): the OU gets a set of **one statement**
  — no classic `sagemaker:CreateNotebookInstance` — so the per-OU sets written there are `Workloads`,
  `Data`, `Identity` and `Interactive`.
...
Every document here — this one, `CLAUDE.md`, `plan/`, `log/` — names an account by its **role in the
