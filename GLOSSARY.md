...
| `D1` … `D35` | Numbered key decisions. **One file each** in `plan/decisions/`, with a one-line summary per decision in `plan/decisions/INDEX.md`. Referenced by ID from the stages that consume them. |
...
| **AZ** | Availability Zone | A physically separate datacenter within a region. AZ *names* (`us-west-2a`) are per-account labels and can point at different physical AZs in different accounts; the AZ *ID* (`usw2-az1`) is the stable identifier. This distinction has a bill attached, so **this project anchors subnets on the ID** (`plan/architecture.md` §4.1). The current mapping: `./aws/AZs.sh`. |
