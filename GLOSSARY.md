...
|---|---|---|
| **OU** | Organizational Unit | A folder of AWS accounts inside an Organization. Policies attach to an OU or to an account — never to a tag. The account is the isolation boundary; the OU is the *policy* boundary (D23), and each OU here is named for the policy set it carries: `Security`, `Interactive` (Sandbox + Development — interactive compute allowed), `Data` (Data Management — no compute at all), `Workloads` (Staging + Production — no interactive compute, no human control plane). **Watch the name collision:** the `Workloads` OU containing the *Staging account* is a different thing from a **Policy Staging OU**, which is a throwaway OU used to test an SCP before it reaches anything real (§11). |
...
| **MWAA** | Managed Workflows for Apache Airflow | AWS's hosted Airflow. Named explicitly in `CLAUDE.md`. The classic form charges an **environment fee per hour of existence** whether or not a DAG runs (~USD 212/month for `mw1.micro`, ~USD 358/month for `mw1.small` in `us-west-2`); **MWAA Serverless** (November 2025) charges per task-hour instead. D7 builds it as alternative **A**, against a native EventBridge + Step Functions + Lambda stack as alternative **B**. |
