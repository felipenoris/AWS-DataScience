...
| `D1` … `D20` | Numbered key decisions, recorded with their rationale in `GENERAL_PLAN.md` §4. Referenced from the stages that consume them. |
| **Sandbox / Staging / Production** | The three environments, one AWS account each. Sandbox is where people work interactively; Staging and Production are *deployment targets* that only a pipeline writes to. See `README.md`, "What the AWS references recommend". |
...
|---|---|---|
| **OU** | Organizational Unit | A folder of AWS accounts inside an Organization. Policies attach to an OU or to an account — never to a tag, which is why the account is the only hard boundary AWS offers. This project has `Security`, `Sandbox` and `Workloads` (holding Staging and Production). **Watch the name collision:** the `Workloads` OU containing the *Staging account* is a different thing from a **Policy Staging OU**, which is a throwaway OU used to test an SCP before it reaches anything real (§11). |
| **SCP** | Service Control Policy | A maximum-permission ceiling attached to an OU or account, evaluated on the **identity** side. It cannot grant anything; it only removes. Used here to deny leaving the organization, deny disabling CloudTrail, restrict regions, and deny writes to S3 outside the organization. |
...
| **Promotion** | — | Moving a versioned artifact from one environment to the next through the pipeline, with a gate in between. In this project the chain is Sandbox → Staging → Production, and the four artifacts that travel are listed in D17. |
