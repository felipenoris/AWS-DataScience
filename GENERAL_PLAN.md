...
| What happened | [`plan/history.md`](plan/history.md) — never a `log/log-stage-NN-*.md`, which is the user's |
...
| [2 — Terraform foundation](plan/stages/stage-02-terraform-foundation.md) | State buckets, module skeletons, the six persona permission sets written from scratch, the policy import, the hygiene checks | not started |
...
- [`log/`](log/INDEX.md): the user records every manual step, in that stage's `log/log-stage-NN-*.md`
  (never edited by Claude) — the stage file's own name, prefixed `log-`, so the two never collide.
  A new stage gets a new file; its row in [`log/INDEX.md`](log/INDEX.md) is
