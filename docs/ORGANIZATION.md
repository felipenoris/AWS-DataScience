...
- The **WireGuard instance**, the single human entry point into the private network, terminated here
...
`awsds-infra-*` SSO profiles, from any network. It approves nothing, owns no dataset, runs no workload and
...
- **The primary working surface is the SageMaker Unified Studio portal** (D26), reached from any network
  with an Identity Center session, like every other AWS endpoint (D39). Signing in to the portal is not signing in to the Data Governance account that
