...
flowchart TB
    ...
    RAWIP["a process that already knows an IP address never asks the resolver<br/>the NAT forwards whatever is routable — design A's known weakness<br/>design B (no NAT at all) is D5's open half"]

    APP --> DNS
    DNS -->|"listed, and every CNAME hop inside the list"| KIND
    DNS -->|"not listed, or a hop leaves the list (EXC-05)"| NX
    KIND --> GWS3
    KIND --> IFE
    KIND --> NATP
    APP -.-> NOROUTE
...
| `pypi.org` | the **index** only: metadata resolves, the wheel download (`files.pythonhosted.org`) is a CNAME into Fastly and does not | NAT |
| `conda.anaconda.org`, `repo.anaconda.com` | answer flat — complete path | NAT |
| `us-west.pkg.julialang.org` → `us-west.pkg.julialang.net`, `storage.julialang.net` | the one two-hop chain, both hops listed; Julia must be pointed at the regional server (`JULIA_PKG_SERVER`) | NAT |
| `releases.astral.sh` | `uv`/`ruff` installer — complete path | NAT |
...
| **No path under design A**: `files.pythonhosted.org`, `index`/`static.crates.io`, `static.rust-lang.org`, `sh.rustup.rs`, `pkg.julialang.org`, CRAN, `deb.debian.org`, `archive.ubuntu.com`, `public.ecr.aws` | each is a CNAME into Fastly, CloudFront, Cloudflare or Global Accelerator; listing the name changes nothing, and listing the CDN namespace ends the control | — |

**Eight of the nine external names work only because their owner flattens the CDN behind an A record** (`EXC-05`): a switch a third party can turn off unannounced, after which that name answers with a CNAME and the notebook breaks in a way that looks like this estate's fault. `./aws/dns-allowlist.py` (`DN-1`–`DN-4`) re-resolves every name on both lists, with no AWS session needed.

**Two consequences worth stating plainly.** First, the app's AWS calls do **not** present the VPN's Elastic IP: they arrive as `aws:SourceVpce` (through an interface endpoint) or as the NAT's address — so any control keyed on `aws:SourceIp = 52.89.212.1` (the persona sets' `DenyControlPlaneOffVpn`, the lake's bucket-policy branch) is about **humans on the tunnel**, not about apps; what reaches the app is the endpoint policies, the bucket policies' `aws:SourceVpce` branch and the D13 boundary. Second, **design A's exfiltration story is "inconvenient", not "no path"**: DNS filtering does not stop a process that already holds an address. Design B — no NAT, no default route, CodeArtifact and ECR pull-through as the only artifact path — is the strict alternative, not built, and step 4.3's measurement is the input D5 is waiting for.
...
- **The path splits by destination**, and the lake's perimeter policy mirrors the split: S3 and DynamoDB leave through the `[P]` gateway endpoints (prefix-list routes beat the default) and arrive wearing the host's **private** address plus `aws:SourceVpce`; everything else exits through the IGW wearing the Elastic IP. Measured in CloudTrail on 2026-08-19/20, and the reason `DenyControlPlaneOffVpn` carries a third, `StringNotEqualsIfExists aws:SourceVpce` condition.
