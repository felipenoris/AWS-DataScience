...
| **an AWS call, any permission set** | through the proxy, because everything is | **direct**: no permission set tests the caller's network (read back 2026-09-17 on the nine provisioned persona roles, Stage 6g step 1.3; D39). Measured off the tunnel 2026-09-18 (6g step 1.4): `describe-log-groups` answers, and `list-buckets` is now the set's own *implicit* deny — on 2026-09-08 the same call was an **explicit** deny, `DenyControlPlaneOffVpn`'s, the statement 6g deleted. The monitored half is owed |
