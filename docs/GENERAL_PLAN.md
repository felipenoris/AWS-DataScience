...
| [6g — Access by identity](plan/stages/stage-06g-access-by-identity.md) | D39 executed: the persona deny deleted from the six permission sets, the lake's laptop branch turned into a principal branch on the drop-box, and the instruments, runbooks and documents that described the previous perimeter | **in progress** — written and applied 2026-09-17: the deny deleted from the six persona sets and the lake's laptop branch turned into a principal branch, both re-planning `No changes` and read back (`INV-19`); `VP-7` inverted, `PX-5` retired. Decisions 1 and 2 taken and applied the same day: the proxy wears its instance's own address (`184.33.8.126` released) and `make up` refuses on the proxy alone. Owed: the behavioural pairs 1.4 and 2.4 |
