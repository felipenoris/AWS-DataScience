...
| [6 — Unified Studio](plan/stages/stage-06-unified-studio.md) | The DataZone V2 domain, project profiles, and the two egress designs compared | **passes 0, 1 and 2a APPLIED 2026-08-21** — the `production/registry/` prerequisite, the two `sagemaker/` prerequisite slices, the step 3 deny fragment across all six persona sets, 1.6's Athena Spark SCP amendment, and the domain itself (`awsds-studio`, V2, in Data Governance). **Not closed**: everything past the console account association is open, and three measurements are owed — 0.1a's canary replay, 1.6's phase-4b probes and 5.0's image push. Earlier: **its cross-stage prerequisite was re-cut 2026-08-21** after an audit found the "pulled forward and applied" clause had never been executed: `production/registry/`'s **5.a** half is now this stage's **pass 0** (Stage 6 step 5.0 pushes into it), and `production/pki/` left the stage entirely |
