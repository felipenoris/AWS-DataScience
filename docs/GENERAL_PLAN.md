...
| [16 — Sandbox lake](plan/stages/stage-16-sandbox-lake.md) | The fourth Sandbox bucket, `awsds-sandbox-lake` — **permanent** per-SSO-group artifacts, mounted into SMUS projects via the portal's S3 connection, vended to laptops via S3 Access Grants; a compensated shadow lake, its trade argued in the institutional-delta row | **PASSES 0-3 APPLIED 2026-08-26** — the bucket, the access role, the Access Grants location and three per-group grants exist; `sandboxlake.py` 11/11 `pass`. The apply surfaced two Lake Formation administrator seats SMUS made for itself in Sandbox, adopted into `consumer-data-v0.4.0`. **Owed: 2.3 and passes 4-6.** Created the same day at the user's request; pre-instrumented by `./aws/sandboxlake.py` |
