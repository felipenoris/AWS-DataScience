...
4. **Private by default.** Data assets and databases never face the public internet. The only public entry
   points are the VPN and, later (Stage 13), an experimental web tier. **The VPN is the only way into the
   private network, and no access to AWS is conditioned on it**
   ([D39](plan/decisions/D39-access-by-identity.md); `docs/plan/objectives.md` carries the requirement):
   a person reaches the AWS console, the AWS APIs and the SageMaker Unified Studio portal from any network
   with an Identity Center session, which the institution grants only on a laptop it monitors. Built by
   [Stage 6c](plan/stages/stage-06c-networking-hub.md) under D38 (2026-09-05): the WireGuard host drops
   every tunnel packet that is not bound for an RFC1918 address, and a client on the monitored profile
   reaches the internet only through the institutional proxy. [Stage 6g](plan/stages/stage-06g-access-by-identity.md)
   removes the two network conditions that still bind persona calls to the hub's addresses, and
   [Stage 11](plan/stages/stage-11-dlp.md) carries the identity premise as a modelled residual, since
   Identity Center's own directory checks no device. First tested by
   [Stage 13](plan/stages/stage-13-public-web-tier.md).
...
9. **Preventive controls come before detective ones, and the two halves are scheduled by different
   ...
   on: Security Hub at Stage 5a, with the first governed data; Macie at Stage 11; GuardDuty at **Stage 15**
...
| [4 — VPN](plan/stages/stage-04-vpn.md) | WireGuard over the Stage 3 network, the only entry point to the private network; step 8's control-plane deny is retired by D39 and removed at Stage 6g | **DONE 2026-08-18 — closed by the GuardDuty split**: passes 1-3 executed and measured; pass 4 left the stage whole for Stage 15, prepared. Decision 4 (third review) moved the host private key into a `[P]` Secrets Manager secret, with [`docs/plan/runbooks/vpn.md`](plan/runbooks/vpn.md) Part K owning every key event; `sandbox/vpn/` was the tree's first `[D]` slice; the close-out log entry is the user's |
| [5a — Data foundation](plan/stages/stage-05a-data-foundation.md) | Lake, Glue, Iceberg, Lake Formation + the three cross-account shares; Security Hub on | **DONE — every pass, 2026-08-18/20**: the governed lake, the governance manager's grants, both cross-account TBAC shares, the consumer side (one `consumer-data` module in two slices — v0.2.0 since the 2026-08-19 revision that withdrew `security-zone`), 4c's persona grants, **4d's behavioural proofs, 4e's `DenyUserCompute` amendment, and pass 6** (Security Hub CSPM org-wide by central configuration on the root, 2026-08-20 — `INV-09` to nine/four). **Two things stay open**: 13.3's triage, which needs a first FSBP report that did not exist when the stage closed, and open question 19, the crawler demander, which no proof can settle. The 2026-08-17 data-governance review added the `zone` tag dimension and classification-scoped LF-TBAC grants (`restricted` by explicit grant only), and withdrew the NFS requirement the same day — no `nfs/` slice (D24 withdrawn) |
| [5b — Redshift Serverless](plan/stages/stage-05b-redshift-serverless.md) | The warehouse beside the lake: one namespace and workgroup in `Sandbox` at **4 base RPUs**, its cost ceiling and audit trail in the same apply, and the access model for both classes of database | not started — written 2026-09-19 under **D40**, which admits a warehouse built by hand and keeps the `RedshiftServerless` blueprint disabled; the requirement reached `objectives.md` on 2026-09-20 and settled four readings D40 had made alone, the load-bearing one being that **where the databases live is an implementation matter**, which legalises the two-account split. Priced: **0.36 USD/RPU-hour**, so 4 RPUs is **1.44 USD per query-hour** and **0.00 at rest** — the most expensive object per unit of time in the estate, which is why the usage limit (`breach_action = deactivate`) lands with the workgroup rather than at Stage 12. Two findings shape it: the **ratchet** past 4 RPUs never comes back down, and the provider documentation demands three AZs where the service documents two while validating nothing |
...
| [6e — Claude Code on Bedrock](plan/stages/stage-06e-claude-code-bedrock.md) | A coding assistant in a Sandbox space answered by Amazon Bedrock inside the estate's perimeter: the model-access form, the inference-profile pins, the endpoint that keeps the call off the proxy, and the data-protection answer written down | **in progress since 2026-09-11** — a session answered from a space on 2026-09-12: Haiku 4.5, the project role, through the `bedrock-runtime` endpoint, the proxy log silent on Bedrock. The scoped set is the 4.5 generation (decision 14; `claude-opus-5` and `claude-sonnet-5` are refused for this account, `EXC-08`), the Region exemption carries its compensating deny (15), and `mode: none` is declared in all 17 enabled Regions (16) and frozen by 7.5. Owed: the SCP battery's rows for the new statements, 8.3's budget notification (decision 6), 6.4's token volume, decisions 11 and 13 |
| [6f — Data governance](plan/stages/stage-06f-data-governance.md) | The SageMaker Catalog over the governed lake: subscription and Share measured, the lake's way into the catalog, approvals, AI-generated metadata, and Production publishing through the lake rather than an association | not started as a plan — the first publish and data product were taken by hand and measured to change no grant; `INT-23` is new |
| [6g — Access by identity](plan/stages/stage-06g-access-by-identity.md) | D39 executed: the persona deny deleted from the six permission sets, the lake's laptop branch turned into a principal branch on the drop-box, and the instruments, runbooks and documents that described the previous perimeter | **in progress** — written and applied 2026-09-17: the deny deleted from the six persona sets and the lake's laptop branch turned into a principal branch, both re-planning `No changes` and read back (`INV-19`); `VP-7` inverted, `PX-5` retired. Decisions 1 and 2 taken and applied the same day: the proxy wears its instance's own address (`184.33.8.126` released) and `make up` refuses on the proxy alone. 1.4 and 2.4 measured from the laptop 2026-09-18 — `describe-log-groups` answers, `list-buckets` is now an implicit deny, the drop-box took the persona's `PutObject` by principal on either path. 1.4 is closed on both profiles; owed: 2.4's Athena read from a space |
| [6h — The Redshift connection](plan/stages/stage-06h-redshift-connection.md) | The first sandbox **schema** and the first SMUS compute connection: write for one project on one themed schema, which several projects may share | not started — written 2026-09-19. *Per database × project* is **three layers in three systems**: a tag on the workgroup **and** its namespace, an IAM policy on the project role, and a Redshift `GRANT`. None is sufficient and all three fail with the same symptom, so the stage's shape is to wire and read them apart |
...
| [15 — GuardDuty org-wide](plan/stages/stage-15-guardduty.md) | Threat detection over the whole organization — delegation to Audit, auto-enable `ALL`, every optional plan switched off (they arrive ON), findings routed to a human for the first time | not started — created 2026-08-18 by splitting Stage 4's pass 4 out whole; nothing blocks it, and it gates Stage 11 step 4 (a month of billing) and Stage 5a step 13.2's ingestion |
...
**D1-D40, all settled** — **D30 settled as a revert** and keeps its file so the record shows what was
tried. **D40 (2026-09-19)** admits a Redshift Serverless warehouse built by hand, at 4 base RPUs, and keeps
the `RedshiftServerless` blueprint disabled: what D26 and D12 refused was a warehouse any project member
could provision in one click. One file each, with its reasoning, consequences and revision trigger:
...
proxy, and where the client plane resolves), **D39** (a person reaches AWS by identity; the VPN reaches
the private network), **D22** (the lake is on the ownership axis), **D26** (one
