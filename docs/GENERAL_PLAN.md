...
| [5 — Data foundation](plan/stages/stage-05-data-foundation.md) | Lake, Glue, Iceberg, Lake Formation + the three cross-account shares; Security Hub on | **PASSES 0-4 DONE except 4d/4e (2026-08-18/19)** — the governed lake, the governance manager's grants, both cross-account TBAC shares, the consumer side (`consumer-data` v0.1.0 in two slices) and 4c's persona grants. Owed: **4d** (every behavioural proof, all needing the tunnel), **4e** (`athena:StartQueryExecution` into `DenyUserCompute`, last) and **pass 6** (Security Hub) — **revised 2026-08-17 after the data-governance review** (the `zone` tag dimension; classification-scoped LF-TBAC grants, `restricted` by explicit grant only); **the NFS requirement withdrawn later the same day — no `nfs/` slice (D24 withdrawn)** |
