...
| [6e — Claude Code on Bedrock](plan/stages/stage-06e-claude-code-bedrock.md) | A coding assistant in a Sandbox space answered by Amazon Bedrock inside the estate's perimeter: the model-access form, the Opus 5 pin, the endpoint that keeps the call off the proxy, and the data-protection answer written down | not started, decisions taken 2026-09-11 (the space; the form in `Sandbox` alone; the picker locked; the retention denies in the baseline document; invocation logging deferred to [Stage 11](plan/stages/stage-11-dlp.md) step 5.6, written there; the scoped set Opus 5, Sonnet 5 and Haiku 4.5, Haiku pinned for background work). Written from the vendor documentation and eight readings. Opus 5 is inference-profile only and its `us.` profile spans three US regions, the first resource that cannot honour D1; `bedrock-runtime` already reaches a space **by the wrong door**, public rather than through an endpoint; and the ZDR default becomes an SCP rather than a trust |
