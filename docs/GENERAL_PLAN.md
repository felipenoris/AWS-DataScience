...
| [6h — The Redshift connection](plan/stages/stage-06h-redshift-connection.md) | The first `sbx_*` database and the first SMUS compute connection: one project given write on one sandbox database | not started — written 2026-09-19. *Per database × project* is **three layers in three systems**: a tag on the workgroup **and** its namespace, an IAM policy on the project role, and a Redshift `GRANT`. None is sufficient and all three fail with the same symptom, so the stage's shape is to wire and read them apart |
