...
| Route 53 **private** hosted zones (D15) | ~1.00-1.50 | USD 0.50/zone-month. **Three at N=1** (Stage 3 step 4.2): `sandbox.internal` — the one that multiplies with the business units — plus `prod.internal` and `pages.internal`, both owned by Production. **Staging gets none**: nothing in it is addressed by a private name, so a zone per VPC-bearing account would be USD 1.00/month resolving nothing. **No public zone before Stage 13** — D15 was revised on 2026-08-09 and the split-horizon design was dropped |
