...
| KMS customer-managed keys | ~9.00 → **~10.00** | ~1.00 per key per month. The set at N=1: one per Terraform-managed account (Sandbox, Development, Data Governance, Staging, Production, Identity) plus a **dedicated derived-zone key in each Interactive account** (D31 — the key policy is what says who may read materialised `restricted` data, and it only works as a control if the key is not also serving state and logs). **Applied 2026-08-19 and the count is unchanged — only the naming moved**: those two keys are `alias/awsds-sandbox-zn-lab` and `alias/awsds-dev-zn-lab`, because encryption granularity is the `security-zone` dimension's job in every account and not only inside the lake (`GOVERNANCE.md`; Stage 5 decision 2 as amended). **A second `security-zone` value is therefore a second key PER ACCOUNT**, which is the direction this row's floor now grows in. `scratch` needed no key of its own: it is a prefix in the same bucket, not a bucket plus the **PKI key in `production/pki/`** (D36 — same reasoning applied to the CA root: a key shared with `foundation/` would put the root back inside the blast radius the slice exists to leave). **The open half of this row is settled and it cost exactly one key** (2026-08-18, Stage 5 decision 2, applied the same day): the lake takes **one** CMK, `alias/awsds-data-zn-lab`, covering all five `awsds-data-*` buckets including the drop-box — because CMK granularity was reframed as the **`security-zone`** dimension's job ([`docs/GOVERNANCE.md`](../GOVERNANCE.md)) and one zone exists. The old sentence here, "a CMK per data domain, Stage 5 step 1", described a design that no longer exists. **It stays a floor rather than a count for one reason only**, and it is now a named one: a second `security-zone` value is a second key, and Bucket Keys make that a bucket-level change rather than a migration |
