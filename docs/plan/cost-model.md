...
([D40](decisions/D40-redshift-warehouse.md)). **It exists in Sandbox since 2026-09-20** and its first measured bill was **0.4743 USD** — 4,743 RPU-seconds for the whole of [Stage 5b](stages/stage-05b-redshift-serverless.md)'s execution, including a deliberate usage-limit breach and the 30-minute query that caused it. **At rest it bills 0.00 per hour and its storage bills anyway**: `make down ENV=sandbox` destroys the workgroup, because Redshift Serverless has no pause, and leaves the Redshift Managed Storage, which is the irreducible price of keeping the data between sessions — the same trade the GitLab EBS volume makes under `[D]`, and it belongs in this file's floor rather than only in the stage. The monthly floor bills *per hour of existence*; the
