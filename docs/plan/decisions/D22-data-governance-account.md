...
**Amended 2026-09-05:** the consumer list drops to Sandbox — the second consumer's share is revoked at [6b](../stages/stage-06b-development-becomes-staging.md). Data Governance still holds no VPC, so [INT-13](../integrations.md) stays on the manual-push path rather than a CodeConnections host.

**Applied 2026-09-06.** `datalake.py` reads one consumer; the RAM share and both re-grants are revoked (step 2.3), and six rows of the grant register in `docs/AWS_STATE.md` carry a dated **REVOKED** annotation rather than being deleted.
