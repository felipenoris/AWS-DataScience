...
## Certificates for internal names
...
On 2026-08-09 `CLAUDE.md` began stating explicitly that GitLab and GitLab Pages are reachable
**only through the intranet (VPN)** and never face the public internet. That makes the whole interactive
layer a closed audience, and it exposes what the public domain was buying: **a trust chain that browsers
already ship**, not reachability — nothing is published. For an audience of three clients that we build
ourselves, that convenience is purchasable more cheaply, and it is not free of cost:
...
So the decision is phased rather than reversed: the original mechanism is exactly right *for the thing
...
2. **Trust.** One **internal root CA**, generated once and living in `production/foundation/` `[P]` — the
   ...
   either way.
3. **Distribution — a build dependency, not a runbook note.** The root has to be in the trust store of
   every client that speaks to an internal endpoint:
...
4. **The CA private key is state.** Generating it with the Terraform `tls` provider puts it in the state
   file; the state bucket's KMS key is then inside the CA's trust boundary. Acceptable at lab scale — the
   alternative, generating it offline and keeping only the certificate in Terraform, is available if the
   trade stops being comfortable.
...
AWS-served surfaces need none of this: the Identity Center access
...
## Phase 2 — Stage 13: the public domain

Stage 13 registers the domain, creates the **public** hosted zone, and issues public ACM certificates for
the public ALB (and, for CloudFront, in `us-east-1` — `docs/plan/architecture.md` §4.1). This is the original
D15 mechanism, applied to the one tier it fits.

**One question is left to Stage 13:** whether the internal endpoints
...
The revision takes ~USD 1.50/month off the pre-Stage-13 floor; the argument is the name leak and the
removed blocker, and `docs/plan/cost-model.md` and `docs/PRICING.md` carry the figure.
