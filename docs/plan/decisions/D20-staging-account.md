...
**Applied 2026-09-06.** The account is `Staging Account` in `Workloads`, at **10.50.0.0/16**, and **10.40.0.0/16 is free** from Stage 6b step 4.5, and **stays unallocated**: 6c step 0.2 builds the hub from 10.30 (this VPC, re-labelled), 10.31 and 10.32, and D38 reserves 10.60 for the account its trigger names. One sentence of this decision died in the doing: `production/foundation/peers.tf` opened its exclusions list with *"No peering to Staging (D20)"*, and the peering it has held since Stage 3 **is** the peering to Staging. The topology never changed; the name the VPC answers to did.
