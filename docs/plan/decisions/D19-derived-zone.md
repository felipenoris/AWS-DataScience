...
- **EBS and RDS snapshots — the route that bypasses everything this decision leans on.** Derived bytes read
  ...
  preventively, by an unconditional deny in the organization-root SCP set — **Stage 1c step 7.5**.
  RCPs are limited to a service list and SCPs are not, which is why the fix lands on the
  identity side rather than beside the rest of the perimeter.
- **EFS (D24) — retired 2026-08-17.** It recorded the one route nothing closed: no RCP
...
**Revised 2026-08-19, later the same day (the user) — the `security-zone` dimension is withdrawn and
practice (vi)'s key is `alias/awsds-<env>-data`.** The zone framing two blocks up is
...
## The zone re-homed onto the SMUS project path

Revised 2026-08-26, the user's decision. **What was measured, the same day, before the choice** (Stage 6
step 2.4's reading): the Tooling blueprint gives every project an **enforced** Athena workgroup writing into
...
**Production and Staging are not covered by this revision.** They do
