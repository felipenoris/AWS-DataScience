...
With the three-layer model the projection is a ~USD 21-27/month floor plus ~USD 0.28-0.40 per lab hour, so roughly **USD 29-31/month** at the expected usage — the figure in `docs/plan/cost-model.md`, which this row now quotes instead of the pre-D20/D21/D22 estimate of "USD 26-27" it carried until 2026-08-08. This ceiling is what rules out always-on GitLab (~USD 60/month by itself) and confirms the stop/start approach.
