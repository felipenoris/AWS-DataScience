...
The projection lives in [`docs/plan/cost-model.md`](../cost-model.md) and this decision carries no copy of it (amended 2026-08-21). That file computes the monthly floor and the per-lab-hour metering, and its Floor row is the one that moves when a build adds a resource — twice already, both KMS: +USD 1.00 for the lake CMK on 2026-08-18 and +USD 3.00 for Stage 6's three on 2026-08-21. The "~USD 21-27/month floor" this paragraph carried until then was the pre-D29/D31 figure `cost-model.md` retired on 2026-08-08 as an understatement, so a reader comparing the two files was comparing two eras of arithmetic, the older one flattering the ceiling (Lesson 7). The revision trigger below is written against the current projection. What D12 owns is the choice of **USD 50** — the ceiling that rules out always-on GitLab (~USD 60/month by itself) and confirms the stop/start approach.

**How the ceiling is enforced** (amended 2026-08-09). The Stage 1a budget alerts at 50/80/100% and the Cost Anomaly Detection alongside them were **both skipped by the user**, so the AWS Budget exists and notifies nobody: USD 50 is a target, not a threshold anything reacts to. The ceiling is enforced by the operating model instead — the `[E]` teardown of D11 and a manual look at Cost Explorer. That is a weaker enforcement of the same number, and it is deliberate: the alerts are free and can be added in minutes whenever the trade stops being acceptable.
