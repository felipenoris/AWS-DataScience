...
**Amended by [D39](D39-access-by-identity.md):** the laptop reaches the data plane by identity, and a bucket perimeter admits the persona by principal for exactly the actions granted here.
...
**Referenced by stages:** [Stage 1b](../stages/stage-01b-identity-and-controls.md), [Stage 3](../stages/stage-03-networking.md), [Stage 5a](../stages/stage-05a-data-foundation.md), [Stage 9](../stages/stage-09-deployment-targets.md)
...
The one write the data scientist has toward the lake is the **ingestion drop-box** in Data Governance (`s3:PutObject` into a dated prefix — no `GetObject`, `DeleteObject` or `ListBucket`; a letterbox, not a shared folder), granted cross-account by bucket policy to the Interactive-OU roles **and by a matching statement in `DataScientistAccess` itself — both halves, INT-10 as amended (Lesson 28)** — rather than by a sign-in. **Not built:** any general-purpose exchange bucket between environments, which would be a promotion path running parallel to the Stage 8 approval gate. **One consequence:** every `aws:SourceVpce` deny in a bucket policy (Stage 5a, Stage 9) is evaluated against callers from *other* accounts, and a data scientist at a laptop arrives through no endpoint of this estate. The deny therefore admits the persona by principal for exactly the actions this decision grants — the drop-box write, and from Stage 9 the read of the named output prefixes (D39, INT-05) — or every call fails with an `AccessDenied` whose cause is invisible from the error.
