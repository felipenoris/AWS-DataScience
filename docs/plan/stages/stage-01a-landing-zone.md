...
- **A quota increase was requested at 15** (Service Quotas → AWS Organizations → "Maximum number of
  accounts"; it is a global setting but the request is filed from `us-east-1`). It is recorded in `docs/log/log-stage-01a-landing-zone.md`
  as *requested*. **Confirm it was granted before vending the last accounts** — the granted value goes in
  `docs/log/log-stage-01a-landing-zone.md`.
...
this list**, and record it in `docs/log/log-stage-01a-landing-zone.md` beside the vend itself:
...
4. Create the `Sandbox`, `Development`, `Staging`, `Production`, `Data Governance`, `Identity` and
   ...
   **Create the OUs from the Control Tower console, not from AWS Organizations.** An OU created directly in
   ...
   - `Interactive` OU → `Development`, plus a nested **`Sandboxes` OU** holding the `Sandbox` accounts, one
     ...
     vended unit governed the moment it lands. Created this way on 2026-08-09 (`docs/log/log-stage-01a-landing-zone.md`); it is the reason the
   ...
   - `Security` OU → the Log Archive and Audit accounts Control Tower created, **and nothing else.**
     `Identity` was meant to join them and **could not** — the vend was refused on 2026-08-09 (`docs/log/log-stage-01a-landing-zone.md`).
   ...
   **The accounts listed above are the complete set *for this stage*, which is not the same as the complete
   ...
   executable. `docs/plan/institutional-delta.md` records what a larger organization would still add beyond them.
...
5. **Break-glass: the procedure and the alarm (D16).** Three deliverables: the procedure written down, the
   ...
   **5.6 — Record it.** Resource names and the test result in `docs/log/log-stage-01a-landing-zone.md` (the user's file), and the date in
...
6. **Centralized root access management.** Every member account — the ones Control Tower created (Log
   ...
   **that report is the only time anybody will ever look at that account's root**: anything in it other than
   "nothing" belongs in `docs/log/log-stage-01a-landing-zone.md`. The deletion removes password, access keys and signing certificates and
   ...
   **6.5 — Expect the break-glass alarm to fire, and take it as the second test of step 5.** The *actions*
   ...
   arrives on both channels here, step 5 never worked**: `docs/log/log-stage-01a-landing-zone.md` records the 5.5 test as performed but not
   ...
   **6.8 — Record it in `docs/log/log-stage-01a-landing-zone.md`**: date, which capabilities were enabled, whether a delegated administrator
