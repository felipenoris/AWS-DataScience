...
| vii | **Answered: 94,767 RPU-seconds = 26.3242 RPU-hours = 9.4767 USD**, and **recorded wrong twice first** (0.0405, then 0.4743) because both readings were taken of a meter that was still running: one query ran 6 h 33 min after its client stopped polling. **19% of the D12 ceiling.** `ComputeSeconds` lands per half-hour, and Cost Explorer has no same-day data at all | 0.3a, 5.3 |
| xiii | **Answered 2026-09-20 by the user: there is NO credit of any kind** on the Management credits page, so no trial is active and every figure in this stage is billed in full. Decision 9 (*"do not manage it, but do not waste it"*) is moot, and the second window Stage 9 was expected to open should not be assumed either | 0.3a |
