...
| iii | Does the Organizations **policy** delegation coexist with the Control Tower landing zone without raising drift? **The one verification Stage 2 leaves open, and by access rather than by difficulty:** it is a landing-zone read in **Management**, which no `awsds-infra-*` profile reaches — `AWS Control Tower Admin` / `AWSAdministratorAccess`, the same path as 5.1. **Instrumented 2026-08-16: [`aws/cloudshell/management-landing-zone-drift.sh`](../../../aws/cloudshell/management-landing-zone-drift.sh)**, the third script in the folder for accounts with no profile. It does not stop at the flag, because the flag alone cannot answer this: `IN_SYNC` is a **weak negative** — 1d settled that Control Tower compares a closed list of objects it owns, which is also why 1c's ten customer documents never raised drift. So it also prints **the manifest** (what the flag is comparing, and whether anything in it concerns resource policies), **the operation history** (an update that ran *after* 2026-08-15 and returned `SUCCEEDED` is the only *positive* evidence obtainable without writing), and **whether the document is still present and still narrowed to two statements**. **The strong test — `update-landing-zone`/`reset-landing-zone` — is a write and is refused** (Lesson 22: a control whose failing case the harness cannot produce is verified by reading); the occasion arrives on its own at the next real landing-zone update | 5.1 |
