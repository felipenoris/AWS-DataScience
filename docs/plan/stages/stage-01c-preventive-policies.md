...
neither fast nor freely reversible. A `Stage 1b step 7` reference elsewhere means this file's
`Stage 1c step 7` — the same step under a different prefix. Navigate by **7.0-7.8**.

**Two sittings, not one.** 7.7 alone is a dozen Control Tower control operations, serialized, minutes
each, blocking other landing-zone operations while they run; the battery of 7.3 is per policy and reads
CloudTrail, which lags. So:
...
written after the thing it guards has already been used arrives late. That is the argument for attaching
policy over accounts that are still empty.

## Before you start — what this stage may not begin without
...
anything reaches a real OU** (D29). The battery is a procedure, not a property.

## What Stage 1b and the 2026-08-13 revision established

Stage 1b closed on 2026-08-12 and `CLAUDE.md`'s objectives were revised on 2026-08-13. Six things enter
this stage as **measurement** rather than as assumption.
...
## What 7.0 measured on 2026-08-13, and where it changes what is left

**Four of these were assumptions this file stated as fact.** The evidence is in the log and in the
snapshots; what follows is only the consequence for the steps that have not run. Each row is settled where
it lands, not here.
...
**Every policy below needs identifiers this plan deliberately does not carry** (`CLAUDE.md`
...
> **Run 2026-08-13, as two scripts rather than a dozen commands with ids threaded between
...
**2. What Control Tower has already attached.** This is the measurement that shrinks the work:
...
**Measured 2026-08-13, and it shrank the work in an unpredicted direction.** There is one
`aws-guardrails-*` policy on every OU **except `Sandboxes`**, plus
...
- **CloudTrail is denied nowhere.** No `cloudtrail:` action appears in any of the six documents. The user
  settled it the same day as a **deliberate gap** — see 7.5.
...
Two consequences, and both still hold:
...
- If a policy read is denied from the Identity profile, that is information rather than a failure: the read
  ...
  Admin** and record which way it went. *(Answered 2026-08-13: none was denied.)*

**One finding belongs to 7.7 rather than to 7.5: `Sandboxes` carries no guardrail policy at all.**
...
Account Factory vend (1a log) and Account Factory only offers registered OUs — **and step 2 already made
`Sandboxes` the expected exception**: it is the one OU with no guardrail policy.
...
report under `aws/output/`). It answers three things at once: whether `Sandboxes` is a registered target,
the enabled-control diff between `Security` and `Identity` that 7.6 asks for, and the baseline 7.7 is going
to add to.
...
describes; Organizations quotas answer in `us-east-1`:
...
*"measured, not reasoned"* cannot be honoured.

**It does not bind under either the old caps or the new ones.** The **root** carries `FullAWSAccess` +
baseline + perimeter + the tag-forcing SCP = 4;
...
#### 7.1 — What makes this step different, and the rules that survive from D30
...
- **A policy attached here has no in-account repair, which is why 7.3's battery is not optional.** A
  mistake is undone by detaching the policy from the Management account, which is exempt from SCPs by
  design (D16), so the battery *is* the control and having the Management console already open is a
  precondition rather than a precaution.
...
- **Write each document as a file *before* pasting it, and keep the file.** The folder exists since
  ...
  **Every statement gets a row in [`POLICIES.md`](../../../terraform-live/identity/org-policies/POLICIES.md), in
...
- **There is a size budget, and it cannot be read from the API — only from AWS's documentation** (7.0 step
  5 has the numbers and the count). Control Tower's guardrails consume part of the SCP allowance at every
  OU, **and 7.7 consumes one more slot per OU**. **Prefer one well-`Sid`-ed policy per node to several thin
  ones**, and watch the RCP side rather than the SCP side — that is the tighter budget now.
- **A third rule arrives with D34: it survives an account being added later.** OUs
...
#### 7.2 — Preconditions, in this order
...
- **The trusted-resources deny is tested by its complement, because the obvious test cannot be run and its
  evidence would not exist.** Attempting a `PutObject` to a bucket outside the organization and reading the
  CloudTrail record fails on two independent problems:
  ...
  **Run the same complement against ECR, because key population is per-service and the S3 run says nothing
  about it** (the ECR half of the 7.5 statement). What the bucket run proves is that
...
- **Then move the attachment to its real target** — the OU named in 7.4/7.5, or the organization root for
  ...
  **The canary has one permanent limitation:** once the root set is attached, `Policy Canary` inherits it
  forever, so every later candidate is
...
- **Verification (iii) belongs to 7.0 step 2**: whether the hand-written SCPs and RCPs conflict with the
  ones Control Tower manages is a thing to read *before* writing, not a surprise to notice after attaching.
  What stays here is the residue — a candidate whose deny composes badly with a Control Tower guardrail
  shows up as a `must still succeed` failure in the battery.

#### 7.4 — The order of attachment. This is an instruction, not a listing order

Two pairs interlock, and attaching to the OUs in listing order breaks one of them.
...
1. **Account-level S3 Block Public Access in every account, enumerated rather than implied.** The
   ...
   **The interlock is not a one-off; treating it as one leaves every future account without the
   setting.** D34 keeps vending as a standing capability and D35 says
   ...
   >
   > **What the carve-out costs: it is the one wildcard-account ARN in this
...
**The set is two documents, not one.** The perimeter statement is the one
...
- **Deny `organizations:LeaveOrganization`, measured rather than reasoned: a real principal can call
  it.** Every vended account carries `AWSOrganizationsFullAccess` →
...
- **CloudTrail, Config and GuardDuty: the gap is measured** (7.0 step 2, 2026-08-13). Three different
  answers, and writing them as one line is how the wrong one gets written:
  ...
  - **CloudTrail — denied nowhere, and deliberately left that way** (user, 2026-08-13). **No `cloudtrail:`
    action appears in any of the six guardrail documents.** The deny was still not written, and the reason
    is what makes it a choice rather than an omission: the trail is **organization-level and lives in the Management
...
- **Deny writes to S3 and ECR resources outside this organization** (`aws:ResourceOrgID`) — the
  ...
  **ECR was added 2026-08-12 (user)**, the second amendment to this statement and what the two-document
  ...
  - **`ecr:GetAuthorizationToken` is deliberately not in the list, and the reason generalizes into a rule
    the whole document is written under** (`docs/plan/conventions.md`). It is
  ...
  **Write the condition the way 7.8 writes the RCP's; it is the same trap:**
...
- **Deny sharing snapshots and AMIs outside the organization** (added 2026-08-12, out of D6 and the D19
  ...
  (`docs/plan/architecture.md` §4.2), which is why this is an SCP, here, and not part of 7.8's perimeter
  RCP. Four precisions:
  ...
  - **No carve-out** — the same standard 7.6 applies to the `CreateNotebookInstance` deny. Nothing this project builds shares a snapshot outside the organization,
...
- **Deny `ecr-public:*` outright** (added 2026-08-12, user). ECR Public publishes to a world-readable
  ...
  organization and the data is public anyway — which is why the perimeter statement in
  ...
  - **This does not break pulling from `public.ecr.aws`.** An anonymous pull from the public gallery
    involves **no IAM action at all** — it is not an
  ...
  - **It does *not* overlap the Region control — measured 2026-08-13, against a prediction that ECR Public
    being a `us-east-1`-only API would make `CT.MULTISERVICE.PV.1` deny it already.** **`ecr-public:*` is
    one of the 86 entries in AWS's `NotAction` list**, so the Region control exempts it outright and
    ...
    shape: "another control probably covers this" is a guess about a list somebody else maintains, and the
    honest version is to read the list — which is what verification (vii) is for. **This is not the
    CloudTrail/Config case above** — there the duplication was of a Control Tower control with the *same*
    intent, and the instruction was to write only the gap. Here a different control catches the action for
    a different reason, and one statement is the cost of not depending on that.
  - Tested in the 7.5 window, before 7.7, and **re-run after the Region control was enabled on
    `Policy Test`, where it still comes back naming `p-1fp032g8`**, the baseline — the Region control never
    reaches this action.
...
- **Deny `s3:PutAccountPublicAccessBlock`, with the decision-7 carve-out**, protecting the setting enabled
  ...
  **Never declare `aws_s3_account_public_access_block` in a Terraform slice.** It is the obvious
...
*Recorded under 7.6 rather than back in 7.5: both amendments came out of **one** review held after the
per-OU documents were attached, by the same method as the OU documents' own re-read.*
...
**The finding that outlives this sitting is about probing.** The
...
recording *untested* too early understates the ceiling.
...
**The residual:** if Stage 6 ever provisions a project *into* Data Governance,
...
**What was added or narrowed while writing, each with its reason:**
...
> **The Unified Studio surface: every deny below is written against an API, not against a feature name.**
> `CLAUDE.md` gained six named SMUS features on 2026-08-13, and one of them —
...
- **`Workloads` OU** (D20) — deny `sagemaker:CreateDomain`, `sagemaker:CreateUserProfile` and
  ...
  **What is *not* the gate.** The account association is
...
- **`Interactive` OU** (D21) — **no SCP implements "human infrastructure changes denied".** That phrase
  appeared across six files as a property of this OU's SCP set. What stops the data scientist from
  creating a VPC is `DataScientistAccess` and its permissions boundary — an **identity** policy,
  enumerated by hand, which is the sort of thing an SCP backs up rather than the thing an SCP *is*
  (Lesson 5). Corrected across those files on 2026-08-09.
  *(Note the tense: `DataScientistAccess` does not exist yet — it is written at Stage 2 step 5, from the
  ...
  >
  > **The table above settled it.** The candidate was left unadopted because
...
- **`Identity` OU** (D10, D23 as revised 2026-08-09) — **the tier this plan did not have, because the
  ...
  - **The baseline is established, against the assumption that it would not be** (7.0 step 2,
    2026-08-13). *"An OU created outside Control Tower's own flow carries no policy set until code
    attaches one"* is **false here: `Identity` carries `aws-guardrails-coSzJr`, the standard eight
...
#### 7.7 — The Control Tower managed controls: use theirs, do not hand-roll them
...
> **`Sandboxes` is the one to go in expecting trouble from** (7.0 step 2, 2026-08-13). It is the
...
- **Region restriction.** Goal: **`us-west-2` as the only allowed region**, and there are **two** managed
  controls for it (the table below). Three reasons it is not in the hand-written set:
  - **Global services resolve to `us-east-1`.** `iam:CreateRole` goes to `iam.amazonaws.com`, which
    ...
    the allowed regions: the second is much
    looser, permitting `ec2:RunInstances` in a region with no Config recorder, no GuardDuty and no
  ...
  **What to have before you start.** The home region cannot be denied, and *nothing must already
  ...
  **Enabled on `Policy Test` 2026-08-13, and verification (vii) is answered from the deployed document
  ...
  (`p-q3y11w1n`), one statement, `Sid: CTMULTISERVICEPV1` — so it can be read back with
  `organizations describe-policy` from Identity and diffed like anything else, repeatably, per OU.
...
- **The two root-user controls, and the one parameter that decides whether they break 1a step 6.**
  ...
  > this the first control in the stage whose verification is *reading* rather than *attempting*.
  >
  ...
  > root lives, is SCP-exempt regardless (D16). **The canary is what made that true**: the mistake happened
  > before the control reached anything real.
  >
...
- **RCPs** — deny access to **S3, STS, KMS, SQS, Secrets Manager, DynamoDB and ECR** from principals outside
  ...
  **Seven services, not five — widened by the user on 2026-08-12.** The first five were the set RCPs
  ...
  - **DynamoDB is inert today and is adopted anyway.** **This project has no DynamoDB table and no plan
    for one** — D3 settled Terraform state on native S3
...
- **Tag policies** — standardize the mandatory tags from `docs/plan/conventions.md`, with one precision:
  tag policies constrain *tagging operations*, they cannot force a resource to be created with tags at
  all. **The forcing function is an SCP with `aws:RequestTag`/`aws:TagKeys` conditions on the create
  actions that matter.** One or the other, or the tags are a convention.
  ...
  **The forcing SCP is a decision, and it is the one policy in 7.5-7.8 that binds the builder as hard as
  it binds anyone** (Lesson 18). Three things it breaks if it is written broadly:
  ...
  **So adopt it narrowly or not at all**, and adopt it *here*, in writing: the defensible scope is the
  ...
  Tower principals per 7.1's enumerated-ARN rule. It is **decision 5**, due while executing, and its cost
  lands on a stage nobody is looking at yet.
  **`s3:CreateBucket` is excluded, because it contradicts the first bullet above.**
  ...
  > **Measured 2026-08-13: the first half of that instruction came back *yes*, so the premise this
  ...
  > document.
  >
  > **What this leaves decision 5 as:** `rds:CreateDBInstance` is
...
- **Declarative policies** — enforce IMDSv2 and EC2 public-access defaults org-wide. Policy type enabled
  ...
  **What "public-access defaults" means, and the limit the name hides (2026-08-12).**
...
- **Every attached policy ID is recorded** in `docs/log/log-stage-01c-preventive-policies.md`, beside the filename
  ...
  and what makes Stage 2 step 5.5's import land on an empty plan.

## Decisions

**Three of the landing zone's decisions were settled before execution, on 2026-08-13, so this stage no
longer opens with a blocking question.** Each one is still written into
`docs/log/log-stage-01c-preventive-policies.md` at the moment it is applied (Lesson 16):
...
**Three are already answered, on 2026-08-13, and they are kept here with their answers**: a verification
whose answer is only in a log is one the next reader re-runs.
...
**(vii), answered from the documentation rather than by execution:** *"is Region deny landing-zone-wide,
i.e. untestable against `Policy Test` first?"* — **the landing-zone control `GRREGIONDENY` is; the
OU-scoped `CT.MULTISERVICE.PV.1` is not, and can be canary-tested.** Decision 6 replaces the question.
