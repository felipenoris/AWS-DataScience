...
- **The blast radius this creates, which is easy to miss and is load-bearing.** An Identity Center
  ...
  `docs/ORGANIZATION.md` already ascribes to this account; it is recorded here because D33's groups make it
...
display name** rather than by GUID (`docs/plan/conventions.md`, "The identity seam"). Nothing here changes if
...
- **It restores the shape the other six sets already follow** — `<Persona>Access` — so the administrator set
  stops being the only one named after a *permission level* rather than a group. `docs/plan/conventions.md` asks
...
it `PowerUserAccess` "until Stage 6", which contradicts `docs/ORGANIZATION.md` ("no permissions to perform
...
with a **permissions boundary** and `PassRole` scoped per the IAM rules in `docs/plan/conventions.md` §6.
...
- **This is where verification (vi) is *opened*, and it cannot be closed here.** The question is whether
  ...
  `docs/log/log-stage-01b-identity-and-controls.md` and in D32. Stage 2 step 5.2 then models nothing about them,
...
- **Outcome: every measured account is identical, and Stage 3 anchors on `zone_id` anyway.** The full
  table is in `docs/log/log-stage-01b-identity-and-controls.md`. This step used to end "*if* the mappings differ,
  ...
  error. `docs/plan/architecture.md` §4.1 carries the reasoning; `docs/plan/open-questions.md` item 3 is closed on it.
  ...
  **`aws-controltower/CloudTrailLogs-*` in the Management account** — 1a step 5 established exactly this
  ...
  - **Do not filter by group. Match the six events above and nothing else** — settled 2026-08-09, and it
    ...
    **This inverts at scale, and that is written down rather than left implicit:** with hundreds of data
    ...
    fortnight — which is worse than not having it. `docs/plan/institutional-delta.md` carries the row, and the
