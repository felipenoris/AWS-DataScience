...
4. Peer public keys supplied through a git-ignored `.tfvars` (keys are generated on the client and the
   ...
   **The peer network gets its own CIDR, chosen here and written down: `10.90.0.0/24`.** It has to avoid all
   ...
   Record it in `docs/plan/architecture.md` §4.1's spirit: a variable with this default, not a literal scattered through the module.
...
API endpoints, which the full tunnel already sends out through the WireGuard Elastic IP (`docs/plan/architecture.md` §3, "How a human
