...
| **Prerequisites** | Stage 3 — specifically `sandbox/foundation/` (the public subnet, the S3 gateway endpoint **and the 9.3 allow-list**, which this stage is the first to exercise) and the Sandbox↔Production peering (Stage 3 step 6). Stage 3 step 0 is done (2026-08-16); **passes 1-3 are not, and every step below waits on its pass 1**. D4 is decided: self-managed WireGuard |
...
- **1.3 — [Claude] Register the slice in the D11 machinery, in the same commit that creates it** — three
  edits, the first two corrections to this step's earlier wording:
