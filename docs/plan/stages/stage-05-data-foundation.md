...
| | |
|---|---|
| **Status** | **EVERY PASS HAS RUN — 0-4e AND 6 (2026-08-18/20).** **Pass 6 ran 2026-08-20 in two legs**, by hand as `AWS Control Tower Admin` → `AWSAdministratorAccess`, CloudShell: the delegation from Management, then central configuration from Audit — a `NO_REGIONS` aggregator, `ConfigurationType: CENTRAL`, the `awsds-fsbp-only` policy on the **root**, and Management carved out `SELF_MANAGED_SECURITY_HUB` (13.1c). **Sixteen of the eighteen associations settled `SUCCESS` — all seven OUs and all nine live accounts**; the two that did not are the **suspended `Sandbox` and the `ROOT` above it**, which is why `EXC-01` now carries a *narrower* invariant than "every row `SUCCESS`". `./aws/datalake.py`: **0 FAILED**, `DL-11` `enabled` + `FSBP on` in all six profiled accounts with **`V2 PRODUCT` `absent` everywhere** (13.0 holding). `INV-09` restated by 13.4 to **nine principals, four delegations** — `securityhub` → `Audit Account`, `ACTIVE` — measured, not derived. **What is NOT done is 13.3's triage**: it needs a first report, and enablement is minutes old. **Earlier: PASSES 0-4d DONE (2026-08-18/20)** — pass 4a/4b applied 2026-08-19: `terraform-modules/consumer-data/` v0.1.0 and the two consumer slices, `1 added` + `15 added` per account in Recipe D's two steps, both re-planning `No changes`, `./aws/datalake.py` **0 FAILED**. **4d closed 2026-08-20 by the `DenyControlPlaneOffVpn` amendment and the proofs it unblocked**: applied `0 to add, 6 to change` → re-plan `No changes` → read back on **both provisioned roles** → `VP-7` both halves → then, as the persona, the drop-box asymmetry in three verbs and **D13's own mechanism** as the implicit deny it argues from (Lesson 33 closed; Lesson 28 amended to three terms by the `PutObject`'s `SSEKMSKeyId`). **4e DONE 2026-08-20 — pass 4 is closed**: `athena:StartQueryExecution` added to `DenyUserCompute` in **both** documents by **Recipe A, not by hand** (the item's own instruction was stale by a stage — correction in its step 3), `0/3/0` with one declared rider, re-plan `No changes`, both read back, phase-4b re-probed from both accounts plus the *must still succeed* trio — and the probe found that **Athena's denial names no policy**, so the attribution moved into a contrast probe and `probes.py` gained three entries rather than two. What remains of the stage is **pass 6** (Security Hub); one input stays owed to the user — **open question 19, the crawler demander**, which no proof can settle; **4c DONE 2026-08-19** — the seven persona statements applied and read back from both provisioned roles, carrying a finding: the drop-box write is cross-account, so pass 1's bucket policy was only ever half of it (6.2 corrected). Earlier: **PASSES 0-3 DONE (2026-08-18/19)** — the governed lake exists, its grants are made and it is shared: pass 1 `58 to add` applied in two steps, pass 2 `9 added` (the governance manager's own grants), pass 3 `4 added` (the two cross-account shares, four RAM shares `ACTIVE` and held, zero invitations, INT-11 closed). **Passes 4 and 6 remain** — the consumer side, which now opens with a data lake administrator per account (7.3), and Security Hub. Each pass row below carries its own result. **Originally revised 2026-08-16 into the pass/verification format**, with five corrections against earlier stages folded in: the `CROSS_ACCOUNT_VERSION` defence moved to the step whose apply actually touches `DataLakeSettings` (5.4, not 7), the EFS mount rule rewritten against the WireGuard instance's SG (Stage 4's NAT means the peer CIDR never reaches AWS), step 3's role-protection re-read as already delivered by Stage 2's shared deny fragment, step 9 split into what can land now versus what waits for the blueprint-provisioned roles (INT-15), and step 13's delegation mechanics written for the two accounts that hold no CLI profile. **Revised again 2026-08-17 after the data-governance review** (AWS guidance read against the plan; links in `docs/REFERENCES.md`): the LF-Tag ontology carries a **`zone` dimension** from day one and the consumer grants are **LF-TBAC expressions scoped by classification** (`restricted`/`personal` by explicit grant only — Stage 11 then narrows *within* restricted instead of beginning enforcement); the sample table gains a restricted column so the share deliverable proves the scoping; the grant *method* joined decision 5 and the LF-TBAC cross-account prerequisite joined 7.1; the results-zone ceiling on decision 6's grain is stated at step 8; **the raw share to Sandbox is kept deliberately** (data engineers develop the raw→curated ETL there — `docs/plan/institutional-delta.md` row added); and the missing quality gate is declared (3.8). **Revised once more later the same day (2026-08-17): the user withdrew the NFS requirement from `objectives.md`, and [D24](../decisions/D24-shared-filesystem.md) is withdrawn with it** — pass 5 (steps 10-12, the `sandbox/nfs/` slice), question vii and the EFS cost row removed; no other step consumed the filesystem |
...
| **6** | 13 | Security Hub org-wide — **the step was REVISED 2026-08-20 before it ran** (its mechanism did not exist as written): the product is **Security Hub CSPM**, not the v2 product beside it, and the org-wide mechanism is **central configuration** on the root, not auto-enable. **RAN 2026-08-20, both legs** — `DL-11` `enabled` + `FSBP on` in all six profiled accounts, v2 `absent` everywhere, `securityhub` delegated to `Audit Account` `ACTIVE`. **13.3's triage is the one thing left**, and it needs findings that do not exist yet | by hand: Management (the delegation), then Audit (everything else) | `AWS Control Tower Admin`, **CloudShell preferred over console** — 13.1a's linked-Region collision |
...
**4a, 4b and 4c are DONE (2026-08-19).** The four debts below are what "pass 4" still means. **4c — the
persona grants in `identity/sso/`** — was deliberately sequenced *after* the slices rather than with
them: the permission set is one document provisioned into many accounts, so before these slices existed
the derived-bucket and workgroup ARNs could only have been wildcards; sequenced after, they are read
from the two slices' state and enumerated exactly (`backend.py` emits `data_consumers` + `lake`; the
slice gained its fifth and sixth cross-account lookups). Applied as **seven statements** in
`DataScientistAccess` (`1 changed`, re-plan `No changes`, and both provisioned roles read back carrying
all 18 statements with the `$${aws:userid}` policy variable intact): the Athena run family on the two
...
1. **The crawler pair (3.3, verification (iii))** — the phase-4 positive half. `awsds-data-catalog-maintenance`
   ...
   **CLOSED 2026-08-19 as far as it can close (4d's act 1)**: the negative half fired from
   `InfrastructureAccess` in the `Data` OU — *explicit deny in a service control policy*, the statement's
   first exercise — and the positive half is **unreachable by any existing principal** (trust admits
   `glue.amazonaws.com` alone, `Schedule` null; Lesson 22, closed by reading). The missing DEMANDER is
   open question 19; the maintenance-pair deliverable bullet carries the full correction;
2. **the drop-box asymmetry halves that exist** (the writer's `PutObject`, its denied `GetObject`, the
   ...
   consumers (4d groups A/B) — all three verbs dying on `DenyControlPlaneOffVpn`, one unrelated deny —
   and the amendment applied that day unblocked it. Measured from the persona in `Sandbox 1`, tunnel up:
   **`PutObject` succeeds**, and `GetObject` on the object it just wrote, `ListObjectsV2` on `incoming/`
   and `DeleteObject` on its own object are each denied **implicitly** — absence of grant, not an
   intervening rule. `AllowInteractiveWriterPutOnly` moves from attached to **exercised** (Lesson 20
   discharged for it), and D18's refusal of an exchange bucket becomes a construction rather than a
   convention. Two things the proof produced beyond its own claim: the success response's `SSEKMSKeyId`
   showed a **third** grant on the call — the lake CMK's key policy — which amends Lesson 28's arity; and
   the object it left is uncollectable until Stage 9's pickup exists (`AWS_STATE.md` `EXC-02`). The
   `Development` leg was deliberately not run: the resource half is an `ArnLike` covering both accounts
   and both provisioned roles were read back, so a second uncollectable object buys weaker evidence.
   **The crawler's read stays with item 1** — it has no demander;
...
4. **4.3's `athena:StartQueryExecution` amendment to `DenyUserCompute`, LAST**, through battery phase 4b.
   ...
   only thing it has to follow is any Athena-borne write *in this account*, i.e. item 3. **Item 3 is done
   (2026-08-20, 12 rows in), so the last blocker is cleared and this is the stage's next act.** The
   sequence below was worked out on 2026-08-20 from `POLICIES.md`, the battery runbook and `probes.py`;
   the two findings marked ⚠ are corrections to what this item used to say.

   1. **⚠ TWO DOCUMENTS, ONE SITTING — not one.** `POLICIES.md`'s `Identity` section states it and this
      item did not: `DenyUserCompute` exists identically in `awsds-org-scp-ou-data.json` **and**
      `awsds-org-scp-ou-identity.json`, deliberately, as one idea in two files. `./scripts/check-index.py`
      compares **`Sid`s, not action lists**, so a one-document amendment passes every gate and leaves the
      two silently divergent — **Lesson 14 exactly**, in the shape the file itself predicted.
   2. **⚠ THE COST OF CLOSING IT, stated before rather than discovered after.** `DenyUserCompute` carries
      **no condition**: after the amendment, *nothing in Data Governance can run an Athena query, ever* —
      `InfrastructureAccess` included. The reading that proved the sample rows exist (`count(*) = 12`,
      taken in-account on 2026-08-20) **stops being reproducible from that account**; the equivalent
      afterwards is a query from Sandbox or Development over the share. That is the design working —
      D13 says the lake account is not a query surface — but it removes a diagnostic, and a later session
      debugging the lake will meet the deny and should meet this sentence first.
   3. **⚠ CORRECTED 2026-08-20, ON THE SITTING THAT RAN IT. This item used to say "amend both JSONs,
      then `update-policy` in place", and that instruction was stale by a whole stage.** It was written
      from `POLICIES.md` and the battery runbook, both of which describe the **Stage 1c** procedure, when
      these documents were pasted into the console by hand. **Stage 2 step 5.5 adopted all ten into
      Terraform** (`aws_organizations_policy.this`, imported, `prevent_destroy`), so a hand
      `update-policy` is drift the next apply reverts, and it skips the four `precondition` blocks that
      exist to catch exactly a bad amendment. **It would have broken something concrete here:**
      `awsds-org-scp-ou-data.json` carries `<ACCOUNT_ID_DATA>` inside the D27 crawler carve-out, and
      uploading the tracked file raw leaves an `ArnNotEquals` comparing against that literal string —
      the carve-out silently matches nothing, with no error anywhere. **Lesson 11**: Stage 2 changed
      *who authors* these documents, which invalidated every procedure written about them.
      **The procedure is [Recipe A](../runbooks/terraform-changes.md)** — edit the JSONs, `gen-backend-hcl`
      + `gen-tfvars`, `init`, plan to a file outside the repository and read it, apply that exact file,
      re-plan for `No changes`, then read each document back from Organizations as 7.5a/7.6a did.
      **DONE 2026-08-20**: `0 to add, 3 to change, 0 to destroy` — the two documents, plus a **declared**
      rider (commit `6a5bf33`'s message ends by naming it: the RCP description, `description` only, no
      `Statement` touched). It could not be excluded without `-target`, which §8 of the runbook forbids
      outside Recipe D, so it was carried deliberately and said out loud rather than dropped in. Re-plan
      `No changes`; both documents read back carrying the action; `CHK-3` still passes, which is the
      placeholder substitution proving itself.
   4. **Re-probe in phase 4b, from `awsds-infra-data` AND `awsds-infra-identity`** (the runbook's rule:
      a document amended and not re-probed is one whose last measurement describes a version that no
      longer exists), plus the *must still succeed* trio. **Probe shape, and its fork**: call
      `athena:StartQueryExecution` against a **workgroup that does not exist**, so a not-denied call
      fails harmlessly rather than actually running something — `safety = blocked` in `probes.py`. Two
      distinguishable outcomes: `AccessDenied` naming the policy = the amendment landed; a
      workgroup-not-found/`InvalidRequestException` = **untested**, because Athena validated before it
      authorized (Lesson 21's fork, per-action and not knowable in advance). **If it lands on
      *untested*, do not escalate silently**: the only escalation is a *real* workgroup, and that
      leaves a failed execution record in the account — a cost to accept deliberately and write down,
      not to slip in.

      **DONE 2026-08-20, and the fork resolved a THIRD way that neither branch above anticipated.**
      Both accounts answered `AccessDeniedException` — so not *untested*, and Lesson 21 resolved the
      useful way: **Athena authorizes before it validates for this action.** But the message
      **names no policy**: a bare *"You are not authorized to perform: `athena:StartQueryExecution` on
      the resource"*. Every probe written before this one happened to hit a service that names the
      document, and the battery's `classify()` encodes that assumption — it files a policy-less
      `AccessDenied` as `DENY-NOT-SCP`, *"an IAM/permission-set deny, not the ceiling"*, which here is
      simply wrong and which no wording will fix. **So the attribution was moved out of the string and
      into a contrast probe**: the same call, same principal type, same region, one session, from
      `Sandbox 1` — an account in `Interactive`, which this amendment does not reach — got past
      authorization and died on `InvalidRequestException: WorkGroup is not found`. Given that, the two
      denials can only be the per-OU documents. **`probes.py` therefore gained THREE entries, not two**,
      and the third is an `allow`; the two denies will report `note` rather than `ok` forever, which is
      the honest reading and is written into the comment beside them. The *must still succeed* trio
      (`sts:GetCallerIdentity`, `s3 ls`, `ec2:DescribeVpcs`) passed in both accounts.
   5. **Then the paperwork that the amendment invalidates**: `POLICIES.md`'s Athena non-coverage bullet
      says in its own words that "this row is rewritten when it lands" — both sections — and
      `./aws/org-policies.py` is re-run (this stage's step 871 already asks for it). `probes.py` gains
      the two new entries in the same sitting, or the battery's next run measures the old ceiling.
      **DONE 2026-08-20.** Both `DenyUserCompute` rows carry the action and the names-no-policy caveat;
      the non-coverage bullet became a blockquote kept *because the shape of the debt outlived it* — an
      allow whose justification had been withdrawn for two days is the state that list exists to make
      visible; the `Identity` row records that what closed there was a **shape without content**, since
      that account has no lake. `probes.py` is at **96** entries (three, not two). `./aws/org-policies.py`
      re-run: all checks passed, both documents attached, **`CHK-3` still passes**. `check-index.py`
      clean, `make check: OK`.
...
that is correct rather than missing. *(**CORRECTED 2026-08-19, at 4c's authoring: the last clause is
wrong.** The persona and the bucket sit in different accounts, and cross-account evaluation requires
**both** an allow in the trusting account's resource policy **and** one in the trusted account's
...
- **The share pair, in both consumers:** a sample Iceberg table written in Data Governance queries through
  ...
  half. **One caveat the reading carries and the bullet did not predict**: the S3 denial arrives as an
  *explicit deny in an identity-based policy* rather than the implicit deny D13 argues from, because
  `DenyControlPlaneOffVpn` fires on every direct S3 call made from the tunnel (the drop-box bullet
  below). D13's own mechanism was therefore unmeasured — masked by a deny that would refuse the call
  whatever Lake Formation had granted. **RESOLVED 2026-08-20, after the amendment applied**: the same
...
- **The workgroup boundary:** a query whose client asks for a result location outside the derived prefix
  ...
  **overrides** the client's `OutputLocation` instead of refusing it, so **the reading is the resulting
  ...
  permission error two causes could explain. **ANSWERED IN BOTH CONSUMERS 2026-08-19 (groups A and B), from two different provisioned roles** - in Development the hijack asked for `scratch/hijack/` and the execution recorded `results/`, exactly as in Sandbox.
- **The maintenance pair:** the raw crawler runs as `awsds-data-catalog-maintenance` (the phase-4 positive
  ...
  **The consequence reaches past this bullet:** D18/D25's ingestion is `PutObject → crawler catalogues`,
  and the two halves were broken for unrelated causes — amending `DenyControlPlaneOffVpn` does not make a
  crawler run. **Since 2026-08-20 the write half is fixed and measured, so this is the only half left**:
  a file can now be delivered into the drop-box and nothing will catalogue it. **And it surfaces an untaken decision: the crawlers' `Schedule`** — whether it exists, at
...
- **The drop-box asymmetry, the halves that exist:** an Interactive-OU session `PutObject`s into the dated
  ...
  for exactly this reason; the identity twin carried one. The log entry of that date carries the
  elimination and the control that isolates it.
  **ANSWERED 2026-08-20, after the amendment applied — and the asymmetry is real in three directions.**
  From the persona in `Sandbox 1`, tunnel up: `PutObject` into the dated prefix **succeeds**;
  `GetObject` on that same object, `ListObjectsV2` on `incoming/` and `DeleteObject` on it are each
  denied **implicitly**. The delete probe is what completes the claim rather than decorating it — a
  writer that can retract is a writer that can launder, so "put-only" is a statement about three verbs,
  not one. The `PutObject` was sent as a **fixed-size file, never stdin**: an unknown-length stream goes
  multipart, and the identity half grants `s3:PutObject` with no multipart companion, so the probe would
  have failed for a reason that is not the one under test. And its success response carried the finding —
  `SSEKMSKeyId` naming the lake CMK, i.e. a **third** grant on the call (Lesson 28 amended).
  **Still owed here: the crawler's read**, which has no demander (the bullet above).
