...
1. KMS CMKs per data domain; S3 buckets `raw`, `curated`, `artifacts`, `logs` with
   versioning, encryption, **S3 Bucket Keys** (`docs/plan/cost-model.md`), lifecycle rules, `prevent_destroy`, and a bucket policy
   that denies access not coming through the VPC endpoint (`aws:SourceVpce`) — the resource-side half of
   the trusted-networks axis in `docs/plan/architecture.md` §4.2, complementing the endpoint policies from Stage 3.
   ...
   Take the policy shape from `data-perimeter-policy-examples` (`docs/plan/architecture.md` §4.2). While in the bucket policy, add a
...
4. Iceberg tables on S3. **Table maintenance gets an owner on day one**: scheduled `OPTIMIZE`
   ...
   on general-purpose buckets. Recorded in `docs/plan/institutional-delta.md`.
...
6. **Implement D13 — make Lake Formation enforceable — which D22 makes structural.** In the old layout
   ...
   **What that objective's *grain* is has been open since 2026-08-13, and it is decided here rather than
   ...
   objective is met at that grain with the difference written down (`docs/plan/open-questions.md` item 13). A
...
9. **Build the derived zone deliberately (D19), in each Interactive account.** D13 makes the *entitlement*
   ...
   data is `restricted`. Nothing enforces that automatically at this scale — it is policy, and `docs/plan/institutional-delta.md` records
   ...
   **Cost:** one CMK per Interactive account, ~USD 1 each (`docs/plan/cost-model.md`). **The zero-cost variant if that ever
...
11. **Access from the user's own machine**, which `CLAUDE.md` asks for ("exchange files between users, the
   ...
   in `docs/plan/institutional-delta.md` as a real gap for an institution.
...
13. **Enable Security Hub org-wide, from the Audit account** (delegated in Stage 1b step 8, moved out of
    ...
    **Cost:** per check and per finding ingested above the free tier (`docs/plan/cost-model.md`). Note the
