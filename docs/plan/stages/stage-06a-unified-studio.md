...
flowchart LR
    subgraph DG["Data Governance · registry, never runtime · D26"]
        DOM["DataZone V2 domain · IdC auth<br/>profiles: experimentation · engineering<br/>SageMaker Catalog on Stage 5a's LF substrate"]
    end
...
and `docs/plan/integrations.md`; `step 2` (the project-role grants) from Stage 5a step 9.3. They do not
...
| ~~1.3~~ | **Done 2026-08-21** — the associations auto-accepted (both members), `SMUS_ASSOCIATED` filled, and the second applies ran: 2c and 2d above | **user** + Claude |
...
> **Read the wording, never the exit code, and read the pair rather than one half.** Stage 5a pass 4e measured
...
(`aws ram get-resource-share-invitations`, the INT-11 shape — the Stage 5a LF shares auto-accepted and
...
> so (b) has nothing to accept and **the 7-day expiry never starts running** — the shape Stage 5a's LF
...
> Stage 5a pass 4 (`docs/SMUS.md` carries the distinction).

**The console recommends ≥ 3 subnets in 3 AZs; D9 built 2 — verification (iii)**, answered before
anything is layered on it.

> **`DataLake` lands on a Lake Formation surface Stage 5a already owns, and the two meet in one
> resource** (written 2026-08-19 from what Stage 5a passes 1 and 3 measured). Decision 4 makes this
...
>   `IAM_ALLOWED_PRINCIPALS` default is born deferring to plain IAM, permanently and invisibly. Stage 5a
...
>   default blocks together (INT-11's failure mode — the reason `DL-5` exists). Stage 5a pass 4 writes
...
`enableTrustedIdentityPropagationPermissions` here — decision 2, the mechanism behind Stage 5a's grain
...
> **Measured elsewhere and inherited here** (2026-08-20, Stage 5a pass 4e, which denied
...
**Not pulled forward — decided 2026-08-19, by the user.** Landing this in Stage 5a's phase-4b sitting beside
...
(Stage 5a pass 4c; the two-sided rule is `docs/GOVERNANCE.md` §Drop-box and INT-10). **And the resource side
...
> matching theirs. `dev/` was empty in every project: the prefix marker and nothing else.
>
> **Finding A — a deleted project keeps its storage.** Five project prefixes stood under the domain id
> against **one** project in `list-projects`; the other four are the deleted attempts of 2026-08-22, and
> one of them (`ddjgwl1ebyhcb4`) carries a complete `.git` tree — 40 objects — plus a notebook. The bucket
> has versioning, a 90-day **noncurrent** expiry and MPU abort from the house module, and **no rule that
> expires a current version**, so nothing reaps them. This is a copy of a data scientist's work outliving
> the container that governed it, in a bucket whose lifecycle nobody wrote — **not** Lesson 1's shape (the
> copy is not less governed; it is under the project CMK), but the *deletion* story the derived zone has
> and this bucket does not. **It is Stage 11's to inherit** and is written into that stage's scope in the
> same sitting, because Stage 11 cannot discover a prefix nothing points at.
>
> **Finding B — the fourth designed destination exists, and decision 6 pre-declared it.** The Tooling
> blueprint provisions the project its own Athena workgroup, `EnforceWorkGroupConfiguration = true`,
> output location `…/<project-id>/dev/sys/athena/`. So **it does not write into the derived zone** — the
> premise decision 6's 2026-08-20 revision installed is now measured, `results/` stays persona-grain, and
> every consumer of the three-family contract stays true. What the branch costs is what decision 6 said it
> would: the **encryption** row already exists (§Encryption names the project CMK), and what is owed is an
> **expiry** and a place in **Stage 11's Macie/data-event scope**. Whether the location is ours to
...
**2.6 — ~~Extend Stage 5a's extension point to the real role names~~ Re-cut 2026-08-26: the derived
...
Stage 5a step 9.3's extension point with them. What replaces it, **written and executed 2026-08-26/27**
...
1. **Empty both derived buckets by hand** (console *Empty bucket* — versioned, so the CLI `rm` leaves
   ...
   — **4 versions, not none; "never written" was this step's own unmeasured guess, corrected by the
   reading that preceded the delete** (the same verification queried from Development too). The house
   module sets `force_destroy = false`, so a non-empty destroy refuses.
2. **`identity/sso/` first** — `awsds-infra-identity`, after `./scripts/gen-tfvars.py identity sso`:
   the six statements, the two locals, the `consumer_data` lookup and `data_consumers` leave. **Order
   is load-bearing**: sso reads the consumer `data/` states' workgroup/bucket outputs, so applying the
   slices first would break sso's plan by name.
3. **`consumer-data-v0.6.0`** (Recipe B, two-commit order) then `sandbox/data/` + `development/data/`
   as `awsds-infra-sandbox-1` / `awsds-infra-dev`: destroys the bucket + workgroup, edits the key
   policy in place (the persona statement leaves; the sandbox lake's statement stays the only
   cryptographic one). Re-plan `No changes`; `./aws/datalake.py` — `DL-8`/`DL-9` flip from their
   deliberate transitional FAIL to pass; `./aws/sandboxlake.py` still 12/12 (the lake's key statement
   untouched).

**What the choreography did not predict, and it cost a fourth act** (2026-08-26/27): step 3's destroy
**refused both workgroups** — `DeleteWorkGroup` counts query *history* as contents (**4** executions in
Sandbox and **2** in Development against **0** named queries and **0** prepared statements, measured rather
...
The LF machinery (`DataLakeSettings`, resource links, re-grants) **stays** — it is what SMUS
subscriptions ride; whether the persona's own `SELECT` re-grants retire now that no persona engine
exists is re-read at Stage 9/14, not here.
...
> an A record, so the delta is narrow — and the two bypasses of any name-based control are untouched and
> **neither is DNS-closable**: a raw address asks no resolver, and a query to a public resolver or over
> DoH is never seen by the VPC resolver at all. The L7 answers are
> [`docs/plan/architecture.md`](../architecture.md) §4.3a, and they are not built.
...
and why Stage 5a pass 4d made it one, is the `s3` bullet of the same section this step already reads (added
...
`data-governance/data/`, and (since 2026-08-26, D19 revised) the **projects bucket's** condition in
`terraform-modules/sagemaker-prereqs/` — the derived buckets this sentence used to name are removed, so
...
**And since 2026-08-30 it owes it against a different ceiling**: `vpc-egress-v0.4.0` is expected to give
pip, cargo, rustup, CRAN, apt and ECR Public a path (4.1's block), so a friction reading taken under the
...
table's 5.0 row and the owed table's 5.0 row carry the record; `default-v0.1.0` in both repositories).
...
- **The working session:** sign in through the VPN, open the portal, work in `experimentation` (compute in
  Sandbox) and `engineering` (compute in Development), install a package, and read the Stage 5a lake table
...
1. **The notebook Spark runtime replacing Athena Spark** (1.4, open question 12) — EMR Serverless or Glue
   ...
   **`project.spark.fineGrained`** permission mode (LF fine-grained, EMR ≥ 7.2.0; Glue's `fineGrained` is
   documented for Visual ETL flows, not the notebook). If Stage 5a's `restricted`-column scoping is to hold
   ...
   > writing off Stage 5a's column scoping on the notebook Spark path *before* measuring whether it works.
   > Starting from EMR-S keeps both outcomes reachable; starting from Glue forecloses one.
2. **`enableTrustedIdentityPropagationPermissions`** (1.5, the grain — Stage 5a decision 6's mechanism) —
   recommended: **follow the grain Stage 5a chose**. If per-user on the SQL path, enable it and accept the
...
3. **The Athena Spark disable set** (1.6) — recommended: the `athena:StartSession`/`UpdateSession` SCP via
   ...
   **Three-quarters decided 2026-08-19 by the user, once the re-read had closed the questions evidence
   ...
   - **Timing — not pulled forward.** The amendment and its probes run at 1.6, when this stage opens, not
     in Stage 5a's phase-4b sitting (1.6 carries the trade).
...
4. **Which Lakehouse blueprint(s) the catalog/SQL surface needs** (1.4) — `LakehouseCatalog`,
   ...
   permissions, an Athena workgroup, the shape that lands on Stage 5a's substrate — is
...
6. **Dissolved 2026-08-26 — the derived zone's per-project prefix shape** (D19 revised the same
   ...
   the number stays retired, the text below stays as the record of a question that dissolved before
   ...
   bucket (Stage 5a step 9.3's extension point — the key policy `Decrypt` widened to a list under
   ...
   - **Family-first** (`results/<project>/`, `derived/<project>/…`, `scratch/<project>/`) — extends
     *inside* each family: every existing statement stays true — the persona keeps reading `derived/*`,
     which is Stage 5a's grain decision applied one level up (the human persona reads across projects;
   ...
   > only job is to separate projects from each other. What decides it is finding B instead: the
   > project's own enforced workgroup writes into the **projects** bucket, so nothing a project brings
   > reaches `results/`, `results/` stays persona-grain, and **family-first leaves every existing
   > statement true** — the persona statements in `identity/sso/`, `consumer-data/buckets.tf`'s contract
   ...
   writing one ARN — the role-and-project grain Stage 5a's decision 6 chose.
   ...
   **fourth** designed destination, owed the three things the derived zone already has: a row under
   ...
   the second, undesigned copy zone Stage 5a step 8's enforced location exists to prevent, arriving by a
...
*Answered here: i, ii, iv, v, x (the 5.0 half), xii-xiii (Stage 5a's pair, read in the same session),
...
| xv | **Does a database or table the blueprint creates in a member account come out with no `IAMAllowedPrincipals` grant** — i.e. did Stage 5a pass 4's default-clearing land *before* this stage created anything? The reading is per catalog object, at creation, and there is no second chance: the defaults act at creation time and clearing them later does not reach what already exists (Lesson 27). **The precondition is measured, not assumed, since 2026-08-19**: `DL-6` reads clear in both member accounts and the check now reports per account, so a regression here is the blueprint's doing rather than an open question about the settings | 1.4, 3 |
| xvi | **When the portal fulfils an approved subscription, what shape is the Lake Formation grant it writes** — named-resource or an LF-Tag expression? If DataZone ever writes expressions, Lesson 29 applies to a grantor this repository does not author: an expression on `classification` alone reaches `layer=dropbox`, and the near-miss Stage 5a caught by reading its own plan would arrive from a service instead | 1, 7.4 (Stage 5a) |
