...
| **5** | 5 | the model chain: retrain → register → approve → serve; **the Staging leg after 6b** | `production/sagemaker/` `[P]` amendment + pipeline | `awsds-infra-prod`; pipeline |
...
- **5.4 — [pipeline] Serve the approved version through the chain (D20), after 6b**: in Staging,
...
- **6.1 — [user + pipeline] The scheduled end-to-end (after 6b)**: tag → Stage 8's chain (Staging leg,
...
- **The model chain (5.x):** a version registered `PendingManualApproval` by the pipeline, approved at
  the manual gate, served in Staging first (after 6b), then Production — and Development still reads
  status only.
...
| x | Does the model chain hold end to end — registration only under `awsds-deploy-prod`, training `OutputDataConfig` carrying the CMK, the approved version served in **Staging before Production** (after 6b)? | 5.1-5.4 |
