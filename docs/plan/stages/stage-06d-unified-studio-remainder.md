  ...
  - **The delivery mechanism, read from the service model 2026-09-09: (b) and (c) are one question, and
    ...
    **Neither can be attached to a space.** `SpaceSettings.CodeEditorAppSettings` is a *different
    ...
    - **(c) is the recommendation.** It is declarative and lands **beside 2.2's JupyterLab config**, so
      one intent stays in one place rather than in two syntaxes (Lesson 33, Lesson 51). It sets the
      **container** environment, which supervisord and the server inherit — and the terminal with it,
      which retires the by-hand `export` from every reading in this stage. **`sudo` still strips them**:
      2.2's two image-side files remain necessary.
