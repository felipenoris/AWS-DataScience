...
- **3.1 closes, 2026-09-11: R was the last ecosystem and it is a refusal.** From a terminal on
  ...
  - **Decision due 6 is taken in full (the user, 2026-09-11): CRAN in, conda out.**
    ...
    `terraform validate` clean, **unapplied**, so `DN-3` is red until the apply by construction. conda's
...
6. **Which compute-plane names step 3 adds** (3.1): every `403` the proxy logs is a name to allow or a loss
   ...
   **Closed 2026-09-11 on its original two (the user): CRAN in, conda out.** `install.packages` was
   ...
   run inside a space would move the SageMaker Distribution's own pins. **Unapplied** as of that date;
   `DN-3` is red until the apply.
