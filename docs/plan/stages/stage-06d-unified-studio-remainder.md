...
- **7.2's static half is done 2026-09-07, and it is decisive without CloudTrail.** Read back from
  ...
  feature); with the project role it succeeds **unscoped**. The objective's scoping was granted by
...
- **10.1 — Done 2026-09-12: where it fails, read on every layer.**
  ...
  | SMUS | *"by default, all users of an Amazon SageMaker unified domain have access to the Free Tier release of Amazon Q"*. The vendor names two SSM parameters under `/amazon/datazone/q/<domain-id>` as the domain's switch; none was found under `/amazon/datazone/q` in `Sandbox` or through `awsds-infra-data`, which is **not** evidence of absence until the account that holds them is known (Lesson 62) |
...
- **10.8 — [Claude] The Free tier's limits, and who shares them.** A search on 2026-09-12 attributed to
  SageMaker Unified Studio's documentation the statement that the Free tier's request limits are
  **shared at the account level** — every scientist in `Sandbox` drawing on one allowance — but the admin
  page read the same day does not carry it, and the user-guide page that may did not render. **It is a
  claim until a page is read verbatim** (Lesson 38). The limits themselves are on the vendor's pricing
  page and not in the price list (`docs/PRICING.md`, *Amazon Q Developer*), so they are read and dated,
  not measured. A refusal for quota reads differently from 10.1's, and the runbook names
