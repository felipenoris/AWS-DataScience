...
Stage 5 step 9.3's extension point with them. What replaces it, **written 2026-08-26, applies owed to
the user, in this order**:

1. **Empty both derived buckets by hand** (console *Empty bucket* — versioned, so the CLI `rm` leaves
   versions behind): `awsds-sandbox-derived` (8 result objects of 2026-08-20) and `awsds-dev-derived`
   (never written). The house module sets `force_destroy = false`, so a non-empty destroy refuses.
