...
that consumes them. **D5 is executed, not decided, in this stage**: both designs get built (`docs/plan/architecture.md` §4.3).
...
- **This is the only place in the plan where an artifact reaching an account is not built by a pipeline**,
  ...
  hand-built image in `docs/log/log-stage-06-unified-studio.md`, so the changeover is visible.
- **The `docs/plan/architecture.md` §4.3 verdict on egress design B is provisional until Stage 8.** Design B's whole claim is that
...
4. **Build egress design A (`docs/plan/architecture.md` §4.3):** NAT route plus Route 53 Resolver DNS Firewall with an explicit
...
5. **Build egress design B (`docs/plan/architecture.md` §4.3):** the same domain with no NAT route at all; packages from CodeArtifact
...
6. **Compare them and write the verdict** (the deliverable in `docs/plan/architecture.md` §4.3): measured hourly cost, what breaks in a
...
7. Attach EFS access points for the shared NFS area (Sandbox only, D24).
   **To verify rather than assume, and it is a `CLAUDE.md` requirement rather than a convenience:** D24 and
   ...
   unavailable to project compute, which is a real reduction in scope and belongs in `docs/plan/institutional-delta.md` next to the
...
8. Lifecycle configuration for idle shutdown — mandatory cost control.
   ...
   *apps* only — through the per-project SageMaker AI domain, not through DataZone, which owns no compute
   (`docs/plan/conventions.md` §6). **Project home directories are scratch** by policy: notebooks live in GitLab, data lives in S3,
...
portal is a storey on top of it, not the foundation. The corresponding `docs/plan/institutional-delta.md` row is closed. One property
worth restating because it did not change: the portal, like the old Development Studio UI, is a **public
endpoint**, intended to be controlled by `aws:SourceIp` against the WireGuard Elastic IP (`docs/plan/architecture.md` §3) — adopting
