...
| `sandbox/data/`, `development/data/` (amended through a `consumer-data` tag) | the derived-zone key policy's second `Decrypt` principal — a widened module input under Recipe B, never a slice edit (2.6) | `[P]` |
...
derived-zone key policy `Decrypt` — a second element in `AllowDataScientistUseViaS3`'s `Principal`, which
...
written into the blueprint-provisioned project role's own policy here (INT-15). Both are what Stage 5
