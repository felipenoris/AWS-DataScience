...
**DELIVERED, AND REVISED ONCE THE SAME WEEK — applied in Sandbox 2026-08-22; `development/egress/` carries
the bump in code and not in the account, its slice being down.** **The list is not written in this step,
deliberately** — it is the `dns_firewall_allow_domains` default in
`terraform-modules/vpc-egress/variables.tf`, one copy, each family annotated with the reason it is on, so
this file and the code cannot drift. **The first version was six registrable domains short, and what found
them was running a session rather than re-reading the list** (`vpc-egress-v0.2.1`): `public.ecr.aws` — on
the `aws` TLD, so *not* under `*.amazonaws.com`, and what the BYOI ancestor and the buildbox both pull —
`astral.sh`, `rustup.rs` and `rust-lang.org`, **installer hosts no package index covers**, `julialang.net`
(the storage server, beside the `.org` package server) and `duckdb.org` (extensions fetched at first
query, from inside the notebook process). The old enumeration here was the package indexes and nothing
else, which is the shape of the miss rather than an accident of it.

**Three readings that outlive the fix, and each changes how a later step is written.** A wildcard entry
reaches **every depth beneath it and never a sibling TLD**, so `*.julialang.org` covering
`us-west.pkg.julialang.org` says nothing about `storage.julialang.net` — an ecosystem is not on the list
because one of its names is, and 4.3's query log is the only instrument that settles it. The rule group
**associates to the VPC id, not to a route table**, so this list also filters `sandbox/buildbox/` in the
isolated tier, whose egress leaves through the WireGuard host and never touches this slice's NAT — the
coupling had never been exercised, no build having run while `egress/` was up. And the slice's plan
**never converges** (`EXC-04`): the API canonicalises every entry with a trailing dot, so a change here is
verified by reading the list back from the API and never by a clean re-plan.
