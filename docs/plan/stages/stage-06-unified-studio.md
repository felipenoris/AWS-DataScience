...
| `production/registry/` at rest (pass 0) | ~USD 1.00/month KMS key + the ECR row above + CodeArtifact ~USD 0.10/month | all three are **existing floor lines** in `docs/plan/cost-model.md` — the slice moves no rate, it only makes the floor start earlier. CodeArtifact has no `us-west-2` figure in the Price List API (`docs/PRICING.md` §9); the storage/request rates are the published ones and stay flagged as such |
