...
| **1** | 5.0 | the hand-built `base`/`dev-env` images into the Production ECR — **needs pass 0's repositories, and carries no CA root** (Stage 7 step 2.6) | the **devbox**, build and push in **one session** (`devbox.md` §P) | **user** (docker + push) — **DONE 2026-08-22**, `default-v0.1.0` in both, digests in the owed table |
...
| **2d** | 1.5, 1.7 | the two project profiles, which need 2c done first; INT-16's portal reading | `data-governance/governance/` `[P]`, second apply + browser | `awsds-infra-data` — **profiles DONE 2026-08-21** (after finding 9's re-cut; decision 2 delivered); 1.7's reading: **user, still owed** |
...
| the portal's off-VPN reach | **NEW, and it is the input the `README.md` decision turns on.** Until the grant above, an ungated portal reached **nothing** — `CreateProject` was denied to every principal — so finding 12's reading was taken against a portal nobody could use. Both personas can now instantiate a profile, and **what a persona actually does in an off-VPN portal session is unmeasured**. The reading: create a throwaway project from the portal with the tunnel **down** and record how far it gets — whether the project provisions, whether a space starts, and whether JupyterLab is reachable or stops at `VpcOnly`. Cheap, and it decides whether INT-16 fallback (i) is worth building. **Amended 2026-08-22: "both personas can now instantiate" died the same day it was written — project creation now fails one authorization layer down (the row below), so this reading waits for that apply** | **user**, one browser sitting |
| the blueprint grants | **NEW 2026-08-22 — the first real project creation (a data scientist, `experimentation`, Sandbox 1) got PAST `CreateProject` and rolled back on `Caller is not authorized to create environment using blueprintId <Tooling's>`; nothing provisioned, nothing billed (list-projects empty, no SageMaker domain, no stack).** `CREATE_ENVIRONMENT_FROM_BLUEPRINT` is a separate authorization on each blueprint CONFIGURATION, and all 22 configurations carried **ZERO grants**: the console's enable flow fills "Authorized domain units" (which emits the grant), `PutEnvironmentBlueprintConfiguration` — all 1.4 ran — does not. Same shape as 2.4's gap, one layer down. **The entity id is the undocumented `<member-account>:<blueprint-id>`** (measured by exhaustion, then confirmed against `aws-samples/sample-automate-sagemaker-unified-studio-using-iac`, which also supplies the principal: every root-unit project, designation `CONTRIBUTOR` — copied, not designed). `sagemaker-prereqs` **`v0.3.0`** adds `grants.tf` (one grant per configuration, `for_each`, so a future category-1 blueprint arrives authorized; the detail is a JSON-string `"{}"` — the CFN Unit type in awscc 1.98.0) and the new `root_domain_unit_id` input; both member slices bump the ref and pass it. **Apply owed in Sandbox 1 AND Development** (Recipe A, `awsds-infra-sandbox-1` / `awsds-infra-dev`), then the project retry is the verification — and it gates the off-VPN row above | Claude wrote; apply **user-authorized** |
| ~~5.0~~ | **DONE 2026-08-22 — build and push in ONE devbox session**, as §P requires. Build `rc=0` in ~15 minutes, driven over `ssm send-command` under `systemd-run`; pushed as **`default-v0.1.0`** into both repositories — `base` `sha256:6c53def4…5b3a` (3.96 GB stored) and `dev-env` `sha256:76d9b5e8…3e56` (5.65 GB). Four in-image readings taken rather than assumed (the activity-monitor extension present by name, the BYOI entrypoint inherited, the CA layer asserted empty, the five runtimes), **the tag convention decided here by the user** (`<flavour>-v<semver>` — `docs/SMUS.md` §Custom images owns it) and the scan measurement are the log's fourteenth and fifteenth entries. The devbox is down | **user** + Claude |
...
default encryption key**, which that list does not carry and which decides whether a service-created
bucket in an Interactive account sits outside `docs/GOVERNANCE.md` §Encryption's per-account data CMK.
Verification (xviii); decision 6 is written against the answer.
...
| The two project CMKs at rest (pass 1) | ~USD 2.00/month — `alias/awsds-sandbox-project` and `alias/awsds-dev-project` | **A new floor line, and one with no consumer today.** `terraform-modules/sagemaker-prereqs/` creates them for the resources a blueprint provisions, but no blueprint regional parameter and no Tooling parameter written so far names a key — **verification (xx)** is what settles whether they have a consumer at all. Two key-months either way, at the KMS row's rate |
...
| xx | **Does any blueprint regional parameter, or any Tooling parameter, accept the project CMK?** `alias/awsds-<env>-project` is created by `terraform-modules/sagemaker-prereqs/` for the resources a blueprint provisions, and as of pass 1 **nothing names it**: `blueprints.tf`'s `regional_parameters` carry `VpcId`/`Subnets`/`AZs` and no key, the D13 boundary's only KMS clause scopes the *lake's* data key, and none of the Tooling settings [`docs/SMUS.md`](../../SMUS.md) lists is a key parameter. If a parameter does take one, name it and record which resources land under it — that overlaps (xviii), which reads the project S3 path's own default encryption key at 2.4. **If none does, the two keys have no consumer and the branch is explicit rather than drifting**: delete them, or keep them with a named future consumer and a date. One key-month per account either way | 1.4, 1.5, 2.4 |
