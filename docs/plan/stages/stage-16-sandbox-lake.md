...
| | |
|---|---|
| **Status** | **PASS 0 DONE 2026-08-26 — 0.1 INCLUDED: the user wrote the requirement into [`objectives.md`](../objectives.md) the same day**, as a bullet in the SageMaker Unified Studio list rather than a dated note, carrying the per-group contract and the SMUS mount (**not** permanence — see the 0.1 row below). The three preflight readings answered, one of them with a consequence: the account data CMK carries **no delegate-to-IAM statement**, so pass 2.2 is not a formality but the only way a new reader is admitted at all. Four detail rows are in §"What ran on 2026-08-26" below, which is the only place the results are written. On disk since the same day and awaiting a `[user]` apply: the new `sandbox/lake/` slice (bucket + access role + location + three grants, `terraform validate` clean, `tflint` clean, `checkov` 25/0 with the module's two standing skips), `consumer-data`'s `additional_data_key_policy_statements` input with `sandbox/data/` passing one statement and `development/data/` passing none, and the `layers.py` row that makes the tree twenty-six slices. **Nothing is applied, so no verification below is answered and every claim about behaviour is still documentation** — created 2026-08-26 at the user's request. The requirement arrived in conversation, not in [`objectives.md`](../objectives.md) — step 0.1 is the user writing it there, in that file's own dated-note convention, because a stage is measured against the spec and this stage's distinctive commitments (permanence, the per-group prefix contract, a deliberately ungoverned store beside the governed lake) appear nowhere in it yet. Pre-instrumented the same day by `./aws/sandboxlake.py` (`SL-1`–`SL-5`), and the recurring procedure is drafted, **unexercised**, in [`runbooks/sandbox-lake.md`](../runbooks/sandbox-lake.md) — every command there is a design until passes 4-6 mark it measured, and the runbook earns its past tense from those readings, never from this file (Lesson 37) |
