...
| | |
|---|---|
| **Status** | **Done 2026-08-26** — passes 0 to 6 applied and measured. Every result is in "What ran on 2026-08-26" below, the only place they are written, and the stage log is [`log-stage-16-sandbox-lake.md`](../../log/log-stage-16-sandbox-lake.md) (6.3). **0.1 is done**: the user wrote the requirement into [`objectives.md`](../objectives.md) as a bullet in the SageMaker Unified Studio list, carrying the per-group contract and the SMUS mount — **not** permanence (the 0.1 row). **Re-measured, not re-designed, by [6c](stage-06c-networking-hub.md) (2026-09-05)**: every vending path here is unchanged in mechanism and changes in *address* — the laptop's `s3control:GetDataAccess` now leaves through the proxy and its S3 through `VPC-Networking`'s gateway endpoint, so §T's laptop half and `s3-read-write` are re-run in 6c's pass 6 as the proof that the re-keying was complete. The bucket, the grants, the access role and the runbook are untouched. **Deferred, each with an owner**: §R's trust half (a real project death), verification (iv)'s object-level half (Stage 11's data events), §T's second-project refusal (when a second project exists). Created at the user's request and pre-instrumented by `./aws/sandboxlake.py` (`SL-1`–`SL-5`); the recurring procedure is [`runbooks/sandbox-lake.md`](../runbooks/sandbox-lake.md) |
| **Prerequisites** | **None that block.** Stage 5a's consumer side built this account's data CMK and its `DataLakeSettings` (`sandbox/data/`); Stage 6's create path is measured end to end, so a real project exists to wire; `s3-read-write` is merged (2026-08-24) and is pass 5's instrument **unchanged**; and the S3 Access Grants instance this stage grants against already exists in Sandbox — **SMUS-born 2026-08-22**, nothing here creates it (Lesson 17: read what the service made before adding to it). **Sandbox only, deliberately**: Development has no Access Grants instance until its first project is born, and nothing in this stage touches that account — extending the contract there is a later act, recorded here so it is not discovered as a gap |
...
different, and each is owned by a step below:
...
*The only place this stage's results are written. A row that says a thing exists says which command
showed it.*
...
- **0.3 — [Claude] Three readings before anything is built** (Lesson 22 — read what cannot be attempted):
  ...
  SMUS-born state).
- **0.4 — [Claude] Name what is still only documentation**: verifications (ii)-(vi) below are the
  documented-mechanism claims pass 4 converts into measurements. Nothing before pass 4 states them in the
  perfect tense, and the runbook earns its past tense from those readings rather than from this file
  (Lesson 37).
...
into `docs/log/log-stage-16-sandbox-lake.md` (Lesson 16), with a recommendation stated.
