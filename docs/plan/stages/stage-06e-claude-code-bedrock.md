...
- **7.2a — [Claude reads, user decides] A negative control for `mode: none`, which nothing else
  ...
  **Overtaken by 7.5 on 2026-09-12, as written.** `DenyInvokingModelsThatRetain` now refuses this exact call
  before the retention mode is consulted — measured in `Policy Canary`, refused naming `p-1fp032g8` — so
  the probe would read the SCP, which is already proven, and never reach the mode. The one principal no
  SCP restricts is in `Management`, which holds no model access by decision 2. **The probe needs a
  carve-out to mean anything**: a condition on `DenyInvokingModelsThatRetain` exempting one named probe
  principal, applied for the sitting and removed after, so that the refusal — or the success — belongs to
  the mode alone.
...
- **8.4 — [Claude] Attribute the spend.** Invocation logging is off, so `identity.arn` grouping is not
  ...
  which this client does not. **Read 2026-09-12**: the evening's 45 invocations were **all the one
  project role**, all through the runtime endpoint, from three space addresses (`10.20.43.114` 20,
  `10.20.61.134` 14, `10.20.64.199` 11). **The source address is the only field that separates one
  space from another**, and it changes at every app restart, so a window attributes spend to a space
  only while its app runs; nothing attributes it to a user.
...
| (ii) | Can the principal of step 1 invoke **each of the three scoped models**, measured by a call rather than by a policy read? | **Answered 2026-09-12, in two channels.** Haiku 4.5 in step 6; Sonnet 4.5 (20 invocations) and Opus 4.5 (22) in the evening's sessions, all as the project role through the runtime endpoint, CloudTrail and CloudWatch agreeing call for call (`./aws/bedrock-usage.py` `BU-4`) |
...
| (v) | Does a session reach **no** Anthropic host? | 6.2, and **held a second time, on the wrong image**: a space created by mistake on AWS's SageMaker Distribution — no proxy variables, no managed settings — had something in it — by every sign the Claude Code client — ask for Anthropic's API, and **the DNS Firewall blocked `api.anthropic.com` 46 times** from `10.20.10.101` between 20:58 and 21:32Z on 2026-09-12, that space's lifetime. The attribution is by time, since its interface no longer exists. So the requirement survived the loss of both the image's settings and the proxy: the name never resolved |
...
| (xiii) | Is `mode: none` **enforced**, or only declared? | **No instrument is left in this estate as built.** No control-plane call shows a retention mode's effect, and the two models the vendor names as retaining read exactly like the scoped three (7.2). 7.2a's invocation was the only instrument, and **since 7.5 the SCP refuses that call before the mode is consulted**, so it measures the SCP. Answerable only with a temporary carve-out in `DenyInvokingModelsThatRetain` for one probe principal (decision 11) |
| (viii) | Does the CloudTrail record carry the prompt? | 4.6 — it must not |
| (ix) | What does one session cost? | 6.4, 8.1 |
| (x) | Does the configuration survive a new space? | **Answered 2026-09-12, by the user.** A new Code Editor space on `default-v0.4.0` (`remote-editor-claude-v4-3rd`, `ml.m7i.xlarge`, `RemoteAccess: ENABLED`) reaches the internet through the proxy and runs a session, and the stranded `remote-editor-claude`, repaired in the portal to version 4, did the same: a Sonnet 4.5 prompt created a Python project, and the picker offered **Default (Opus 4.5), Sonnet 4.5 and Opus 4.5** — exactly `availableModels` plus the `ANTHROPIC_MODEL` default, Haiku being pinned for background work and not selectable by design. Both read the pins from the image, since `/etc/claude-code/` is rebuilt from it at every app start and the earlier hand edit did not survive. **One false negative on the way**: a space created on AWS's `sagemaker-distribution-cpu` by mistake had no proxy variables and no internet and read like a defect in this image, until `describe-app` on the deleted app named the image and `./aws/devenv.py`'s new `DE-5` read all six variables out of `v0.4.0`'s config blob (`dev-env.md` §C7) |
| (xi) | Does `/model` offer exactly the picker set, and `/status` name Bedrock and `us-west-2`? | **`/model` half answered 2026-09-12, by the user**: *Default (Opus 4.5), Sonnet 4.5, Opus 4.5*, exactly `availableModels` plus `ANTHROPIC_MODEL`'s default, with Haiku pinned for background work and not selectable. **`/status` not read** |
| (xii) | Does a background task bill Haiku rather than the primary model? | **Answered 2026-09-12 by timing, across three sessions and two primary models.** Each of the evening's three Haiku invocations is **the first call of a conversation, 1-2 s before the primary's burst**: 21:35:31 before 19 Sonnet calls, 21:57:03 before 13 Opus calls, 22:13:55 before 9 Opus calls — the shape of the session-title call, billed on Haiku. CloudTrail carries no content, so the title itself is not read; the pattern is |
...
| **14** | Which models the scoped set names | **Opus 4.5, Sonnet 4.5, Haiku 4.5** (2026-09-12), the three proven by invocation. `claude-opus-5` and `claude-sonnet-5` are refused by AWS for this account and no reading exposes that gate (M4, `EXC-08`); the sales contact the refusal names is worth opening in parallel and the two are not exclusive. **The switch costs one model's rate**: Opus 4.5 prices identically to Opus 5, Haiku did not move, Sonnet goes 2.20/11.00 → 3.30/16.50. It lands in four places, which is the same list with the same four consumers — `sandbox/bedrock/`'s `models` map, the managed-settings pins, the endpoint policy (action-scoped only, so unchanged) and 7.5's retention deny; the SCP of decision 15 is a fifth, and `./aws/bedrock-scope.py` `BS-7` is what keeps them in step. **The grant half applied 2026-09-12** (`0 to add, 1 to change`, a new policy version, the attachment untouched, re-plan `No changes`). The pins live in the image, so the rest was a `default-v0.4.0` release and the whole `dev-env.md` §B chain — **done 2026-09-12**: built and pushed by the user (`dev-env` `sha256:ba1b395e…`, 5,453 bytes smaller than `v0.3.0`, which is the shape a four-string edit has), no app to delete, detached, applied for `1 to add, 0 to change, 1 to destroy`, the new version's `ContainerImage` read against the pushed digest, re-attached on both app types from the slice's own output, re-plan `No changes`. **What `v0.3.0` shipped is worth keeping in view**: `ANTHROPIC_MODEL` and both alias pins on `claude-opus-5`/`claude-sonnet-5` with `availableModels` locked to `["opus","sonnet"]` — a picker offering exactly the two models that refuse, which is the symptom the user hit on 2026-09-12 and which two independent things now deny (the commercial gate and decision 15's own `NotResource`) |
...
| **11** | Whether to run 7.2a's Fable probe, the only negative control `mode: none` can have | **Changed by 7.5 on 2026-09-12.** The probe as written now reads `DenyInvokingModelsThatRetain`, not the mode, so running it proves nothing new. The decision is now **whether to carve one probe principal out of that statement for one sitting** and invoke `us.anthropic.claude-fable-5` as it — a refusal naming the retention mode proves the setting enforced, a success is a finding of the first order — **or accept the vendor's sentence** and record the acceptance with its date. Recommended: the carve-out, since it is one condition added and removed in the same sitting and the requirement rests on this answer |
| **13** | Whether the `bedrock` control-plane endpoint carries an action scope too | **taken by Claude on 2026-09-12 under a stated assumption, and not reviewed by the user.** Only `bedrock-runtime` is scoped. The argument is that the control plane's API surface is long, growing, and called by the SMUS blueprints for things an assistant never does, so a list written for one consumer silently refuses the other. The cost of that choice is named in 4.4: `PutModelInvocationLoggingConfiguration` still traverses its door, and nothing else covers it until Stage 11 **Evidence for the review, read 2026-09-12** over three days of `bedrock.amazonaws.com` in Sandbox: **the only action that traversed the control-plane endpoint was `ListInferenceProfiles`, by the project role** — 16 calls across two `make up`s, the client at startup. Nothing else used it: every other control-plane call came without an endpoint, from an operator's laptop or from AWS Config. **So the assumption behind this decision — that SMUS blueprints call the control plane through this endpoint — is not observed; but none of the six `AmazonBedrock*` blueprints has been used, so it is unexercised rather than refuted.** A scope of `bedrock:ListInferenceProfiles` would have broken nothing measured so far |
