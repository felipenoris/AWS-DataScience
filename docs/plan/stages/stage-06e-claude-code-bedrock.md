...
| (x) | Does the configuration survive a new space? | a second space from `default-v0.4.0`, which is image version 4 since 2026-09-12 — the first space that reads the pins from the image rather than from a file edited by hand inside the container. **It has to be a new space**: an existing one carries its own copy of the version number and overrides the domain default (`EXC-09`), and `remote-editor-claude` still names the destroyed version 3 by the user's decision, to see how that behaves |
