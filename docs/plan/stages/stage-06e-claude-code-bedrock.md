...
| **14** | Which models the scoped set names | **Opus 4.5, Sonnet 4.5, Haiku 4.5** (2026-09-12), the three proven by invocation. `claude-opus-5` and `claude-sonnet-5` are refused by AWS for this account and no reading exposes that gate (M4, `EXC-08`); the sales contact the refusal names is worth opening in parallel and the two are not exclusive. **The switch costs one model's rate**: Opus 4.5 prices identically to Opus 5, Haiku did not move, Sonnet goes 2.20/11.00 → 3.30/16.50. It lands in four places, which is the same list with the same four consumers — `sandbox/bedrock/`'s `models` map, the managed-settings pins, the endpoint policy (action-scoped only, so unchanged) and 7.5's retention deny. The pins live in the image, so delivering it is a `default-v0.4.0` release and the whole `dev-env.md` §B chain |
...
| **15** | What compensates M5's Region exemption | **Written 2026-09-12, apply owed.** Two statements in `awsds-org-scp-ou-interactive.json` — the OU where the hole was opened, not the root: `DenyBedrockInvocationOutsideTheScopedModels` (a `NotResource` ceiling naming the six ARNs, so the blueprint cannot widen what a project may invoke) and `DenyBedrockReadsOutsideTheRoutedRegions` (the region axis the Control Tower lever has no way to express, bounded by M0's list). `POLICIES.md` carries both rows. The two retention actions are deliberately outside the Region condition — decision 16 declares the mode in every Region, so a Region condition here would forbid the calls that keep it declared; the mode axis guards them instead, in 7.5. Attaching re-runs the battery (`scp-battery.md`) |
| **16** | Declaring `mode: none` where a prompt is processed | **Every enabled Region, not the routed three** (2026-09-12, the user). The routed list *is* readable programmatically — M0's `get-inference-profile` — but AWS owns that routing and can add a Region under a pinned model id, so a mode set only where it routes today is a race whose losing side is silent. Measured the same day: `get-account-data-retention` answers in **all 17 enabled Regions**, `inherit` everywhere but `us-west-2`. Declaring `none` in all of them removes the race, makes the invariant checkable without knowing the routing (`INV-18` becomes *every enabled Region reads `none`*), and costs 17 one-time calls. **Order:** 7.5's mode deny first — it permits `none` and forbids everything else, so it makes the next step harmless — then `bedrock:PutAccountDataRetention` in M5's exemption, then the writes, then the reading in `./aws/bedrock.py`. A Region opted into later starts at `inherit`, so the check enumerates Regions rather than carrying a list. **Owed** |
