...
| **Explicit HTTP `CONNECT` proxy** (Squid) — **CHOSEN, and built at [Stage 6c](stages/stage-06c-networking-hub.md) under [D38](decisions/D38-single-egress-hub.md)** | the `CONNECT` line, which is plaintext to the proxy — so it survives **ECH**, which will eventually blind SNI matching | **0.0104/h — `t3.micro`, and it is the SHAPE THAT WAS BUILT, not a range** (settled 6c step 7.4, 2026-09-06). `PRICING.md` §8. It began as a `t3.nano` at 0.0052 and was sized up because **`dnf` was OOM-killed on 415 MiB** — a measurement, not a preference. *This cell read "~0.008–0.017/h" until 2026-09-05, roughly twice the measured rate, from a figure nobody had taken from the price list; it then read as a two-value range until the host existed.* **And it charges nothing per GB**, which is the half the comparison above kept omitting: a NAT gateway adds 0.045/GB of processing and an EC2 proxy adds none | an EC2 to run and patch; every tool needs `http_proxy`/`https_proxy` (pip, cargo, `Pkg`, R, apt, the container runtime); and the proxy must resolve **outside** the DNS Firewall, so it needs its own resolver or a VPC without the rule-group association |
