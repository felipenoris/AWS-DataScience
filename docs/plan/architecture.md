...
| **Explicit HTTP `CONNECT` proxy** (Squid) — **CHOSEN, and built at [Stage 6c](stages/stage-06c-networking-hub.md) under [D38](decisions/D38-single-egress-hub.md)** | the `CONNECT` line, which is plaintext to the proxy — so it survives **ECH**, which will eventually blind SNI matching | **0.0052/h (`t3.nano`) or 0.0104/h (`t3.micro`)**, `PRICING.md` §8 — *this cell read "~0.008–0.017/h" until 2026-09-05, roughly twice the measured rate, from a figure nobody had taken from the price list* | an EC2 to run and patch; every tool needs `http_proxy`/`https_proxy` (pip, cargo, `Pkg`, R, apt, the container runtime); and the proxy must resolve **outside** the DNS Firewall, so it needs its own resolver or a VPC without the rule-group association |
