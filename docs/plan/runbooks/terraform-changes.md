...
5. **Plan to a file outside the repository, and read it** — the add/change/destroy counts, and that
   ...
   **`-input=false` is not optional and it is not about tidiness** (added 2026-09-06, Stage 6c pass 3).
   A plan with a missing variable **prompts**, and a prompt with no terminal to answer it **waits
   forever while holding the S3 state lock** — after which every other command, in other slices, fails
   with `Error acquiring the state lock` naming a timestamp that explains nothing. The flag turns that
   into an immediate, self-describing error. Put it on every `plan` and `apply`, backgrounded or not:
   the generated tfvars are regenerated by hand, so "a variable is missing" is a routine state, not an
   exotic one.
...
3. **Tag the new version** — one tag per command, semantic-version bump, **never reuse or move a tag**
   ...
   **CONFIRM THE COMMIT LANDED FIRST, AND NEVER CHAIN THE TAG ONTO IT.** On 2026-09-06 a
