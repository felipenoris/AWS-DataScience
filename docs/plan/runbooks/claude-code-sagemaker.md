...
| Did the invocation take the private door? | CloudTrail in Sandbox: `InvokeModelWithResponseStream` is a **management event**, so the organization trail carries it with no data-event charge. The reading is `vpcEndpointId` on the event. **A streaming invocation writes two events** — one carrying `requestParameters.modelId`, its sibling carrying `requestParameters: {}` — while a plain `InvokeModel` writes one. Counting events that name a `modelId` and carry no `errorCode` counts each invocation once (measured 2026-09-12). A **denied** call carries no `modelId` at all |
...
| How many tokens did a window consume, per model, and what does that cost? | [`./aws/bedrock-usage.py`](../../../aws/bedrock-usage.py) — **with no argument, this month to now**; `YYYY-MM` for a month; two instants **with an offset** for a window, since a naive time is refused. CloudWatch `AWS/Bedrock` in `us-west-2` carries a cross-region profile's tokens with invocation logging off, and the script reconciles it three ways before reporting: the dimensionless series against the per-model sum (`BU-2`), invocations against per-model plus client errors (`BU-3`), and CloudTrail's successful invocations per model (`BU-4`). The dollars are measured tokens times `docs/PRICING.md`'s rates — **computed, not billed** — and cache writes are priced at the 5-minute rate, which a 1-hour write exceeds |
| Which session spent it? | **No channel answers this.** Every space in a project invokes as its project role, and neither CloudWatch nor CloudTrail names the space. A window measures one session only if nothing else ran in it — which is how stage step 6.4 is taken |
| What did it bill? | Cost Explorer by usage type, a day late. It is the only reading that separates a 5-minute cache write from a 1-hour one |
