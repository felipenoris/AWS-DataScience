...
```bash
cd /opt/awsds/images && sudo docker build -t awsds/base:local base && sudo docker build --build-arg NO_PROXY_LIST="$NO_PROXY_LIST" -t awsds/dev-env:local dev-env
```

**`dev-env` refuses to build with an empty `NO_PROXY_LIST`** (6d decision 8): since 2026-09-10 the image
carries the six proxy variables as `ENV`, and one without its bypass list would send every AWS call in a
space out through the hub as a public call. The value is generated by the account the image will run in
and is read **on the laptop**, before the session, then exported in the host shell — one paste, and the
`export` keeps it out of the command's own history line:
