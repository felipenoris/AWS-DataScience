...
| **Deployment Manager** | — | The SSO persona that approves promotion of an artifact along Development → Staging → Production, and the time-boxed elevated role used to debug a failed production job. Acts in GitLab, in a group named `deployment-managers` **there**; the directory group is `sso-group-deployment-managers`; read-only in the four lifecycle accounts and **absent from Data Governance**. Cannot grant access to data. |
