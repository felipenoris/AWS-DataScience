...
| **Sandbox / Staging / Production** | The three environment *roles* (four until 2026-09-06, when [Stage 6b](plan/stages/stage-06b-development-becomes-staging.md) converted `Development` into `Staging`), each one AWS account — **except `Sandbox`, which is one account per business unit (D35)**, so the chain reads N Sandboxes → one Staging → one Production. N is 1 today. Sandbox is experimentation **and** where pipelines are engineered — the unit of work is a notebook, then a repository, and since 6b all authoring happens there; Staging and Production are *deployment targets* that only a pipeline writes to. Promotion runs Sandbox → Staging → Production, through git: a pipeline definition leaves Sandbox as a repository, never as an account. See `README.md`, "Three distinctions the layout is built on". |
...
| `INT-nn` | Numbered cross-account integrations that must be proven, each with a fallback: `docs/plan/integrations.md`. Replaces the old "§4.4 row *n*" references, which renumbered whenever a row was inserted. |
...
| **Execution role** | — | The role an **AWS service** assumes to run code on a person's behalf: the SageMaker training or processing role, the Glue job role, a Lambda's role. **It has no permission set and cannot have one** — a permission set materialises only through an Identity Center *assignment*, whose principal must be a user or a group, and the role it generates trusts a SAML provider that no service principal can use. So an execution role is a plain `aws_iam_role` written in its own account's slice (`sandbox/sagemaker/`, `production/sagemaker/`), never in `identity/sso/`. **This is the seam most of the DLP argument rests on:** notebook code runs as the execution role, *not* as the human's `AWSReservedSSO_*` role, so D13's constraint — the roles running notebooks hold no S3 on Lake Formation-registered prefixes — is a claim about this family, and the same sentence about a permission set would be notation. `iam:PassRole` is the bridge between the two families and the reason it is never granted unqualified. **`INT-15` is this entry's live risk:** D26 handed authorship of the project execution roles to a blueprint (Lesson 11). |
...
| **AFT** | Account Factory for Terraform | AWS's tooling for provisioning Control Tower accounts from code. Deliberately not used: this handful of accounts, created once, by hand, does not repay the setup (`docs/plan/institutional-delta.md`). |
...
| `IAMAllowedPrincipals` | A Lake Formation backwards-compatibility setting that makes the catalog fall back to plain IAM. If it is left in place, Lake Formation grants restrict nothing — which is why every stage that creates a catalog verifies the grant rather than trusting it. **Two properties measured 2026-08-18 and worth carrying**: it is a `Create*DefaultPermissions` **default**, so it acts when an object is *created* and clearing it later never reaches what already exists; and it **cannot be written empty in Terraform** — the blocks are Computed and `= []` is refused — so the clearing is applied, then read back (Lesson 27, Recipe D). |
| **Data lake administrator** | A Lake Formation principal named in an account's `DataLakeSettings`. Two things depend on it that are easy to attribute elsewhere: **a resource shared into an account is invisible there until the account has one** (its RAM holds the share, its catalog is empty — the two look identical to "the share failed"), and **only an administrator can pass a cross-account grant on** to a local principal, which is why every cross-account grant here carries the grantable option. In this project the administrator is deliberately `InfrastructureAccess` alone — the [Governance Manager] holds specific grants instead (Stage 5 decision 5). |
| **Resource link** | A Lake Formation object that makes a database shared *from another account* addressable locally: a **local Glue database** whose target is the owner's. Two things follow and both were measured at Stage 5 pass 4. **It needs its own permission**: `DESCRIBE` on the link is what makes it visible, and a principal holding every permission on the *target* while holding nothing on the link sees **no database at all** — so a re-grant is always a pair. **And it is a catalog object created locally**, which makes it the first thing a consumer account creates: if that account's `Create*DefaultPermissions` still name `IAMAllowedPrincipals` when the link is created, the link is born deferring to plain IAM and clearing them later does not reach it ([Lesson 27](plan/lessons.md)). |
