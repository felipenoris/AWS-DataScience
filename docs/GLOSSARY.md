...
| **Sandbox / Staging / Production** | The three environment *roles* (four until 2026-09-06, when [Stage 6b](plan/stages/stage-06b-development-becomes-staging.md) converted `Development` into `Staging`), each one AWS account — **except `Sandbox`, which is one account per business unit (D35)**, so the chain reads N Sandboxes → one Development → one Staging → one Production. N is 1 today. Sandbox is experimentation (the unit of work is a notebook); Development is where pipelines are engineered (the unit of work is a repository); Staging and Production are *deployment targets* that only a pipeline writes to. Promotion runs Development → Staging → Production; Sandbox feeds Development through git. See `README.md`, "Three distinctions the layout is built on". |
