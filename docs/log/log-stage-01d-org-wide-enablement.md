...
Stage: [`docs/plan/stages/stage-01d-org-wide-enablement.md`](../plan/stages/stage-01d-org-wide-enablement.md).*
...
*Two mechanical substitutions were made on **2026-08-17**, by Claude at the user's request, and are named
here rather than at each line: **account ids → the account's name**, and **the e-mail inside an ARN → that
user's role name** — this repository keeps neither in a tracked file
([`aws/INDEX.md`](../../aws/INDEX.md) rule 1), and the `UnauthorizedOperation` outputs below carry both.
The earlier elision that read `<MANAGEMENT ACCOUNT>` now reads like the rest. Nothing else was edited: the
`AWSReservedSSO_*` suffixes, the policy ARNs, the error wording and the encoded authorization failure
messages arrived as they read.*
...
- **Filtering the account by name failed, and the fix is general.** No account is named `Log Archive`; it
  is **`Log Archive Account`**, which `docs/ORGANIZATION.md` had wrong. Rather than correct the string, the id
