...
| Stage 6e — Claude Code on Bedrock | [log-stage-06e-claude-code-bedrock.md](log-stage-06e-claude-code-bedrock.md) | 2026-09-11, the read-only steps: 2.4 (`InfrastructureAccess` is `AdministratorAccess` alone and carries no off-VPN deny); 3.1 (the project role's every `InvokeModel*` allow lands on `foundation-model/*` and none on the system profile, so a grant is needed) with the validated grant draft; 7.2 (`mode: inherit`, and `allowed_modes` is in no Bedrock API); 7.5's condition key confirmed against a bogus-key control; the retention mode has no Terraform resource and no CloudFormation type while the use-case form has both; both IAM simulators shown unusable here; the rates cross-checked against the agreement rate card. **M1 done 22:31 UTC** — the account's retention mode set to `none`, the three scoped models still available under it, and the setting not yet frozen; **M2 done** — the form submitted by console, verification (i) closed, the `formData` blob decoded (double base64 over flat JSON) and its `useCases` kept as submitted by the user's choice, and `get-foundation-model-availability` identical across all three readings. **Step 3's decision**: the blueprint has no granting lever (fourteen attributes, one policy-shaped, and it is the boundary), so decision 8 went to option (b) — the policy on the project role, `sandbox/bedrock/` written and unapplied, with the runbook's §P as the per-project procedure in both forms. **the slice applied** (2 added, re-plan `No changes`, the project role now carrying four policies). **step 4's code** — `vpc-egress-v0.12.0` (the `bedrock-llm` group, `endpoint_action_scopes`) and `v0.13.0` (the two groups refused together), 4.4's premise corrected, the caller bumped, the apply owed. Still to carry: the endpoints' apply, the first invocation's CloudTrail and proxy readings, and one session's token cost. |
