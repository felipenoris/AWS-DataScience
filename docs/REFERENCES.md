...
- **Route 53 Resolver DNS Firewall canonicalises every domain-list entry to the trailing-dot FQDN form, and the Terraform provider will not paper over it** (read 2026-08-22, `terraform-modules/vpc-egress/dns-firewall.tf`). `list-firewall-domains` answers `pypi.org.`, `*.amazonaws.com.` and `*.` whatever form was written; the provider stores that verbatim and compares it to the config as plain strings, so an undotted config never matches its own refreshed state and every plan re-issues `UpdateFirewallDomains` — `Plan: 0 to add, 2 to change` immediately after a clean apply, permanently. **The provider issue is closed as not planned**, so the workaround is the caller's forever, and its one substantive comment is why this repository canonicalises *toward* the dot rather than stripping it on read: stripping would flip the same perpetual diff onto anyone writing fully-qualified names: <https://github.com/hashicorp/terraform-provider-aws/issues/23543>. **The API model does not vouch for the catch-all**, which is worth knowing before trusting it: `FirewallDomainName`'s regex has a wildcard branch of `\*(\.label)+(\.)?`, requiring at least one label after the asterisk — so it admits `*.pypi.org.` and admits **neither `*` nor `*.`**, while the bare `*` has been deployed and read back since Stage 3. The server is more permissive than its published model, which therefore settles nothing here in either direction: <https://github.com/boto/botocore/blob/develop/botocore/data/route53resolver/2018-04-01/service-2.json>.
