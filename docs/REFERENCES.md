...
- **Account Management: trusted access and `PutAccountName` — Stage 6b step 3's two prerequisites.**
  Read 2026-09-05. Enabling trusted access *"allows the administrator of the management account to modify
  the information and metadata … specific to each member account"*, is performable **only from the
  management account**, and requires the organization to have **all features enabled**; after it,
  *"you can use the `accountID` parameter in those Account Management API operations that support it"*,
  from the management account or a delegated administrator. `put-account-name` itself needs the
  `account:PutAccountName` permission, takes a name of **1-50 characters** matching `[ -;=?-~]+`, and the
  management account **cannot pass its own** `--account-id`. **No propagation delay is documented** — the
  name is read back rather than the call re-issued:
...
- **Terraform `removed` block — how Stage 6c drops the transferred Elastic IP from Sandbox's state
  without releasing it.** Available since **v1.7**; the resource block is replaced by
  `removed { from = <address>  lifecycle { destroy = false } }`, which removes it from state and leaves
  the real resource alone. Read 2026-09-05:
...
- **VPC peering limitations — the page Lesson 44 and D38 quote.** Read 2026-09-05 for Stage 6c. Its
  *"Edge to edge routing through a gateway or private connection"* section states four times that a peer
  cannot borrow the other VPC's gateway — internet gateway, *"a NAT device"*, a VPN or Direct Connect
  connection, and *"a gateway endpoint that provides connectivity to Amazon S3"* — and *"VPC peering does
  not support transitive peering relationships"*. One more limitation from the same list decides INT-22:
  **"You cannot connect to or query the Amazon DNS server in a peer VPC"**, which is why a private zone
  reaches a spoke only by being associated with that spoke's own VPC and why a missing association fails
  as NXDOMAIN rather than as a referral. Also useful and easy to miss: the same-Region peering MTU is
  9001, and *"unicast reverse path forwarding … is not supported"*:
  <https://docs.aws.amazon.com/vpc/latest/peering/vpc-peering-basics.html>.

- **Elastic IP address transfer between accounts — Stage 6c step 4.5's only irreversible-looking act.**
  Read 2026-09-05. *"There is no charge for transferring Elastic IP addresses"*; same Region only; a
  **two-step handshake** with **seven days** to accept, after which ownership returns to the source; an
  accepted transfer stays visible to the source for **14 days**; **"AWS does not notify transfer accounts
  about pending … requests"**, so the two calls are one sitting; and **"any tags that are associated with
  an Elastic IP address being transferred are reset when the transfer is complete"**. The three refusals
  the preflight (`./aws/eip-transfer.py`) checks are named on the page and all three are raised **at accept
  time, in the recipient account**: `InvalidTransfer.AddressAssociated` (the source must disassociate
  first), `InvalidTransfer.AddressCustomPtrSet` (a reverse-DNS record must be removed) and
  `AddressLimitExceeded` (five Elastic IPs per Region by default). BYOIP, IPAM-pool and CoIP addresses
...
- **Amazon ECR interface VPC endpoints** — the minimum S3 gateway-endpoint permission for image pulls: `s3:GetObject` on `arn:aws:s3:::prod-<region>-starport-layer-bucket/*` (Stage 3 step 9.3's fourth family, confirmed). **Re-read 2026-08-22 for the other direction** (Stage 6 step 5.0, `buildbox.md` §P): the same page states that `ecr.dkr` is the Docker Registry API and that *"Docker client commands such as `push` and `pull` use this endpoint"*, while S3 is what a container reaches to **download** layers — so a **push** never touches the S3 gateway endpoint and the allow-list's missing `PutObject` on the starport bucket is the documented shape rather than a gap: <https://docs.aws.amazon.com/AmazonECR/latest/userguide/vpc-endpoints.html>.
...
- Enable Tooling blueprint (SMUS admin guide) — the console flow whose **"Authorized domain units"** field is the half `PutEnvironmentBlueprintConfiguration` does not perform: the API creates the configuration, the console also authorizes it, which is why step 1.4's API-enabled blueprints authorized nobody: <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/enable-tooling-blueprint.html>. Read 2026-08-22, attributing the `Caller is not authorized to create environment using blueprintId` rollback.
...
- `AddPolicyGrant` (DataZone API reference) — `CREATE_ENVIRONMENT_FROM_BLUEPRINT` on entity `ENVIRONMENT_BLUEPRINT_CONFIGURATION`, whose `entityIdentifier` format the reference does not give (every derivable spelling is rejected as *format invalid*): <https://docs.aws.amazon.com/datazone/latest/APIReference/API_AddPolicyGrant.html>; the open re:Post asking exactly that question: <https://repost.aws/questions/QUjdtJm8DRTFG_qx0vbVNh1g/amazon-data-zone-add-policy-grant-list-policy-grants-rejects-environment-blueprint-configuration-identifier-what-is-the-correct-format>. Read 2026-08-22.
...
- Connect to Amazon S3 from SageMaker Unified Studio (the S3 connection Stage 16 wires — the documented console path is **Connections → Create connection → Amazon S3**; the user observed the same act offered as *Add S3 location*, and whether they are one form is the stage's 4.2 reading): the fields — Name (required), S3 URI (optional; with none, every bucket the credential can see is listed), Region, an **optional access role picked from a dropdown of existing roles** — the cross-account case *requiring* the role, and the post-create verification being an **S3 Access Grants location** registered for the URI against that role (a failed registration means delete the connection and recreate; there is no repair path): <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/connecting-amazon-s3.html>. Read 2026-08-26.

- Adding existing Amazon S3 data to a SageMaker Unified Studio project — the two sanctioned prerequisites Stage 16 chooses between: **option 1 (recommended by AWS), a dedicated access role** with the documented trust (`access-grants.s3.amazonaws.com` with `aws:SourceAccount` + `sts:SetSourceIdentity`, and the project role with `sts:ExternalId` = the project id and tag-session conditions) and inline S3 + Access-Grants-management + `iam:PassRole` statements; **option 2, the S3 and Access-Grants-management statements inline on the project role** (no `iam:PassRole` — there is no access role to pass); a bucket policy prescribed **only cross-account**; and the note that SMUS **uses an existing S3 Access Grants instance or creates one itself** — matching the measured SMUS-born Sandbox instance: <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/adding-existing-s3-data.html>. Read 2026-08-26.
...
- S3 Access Grants locations — registering a location (the location role's trust to `access-grants.s3.amazonaws.com`, the `s3:AccessGrantsInstanceArn` self-restriction pattern, and the **KMS `Decrypt`/`GenerateDataKey` statement the location role needs for SSE-KMS data** — Stage 16 step 2.1's shape), and the fact that Access Grants **assumes the location's role** to vend a grantee's credentials (why the vended session's ARN names the access role): <https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-grants-location-register.html>, <https://docs.aws.amazon.com/AmazonS3/latest/userguide/access-grants-location.html>. Read 2026-08-26.
...
- **JupyterLab lifecycle configurations, and the one sentence that decides how the proxy variables reach
  a stock image.** Read 2026-09-05 for Stage 6c step 5.6: a lifecycle configuration is created with the
  CLI and attached by updating the domain's or user profile's `UserSettings` — but **for SageMaker Unified
  Studio, attaching it to the SageMaker AI domain and user profile through the CLI is documented as not
  supported, and the console is the path**. A custom image avoids the question entirely, carrying the
  variables as `ContainerEnvironmentVariables` on its app image configuration:
...
- **Amazon Inspector — supported operating systems and programming languages** — read 2026-08-22, to settle whether the paid upgrade Stage 7 decision 2 defers could ever cover this estate's image. For **ECR container images** the supported languages are **C#, Go (and the Go toolchain), Java (and the JDK), JavaScript, PHP, Python, Ruby and Rust** — *"Amazon Inspector doesn't scan for toolchain vulnerabilities in Rust"* is the one carve-out named. **Julia and R appear nowhere on that list**, which is what turns the Julia/R half of the `dev-env` image from a deferred cost into an accepted residual (`institutional-delta.md`, the image-scanning row): <https://docs.aws.amazon.com/inspector/latest/user/supported.html>. The companion page confirms the division of labour — *"With basic scanning, you can configure your repositories to scan on push… With enhanced scanning, you scan for operating system **and programming language** package vulnerabilities"* — and that basic is billed through ECR while enhanced is billed through Inspector: <https://docs.aws.amazon.com/inspector/latest/user/scanning-ecr.html>.
...
- **MWAA Serverless networking — the page that decided D38's contingency had no candidate.** Read
  2026-09-05. It describes **two** VPC shapes and the requirements lists read as contradictory until the
  section headings are read with them (Lesson 41, second instance). *Public routing over the internet*:
  the subnets *"must have a route table to a NAT device (gateway or instance)"* and two NAT gateways are
  listed as required infrastructure. *Private routing without internet access*: the subnets *"must not
  have a route table to a NAT device (gateway or instance), nor an internet gateway"*, and are served by
  **VPC endpoints with private DNS enabled**, associated to both private subnets and to the workflow's
  security group. Common to both: **minimum 2, maximum 16 subnets**, *"these subnets must be private"*, at
  least two in **different Availability Zones**, up to five security groups, and a security group with a
  **self-referencing inbound rule** and an outbound rule for all traffic. The companion page ships a
  CloudFormation template for the private shape whose only endpoints are `logs`, `monitoring` and `kms`.
  One line is recorded as a **claim to verify rather than to copy**: the private shape's NACL is specified
  as inbound allow-all with an **outbound deny-all**, which a stateless ACL cannot mean literally without
  breaking the endpoints it fronts:
...
- **The 2026-08-16 documentation pass behind the Stage 6 revision** — the pages that replaced beliefs with doc facts in [`stage-06a-unified-studio.md`](plan/stages/stage-06a-unified-studio.md):
  - *Network isolation in SMUS* — the required VPC-endpoint table (`datazone` included), the three-control Athena Spark disable (the SCP on `athena:StartSession`/`UpdateSession` being the only one that spares Athena SQL), and the network-conditioned deny for the domain execution role that INT-16's fallback (i) now names: <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/network-isolation.html>. **Re-read 2026-08-19 and three things were sharper than this row said** — AWS ships the deny statement verbatim (`Sid` `DenyAthenaSparkStartSession`, `Resource` `arn:aws:athena:*:*:workgroup/*`); the Tooling Athena flag applies to **new projects only**, and the third control is *"remove Amazon Athena Spark permissions from individual project IAM policies"*, i.e. a grant-shaped edit on blueprint-authored policies rather than the permissions boundary Stage 6 step 2.1 builds; and `DenyUserAccessFromUnauthorizedVPCs` carries **three** conditions, the third being `StringLike aws:userid = *:user-*`, which is what confines it to portal users and spares the catalog service on the same role. The endpoint tables are split **required** (`athena`, `datazone`(+`-fips`), `ec2`, `ec2messages`, `q`, `s3`, `sagemaker.api`, `sagemaker.runtime`, `glue`, `kms`, `secretsmanager`, `sts`, `ssm`, `ssmmessages`) and **optional** (per blueprint) — and the `q` row pairs with `com.amazonaws.us-east-1.codewhisperer`, *"available only in us-east-1"*, which a `us-west-2` VPC cannot reach through an interface endpoint at all (Stage 6 step 4.2). **Re-read 2026-08-24, attributing the on-VPN portal break — and the page has a THIRD table both earlier readings skipped, *Public internet access***: the portal's client assets, its client APIs (**`agent.datazone.<region>.api.aws`** among them) and the IdC sign-in endpoints *require the public internet* — so one page instructs the `datazone` endpoint AND requires a name that endpoint's private zone shadows (authoritative for the subtree; `NETWORK.md` §5). The required tables are scoped by the page's own no-internet premise, never by `VpcOnly` — its troubleshooting table answers *"Private with NAT + Private with NAT: Works as expected, no action needed"* — which is what the 4.2 comment's "REQUIRED under VpcOnly" got wrong (Lesson 41). Where the two tables collide, the browser-facing one wins. **Re-read 2026-08-25 (verified by panel against the live page)**: the page now also carries a *VPC troubleshooting* section — the scenario table behind the "your domain may be using a public subnet" banner, with *Private with NAT* blessed twice and *"network calls … route over the public internet when that network path is available"* stated outright — and the public-internet section's four tables (portal assets, portal client APIs, IdC sign-in, console web client) are the seed of the Sandbox allow-list's 2026-08-25 portal/console block (`EXC-06`'s exit path). Same reading, required-table diff against this estate: the non-fips gaps are exactly `ec2`, `ec2messages`, `q`/`codewhisperer`, `secretsmanager`, `ssm`, `ssmmessages` — all NAT-served under design A, added only by measurement (verification (viii)). **Re-read 2026-09-05 for the 6b-6d review, and this time the OPTIONAL table was transcribed in full, because design B makes it load-bearing** — its scoping sentence is *"Create these Amazon VPC endpoints if you plan to deploy Amazon SageMaker Unified Studio projects that include blueprints using the services listed below"*, and with no default route a project whose blueprint's endpoints are missing has **no path at all** rather than a slower one: Amazon Bedrock (`bedrock-agent`, `bedrock-agent-runtime`, `bedrock-runtime`); Athena Spark (`athena.sessions`, `athena.dashboard`, `athena.persistent-dashboard`); Glue interactive sessions (`glue.sessions`); CloudWatch (`logs`); EMR (`elasticmapreduce`, `elasticmapreduce-services`, `emr-serverless`, `emr-serverless-services.livy`, `emr-serverless-services.sessions`, `emr-serverless.dashboard`, `emr-dashboard`, `elasticmapreduce-fips`); EMR on EKS (`emr-containers`); RDS; Redshift (six names); **Portal Query Editors (`sqlworkbench`, `sqlworkbench-v2`)**; CodeCommit; and **CodeConnections (`codeconnections.api`, `codestar-connections.api`)** — the last of which is what open question 26's `gitConnectionArn` would need, and therefore Stage 7's input rather than Stage 6's. The same reading confirmed the `q` row still pairs with the `us-east-1`-only `codewhisperer`, and that the sentence *"If Amazon VPC endpoints are missing or misconfigured, network calls … route over the public internet when that network path is available"* is exactly what stops being true under design B — which is why 6c step 5.3 turns the optional table into a costed decision instead of a list edit.
...
- **Remote access to SageMaker spaces from a local IDE** — the `sagemaker:StartSession` API behind "connect your local VS Code to a Unified Studio space", the three connection methods (deep link, AWS Toolkit, SSH), and AWS's own recommendation to scope `StartSession` by tag to a user's *own* private applications. It is the action that matches neither `sagemaker:Create*` nor `datazone:*`, which is why Stage 1c step 7.6 names it explicitly and why `docs/plan/open-questions.md` item 14 treats it as an egress channel: <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access.html> and <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/local-ide-support.html>.
  - **Consumed by 6d step 7.1, 2026-09-07** — the pages the row above and the 2026-08-16 pair had carried without a consumer, read together:
    ...
    - *`session-manager-plugin`, `websocketutil.go`* — the plugin dials with gorilla's `websocket.DefaultDialer`, whose `Proxy` is `http.ProxyFromEnvironment`; the one fact that says the data channel honours `HTTPS_PROXY` **if the variable reaches the process**, which is 7.5's measurement and not a documented claim: <https://github.com/aws/session-manager-plugin/blob/mainline/src/websocketutil/websocketutil.go>.
