...
- Installing the `aws` client: <https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html>.

- Installing `terraform`: <https://developer.hashicorp.com/terraform/install>.

- uv: <https://docs.astral.sh/uv/>. Installs the two Python gates below at the versions `CLAUDE.md`
  pins, and runs every script in this repository: they are Python 3 on the project in `pyproject.toml`,
  and their `#!/usr/bin/env -S uv run --quiet` shebang makes `uv run` resolve the pinned interpreter
  (`.python-version`) and the locked environment (`uv.lock`) per invocation. The project and
  script-running model: <https://docs.astral.sh/uv/guides/projects/>.

- `ruff`: <https://docs.astral.sh/ruff/>. Linter and formatter for the repository's Python, a dev-only
  dependency of the uv project; `pre-commit` runs it through `uv run`, so the gate binds to the locked
...
- `pre-commit`: <https://pre-commit.com/>. The hook collection the repository configures is
  `antonbabenko/pre-commit-terraform`: <https://github.com/antonbabenko/pre-commit-terraform>. The
  `terraform_validate` argument is `--tf-init-args=`, not `--init-args=` (Stage 2 step 6.4).

- `checkov`: <https://www.checkov.io/>. The policy gate of Stage 2 step 6.5; suppressions are inline
  `# checkov:skip=CKV_...` with a reason, never a global exclusion.

- `tflint`: <https://github.com/terraform-linters/tflint>, with the AWS ruleset
  <https://github.com/terraform-linters/tflint-ruleset-aws>. A Go binary with no Python or Homebrew
  path: on a machine without `brew`/`go`/`npm` it is a release download plus a checksum check.

- AWS Pricing Calculator: <https://calculator.aws/>.

- AWS Price List bulk API, offer index (all service codes): <https://pricing.us-east-1.amazonaws.com/offers/v1.0/aws/index.json>. Per service and Region: `offers/v1.0/aws/<serviceCode>/current/<region>/index.json`; the Regions a service is offered in: `offers/v1.0/aws/<serviceCode>/current/region_index.json`. Public, no credentials required; the source of every rate in `docs/PRICING.md`.

- AWS CodeArtifact endpoints and quotas (CodeArtifact is not available in `sa-east-1`): <https://docs.aws.amazon.com/general/latest/gr/codeartifact.html>.
...
- AWS Organizations - Declarative policies: <https://docs.aws.amazon.com/organizations/latest/userguide/orgs_manage_policies_declarative.html>.
  Read for Stage 1c step 7.8; three facts the battery's shape depends on. (i) *"Declarative policies
  are enforced in the service's control plane"*, not in authorization: they name no policy id, emit no
  *"explicit deny"* wording, and govern service-linked roles, which SCPs and RCPs do not. A dry run
  stops after authorization and reports `DryRunOperation` whether or not the policy is attached, so
  `--phase decl`'s probes carry no `--dry-run`. (ii) Detaching rolls each attribute back to its
  previous state. (iii) With no custom `exception_message`, AWS supplies *"This action is denied due
  to an organizational policy in effect"*, so the message is the attribution; the driver reports
  `custom-message` or `AWS-default-msg`. Not on the page, measured 2026-08-14: a successful read of a
  managed attribute echoes the same message — `ec2 get-instance-metadata-defaults` returns rc=0 with
  `"ManagedBy": "declarative-policy"` and the custom text in `"ManagedExceptionMessage"` — so only the
  exit code separates enforcement from confirmation, and `classify` gates both declarative branches
  on it. The page names no management-account exemption, unlike the SCP and RCP pages, so a root
  attach is expected to reach Management; `./aws/declarative-ec2.py -` run there is the only way to
  find out.

- AWS Service Reference Information — the machine-readable list of every IAM action a service
  publishes, one JSON per service at
  `https://servicereference.us-east-1.amazonaws.com/v1/<service>/<service>.json`, indexed at
  <https://servicereference.us-east-1.amazonaws.com/>:
  <https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_service-reference.html>. Used in
  Stage 1c step 7.6 for verification (viii): the only source that says what an action is called today,
  and readable by a script. Gap measured 2026-08-13 (decision 5): it maps `aws:RequestTag` to none of
  EC2's 793 actions while declaring the key in EC2's top-level `ConditionKeys`, where S3 maps it on 11
  of 180 actions and RDS on 35 of 169. A negative answer from this file is evidence only when the same
  service answers positively elsewhere (Lesson 13); for EC2, use the Service Authorization Reference
  page or a probe.
...
- Removing an Account Factory portfolio and product from Service Catalog (the portfolio/principal model behind the Account Factory access error): <https://docs.aws.amazon.com/controltower/latest/userguide/controltower-walkthrough-cleanup-account-factory.html>.

- IAM Identity Center groups for AWS Control Tower — which group gets which permission set in which account: `AWSControlTowerAdmins` = administrator on Management, Log Archive *and* Audit, plus `AWSOrganizationsFullAccess` on member accounts; `AWSAccountFactory` = `AWSServiceCatalogEndUserAccess` on Management only; only `AWSControlTowerAdmins` members reach the Control Tower console: <https://docs.aws.amazon.com/controltower/latest/userguide/sso-groups.html>.
...
- Disabling an IAM Identity Center user (the mechanism D33 cited; D34 withdrew the retirement of `AWS Control Tower Admin`, so no user in this design is scheduled for disabling): <https://docs.aws.amazon.com/singlesignon/latest/userguide/disableuser.html>.
...
- Block public access for EBS snapshots — free, account-level and Regional, settable org-wide only
  through a declarative policy (and then no longer changeable inside the account). *"Block public
  access for snapshots does not prevent private snapshot sharing"* (Stage 1c step 7.8), which is why
  the cross-account share is denied by SCP in 7.5. It does not cover EBS-backed AMIs, which need block
  public access for AMIs separately:
  <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/block-public-access-snapshots.html>.

- IAM Access Analyzer — the six capabilities, and the three facts D6 and `docs/plan/architecture.md`
  §4.2 depend on: the resource-type list for external access (it includes EBS and RDS snapshots, EFS,
  Lambda and SNS, none of which any RCP covers), the narrower list for internal access (S3, RDS
  snapshots, DynamoDB — no EFS, no catalog), and an external-access analyzer analyzing only its own
  Region while unused access does not:
  <https://docs.aws.amazon.com/IAM/latest/UserGuide/what-is-access-analyzer.html>. The billing
  dimensions are on the same page — per principal-month for unused access, per resource-month for
  internal access, per request for custom policy checks — and measured 2026-08-17 (`docs/PRICING.md`
  §6: internal USD 9.00/resource-month, unused 0.20, checks 0.002). The pricing page,
  <https://aws.amazon.com/iam/access-analyzer/pricing/>, adds that internal/unused analyzers are
  charged once during setup and then monthly on the first, not prorated. Creating the internal
  analyzer — org zone of trust from the delegated administrator, one org-level internal analyzer per
...
- Account Management: trusted access and `PutAccountName` — Stage 6b step 3's two prerequisites (read
  2026-09-05). Enabling trusted access *"allows the administrator of the management account to modify
  the information and metadata … specific to each member account"*, is performable only from the
  management account, and requires all features enabled; after it, *"you can use the `accountID`
  parameter in those Account Management API operations that support it"*, from the management account
  or a delegated administrator. `put-account-name` needs the `account:PutAccountName` permission, takes
  a name of 1-50 characters matching `[ -;=?-~]+`, and the management account cannot pass its own
  `--account-id`. No propagation delay is documented, so the name is read back rather than the call
  re-issued: <https://docs.aws.amazon.com/accounts/latest/reference/using-orgs-trusted-access.html>
  and <https://docs.aws.amazon.com/cli/latest/reference/account/put-account-name.html>.

- Control Tower account auto-enrollment — the second of Stage 6b step 0.5's two switches (read
  2026-09-05). The opt-in is the landing zone's `remediationTypes` field set to the single value
  `INHERITANCE_DRIFT`, a top-level field of `GetLandingZone`'s response rather than a key inside the
  manifest, so `aws/cloudshell/management-landing-zone-drift.sh` reads it without parsing the manifest
  (whose schema is unpublished). It needs landing zone 3.1 or later and is set through the console
  settings page or `CreateLandingZone`/`UpdateLandingZone`. An account moved with the Organizations
  API or console into a registered OU "automatically receives baseline resources and controls from
  that OU", and the source OU's are removed, instead of raising inheritance drift. Three limits decide
  Stage 6b step 3.4: it is not retroactive (only accounts moved after the opt-in); it "does not
  create, modify, or terminate AWS Service Catalog provisioned products", so an Account Factory
  account's provisioned product keeps the old OU and the old name until somebody updates it; and the
  account "may show *Moved member account* drift" when the source and destination OUs differ in
  configuration, as `Interactive` and `Workloads` do. Enrollment is eventually consistent, "a few
...
- `aws_organizations_organizational_unit_descendant_organizational_units` — the recursive counterpart, given the root ID (Stage 2 step 5.3; confirmed against the pinned provider in Stage 2 verification (iv)): <https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/organizations_organizational_unit_descendant_organizational_units>.

- Terraform `removed` block — how Stage 6c drops the transferred Elastic IP from Sandbox's state
  without releasing it (read 2026-09-05). Available since v1.7; the resource block is replaced by
  `removed { from = <address>  lifecycle { destroy = false } }`, which removes the resource from state
  and leaves it alone: <https://developer.hashicorp.com/terraform/language/block/removed>.
...
- Building a Scalable and Secure Multi-VPC AWS Network Infrastructure (whitepaper):
  ...
  A reading recommendation, not yet read against this design, so nothing here rests on it. It is the
  closest published statement of the questions D5 and D6 answer for this estate: when a Transit
  Gateway earns its cost against peering, where centralised egress and centralised interface endpoints
  belong, and how DNS resolves across accounts that share them (the family Lessons 40-43 live in).
  Written for an estate larger than this one.

- Route 53 Resolver DNS Firewall: <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall.html>.
  Domain-list syntax (read 2026-08-22): the `*` must replace the entire leftmost label
  (`*prod.example.com` and `prod*.example.com` are both rejected), and it matches that label and every
  subdomain beneath it at any nesting depth, but not the apex — which is why every entry in
  `terraform-modules/vpc-egress/variables.tf` is written twice:
  <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall-user-managed-domain-lists.html>
  and <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/DomainNameFormat.html>. The depth
  half was confirmed from inside the VPC on 2026-08-22: `*.julialang.net` matched
  `us-west.pkg.julialang.net`, two labels deep. Neither page states that DNS Firewall evaluates the
  whole resolution chain — a listed name whose CNAME target is unlisted is blocked, and the Resolver
  query log reports that block against the queried name with the catch-all list id; established by
  paired probe on 2026-08-23 (Stage 6 step 4.3; Lesson 38).

- Route 53 Resolver DNS Firewall, the domain redirection rule setting (read 2026-08-23):
  <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall-rule-settings.html>.
  Chain evaluation is a per-rule setting with a default, not the service's behaviour:
  `INSPECT_REDIRECTION_DOMAIN` (the default this module had been taking) against
  `TRUST_REDIRECTION_DOMAIN`, which "verifies only the initial domain". Announced 2024-05:
  <https://aws.amazon.com/about-aws/whats-new/2024/05/amazon-route-53-resolver-dns-firewall-domain-redirection/>.
  The trust "only applies within a single DNS query transaction", and a client that queries a
  redirection target directly is evaluated "as an independent query with no trust context" — so
  trusting the chain does not make the CDN reachable. Mechanism:
  <https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall-overview.html>; API
  field: <https://docs.aws.amazon.com/botocore/latest/reference/services/route53resolver/client/create_firewall_rule.html>.

- AWS Network Firewall stateful domain list rule groups — the L7 alternative priced in
  `docs/plan/architecture.md` §4.3a and not built (read 2026-08-23):
  <https://docs.aws.amazon.com/network-firewall/latest/developerguide/stateful-rule-groups-domain-names.html>.
  It matches the TLS SNI for HTTPS and the HTTP Host header for HTTP, so it never consults DNS and a
  CNAME chain is irrelevant to it. `TLS_SNI`, `HTTP_HOST` or both, per rule group; a leading `.` is its
  wildcard.

- AWS Network Firewall: <https://docs.aws.amazon.com/network-firewall/latest/developerguide/what-is-aws-network-firewall.html>.

- The Network account and its OU in the Security Reference Architecture (read 2026-08-23, for where
  the buildbox's egress belongs). The SRA puts a Network account in an Infrastructure OU for
  separation of duties rather than topology: it "isolates the networking services, configuration, and
  operation from the individual application workloads", and splits inbound from outbound into two
  VPCs because "the inbound network is generally considered higher risk"; a third, the inspection VPC,
  carries Network Firewall and hosts nothing else:
  <https://docs.aws.amazon.com/prescriptive-guidance/latest/security-reference-architecture/network.html>.
  The multi-account whitepaper names this project's use case as an Infrastructure-OU responsibility —
  "VPCs and network security stacks used for centralized internet traffic inbound and outbound
  proxying and filtering" — and licenses the estate's shape: peering "is best used when ... the number
  of VPCs to be connected is fewer than 10", and Transit Gateway is the default "but specific needs
  around cost, bandwidth, and latency might make VPC peering a better fit":
  <https://docs.aws.amazon.com/whitepapers/latest/organizing-your-aws-environment/infrastructure-ou.html>.

- Centralized egress — the principle, and AWS's cost caveat against its own pattern (read 2026-08-23).
  The principle: "a single, common entry point for all network traffic that is destined to the
  internet", where inspection sits and traffic is allowed "only to specified domains"; its "Best
  practices for securing egress traffic" list recommends starting in logging-only mode, which this
  project never did:
  <https://docs.aws.amazon.com/prescriptive-guidance/latest/transitioning-to-multiple-aws-accounts/centralized-egress.html>.
  The mechanism whitepaper: <https://docs.aws.amazon.com/whitepapers/latest/building-scalable-secure-multi-vpc-network-infrastructure/centralized-egress-to-internet.html>.
  The caveat, AWS arguing against centralization at small scale: "When you centralize NAT gateway
  using Transit Gateway, you pay an extra Transit Gateway data processing charge — compared to the
  decentralized approach of running a NAT gateway in every VPC ... keeping the NAT local in the VPC to
  avoid the Transit Gateway data processing charge might be a more cost-effective option":
  ...
  both, against USD 0.01/GB each way for VPC peering, which has no hourly fee.

- The explicit proxy reached by PrivateLink — the shape that needs no peering and no Transit Gateway
  (read 2026-08-23). A Squid fleet in a hub account behind an NLB published as an endpoint service;
  spokes consume it with an interface endpoint and point `HTTP(S)_PROXY` at it. "L3 network
  connectivity is NOT required between the central hub and 'spoke' account VPCs, as it leverages AWS
  PrivateLink": the proxy is reached as a service, never as a route, so no packet with a public
  destination crosses a VPC boundary:
...
- AWS Network Firewall Proxy — the managed version of the above, not available in `us-west-2` (read
  2026-08-23). An explicit forward proxy as a Network Firewall deployment mode
  ("no-source-preservation"), where "clients must be explicitly configured to send traffic to the
  proxy through a PrivateLink endpoint", with AWS's managed rule groups behind it:
  <https://docs.aws.amazon.com/network-firewall/latest/developerguide/network-firewall-proxy-developer-guide.html>.
  Availability: "Try out AWS Network Firewall in *no-source-preservation* deployment with *proxy*
  functionality in your test environment today in US East (Ohio) region", free during public preview:
  <https://aws.amazon.com/about-aws/whats-new/2026/08/aws-network-firewall-forward-proxy-preview/>.
  A revision trigger: the day it reaches `us-west-2` and GA, a hand-run Squid fleet stops being the
  only way to have an explicit proxy.

- VPC peering: <https://docs.aws.amazon.com/vpc/latest/peering/what-is-vpc-peering.html>.

- VPC peering limitations — the page Lesson 44 and D38 quote (read 2026-09-05 for Stage 6c). Its
  *"Edge to edge routing through a gateway or private connection"* section states that a peer cannot
  borrow the other VPC's internet gateway, *"a NAT device"*, a VPN or Direct Connect connection, or
  *"a gateway endpoint that provides connectivity to Amazon S3"*, and that *"VPC peering does not
  support transitive peering relationships"*. *"You cannot connect to or query the Amazon DNS server
  in a peer VPC"* decides INT-22: a private zone reaches a spoke only by being associated with that
  spoke's own VPC, and a missing association fails as NXDOMAIN rather than as a referral. The
  same-Region peering MTU is 9001, and *"unicast reverse path forwarding … is not supported"*:
  <https://docs.aws.amazon.com/vpc/latest/peering/vpc-peering-basics.html>.

- Elastic IP address transfer between accounts — Stage 6c step 4.5 (read 2026-09-05). *"There is no
  charge for transferring Elastic IP addresses"*; same Region only; a two-step handshake with seven
  days to accept, after which ownership returns to the source; an accepted transfer stays visible to
  the source for 14 days; *"AWS does not notify transfer accounts about pending … requests"*, so the
  two calls are one sitting; and *"any tags that are associated with an Elastic IP address being
  transferred are reset when the transfer is complete"*. The three refusals `./aws/eip-transfer.py`
  checks are raised at accept time, in the recipient account: `InvalidTransfer.AddressAssociated` (the
  source must disassociate first), `InvalidTransfer.AddressCustomPtrSet` (a reverse-DNS record must be
  removed) and `AddressLimitExceeded` (five Elastic IPs per Region by default). BYOIP, IPAM-pool and
  CoIP addresses cannot be transferred:
...
- Walkthrough: configure AWS Control Tower without a VPC — the supported cleanup of an existing account's Account Factory VPC is removing its stack instance from `AWSControlTowerBP-VPC-ACCOUNT-FACTORY-V1`, and the Account Factory fields that stop future vends creating one are internet-accessible subnet off, private subnets 0, every Region checkbox cleared (Stage 3 step 0): <https://docs.aws.amazon.com/controltower/latest/userguide/configure-without-vpc.html>.

- Amazon ECR interface VPC endpoints — the minimum S3 gateway-endpoint permission for image pulls is `s3:GetObject` on `arn:aws:s3:::prod-<region>-starport-layer-bucket/*` (Stage 3 step 9.3's fourth family). `ecr.dkr` is the Docker Registry API — *"Docker client commands such as `push` and `pull` use this endpoint"* — while S3 is what a container reaches to download layers, so a push never touches the S3 gateway endpoint and the allow-list's missing `PutObject` on the starport bucket is the documented shape (Stage 6 step 5.0, `buildbox.md` §P): <https://docs.aws.amazon.com/AmazonECR/latest/userguide/vpc-endpoints.html>.

- SSM Agent communications with AWS managed S3 buckets — the bucket list behind Stage 3 step 9.3's SSM family: `amazon-ssm-<region>` (agent updates) and `aws-ssm-<region>` (SSM document modules) at minimum, plus the patching and Distributor buckets this design does not use: <https://docs.aws.amazon.com/systems-manager/latest/userguide/ssm-agent-technical-details.html>.

- Update dnf on an Amazon Linux instance without internet access — AL2023's default `mirrorlist=` URL points into the regional repository bucket itself (`al2023-repos-<region>-de612dc2`, the S3 dualstack hostname), so the S3 gateway endpoint carries the whole package path; only a repo file referencing `cdn.amazonlinux.com` needs internet (Stage 3): <https://repost.aws/knowledge-center/ec2-al1-al2-update-yum-without-internet>.
...
- Chrome's Local Network Access permission — a public page's request to a private address is gated
  behind a user grant, and ungranted it rejects as `TypeError: Failed to fetch`. A non-AWS term in the
  reach question (Lesson 43): on the full-tunnel laptop the SMUS portal is a public origin while
  `glue`, `sagemaker.api` and the `*.studio.<region>.sagemaker.aws` subtree resolve to the VPC's
  interface endpoints, so the portal needs the grant to work on the VPN at all:
  <https://developer.chrome.com/blog/local-network-access>. Its predecessor, Private Network Access,
  carries the address-space model the gate is built on:
  <https://developer.chrome.com/blog/private-network-access-update>.

- Chromium's proxy flags — `--proxy-server` and `--proxy-bypass-list`, the bypass-rule syntax (suffix
  patterns, IP literals, `<local>`); with neither flag Chrome reads the platform's system proxy, which
  on macOS is empty while the WireGuard tunnel is primary (issue #67), so the flag is the working path
  (`runbooks/client-vpn-proxy-configuration.md` §4.1):
...
- Create a SageMaker Unified Studio domain, manual setup — names the two roles the domain asks for, `AmazonSageMakerDomainExecution` and `AmazonSageMakerDomainService`, and shows the domain being created through the **DataZone** console (Stage 1c step 7.6: which namespace the domain evaluates under): <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/create-domain-sagemaker-unified-studio-manual.html>.
...
- Enable Tooling blueprint (SMUS admin guide) — the console flow's **Authorized domain units** field is the half `PutEnvironmentBlueprintConfiguration` does not perform: the API creates the configuration, the console also authorizes it, which is why step 1.4's API-enabled blueprints authorized nobody (the `Caller is not authorized to create environment using blueprintId` rollback): <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/enable-tooling-blueprint.html>. Read 2026-08-22.

- Manage blueprint authorization, and the domain-unit policy list (SMUS admin guide) — a blueprint's **Authorization** tab (add/remove domain units, the cascade toggle) and the authorization policies a domain unit carries; the DataZone user guide names the blueprint configuration's two grantable policies: <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/authorization-for-blueprints.html>, <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/domain-units.html>, <https://docs.aws.amazon.com/datazone/latest/userguide/assign-authorization-policies-in-blueprint-config.html>. Read 2026-08-22.

- `AddPolicyGrant` (DataZone API reference) — `CREATE_ENVIRONMENT_FROM_BLUEPRINT` on entity `ENVIRONMENT_BLUEPRINT_CONFIGURATION`, whose `entityIdentifier` format the reference does not give (every derivable spelling is rejected as *format invalid*): <https://docs.aws.amazon.com/datazone/latest/APIReference/API_AddPolicyGrant.html>; the open re:Post asking the same question: <https://repost.aws/questions/QUjdtJm8DRTFG_qx0vbVNh1g/amazon-data-zone-add-policy-grant-list-policy-grants-rejects-environment-blueprint-configuration-identifier-what-is-the-correct-format>. Read 2026-08-22.

- `aws-samples/sample-automate-sagemaker-unified-studio-using-iac` — the working CloudFormation that settles both undocumented halves of the blueprint grant: `EntityIdentifier: "${AWS::AccountId}:${EnvironmentBlueprintId}"` (the configuration-owning account) and the principal (every project in a domain unit, designation `CONTRIBUTOR`). `terraform-modules/sagemaker-prereqs/grants.tf` copies it: <https://github.com/aws-samples/sample-automate-sagemaker-unified-studio-using-iac>. Read 2026-08-22.
...
- Connect to Amazon S3 from SageMaker Unified Studio — the S3 connection Stage 16 wires. The console
  path is **Connections → Create connection → Amazon S3** (the user observed the same act offered as
  *Add S3 location*; whether they are one form is the stage's 4.2 reading). Fields: Name (required),
  S3 URI (optional; with none, every bucket the credential can see is listed), Region, and an optional
  access role picked from a dropdown of existing roles, required cross-account. The post-create
  verification is an S3 Access Grants location registered for the URI against that role; a failed
  registration means delete the connection and recreate, there is no repair path:
  <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/connecting-amazon-s3.html>.
  Read 2026-08-26.

- Adding existing Amazon S3 data to a SageMaker Unified Studio project — the two prerequisites Stage
  16 chooses between. Option 1 (recommended by AWS): a dedicated access role with the documented trust
  (`access-grants.s3.amazonaws.com` with `aws:SourceAccount` + `sts:SetSourceIdentity`, and the project
  role with `sts:ExternalId` = the project id and tag-session conditions) and inline S3 +
  Access-Grants-management + `iam:PassRole` statements. Option 2: the S3 and Access-Grants-management
  statements inline on the project role (no `iam:PassRole`, there being no access role to pass). A
  bucket policy is prescribed only cross-account, and SMUS uses an existing S3 Access Grants instance
  or creates one itself — matching the measured SMUS-born Sandbox instance:
  <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/adding-existing-s3-data.html>.
  Read 2026-08-26.

- The DataZone connection API triple behind the form (Stage 16 verification (iv)'s vocabulary): `CreateConnection` carries the access role as top-level `awsLocation.accessRole` (and an `enableTrustedIdentityPropagation` flag), `S3PropertiesInput` is only `s3Uri` + `registerS3AccessGrantLocation` + `s3AccessGrantLocationId` (no role field), and `GetConnection` returns `connectionCredentials` — an STS triple, so connections vend credentials: <https://docs.aws.amazon.com/datazone/latest/APIReference/API_CreateConnection.html>, <https://docs.aws.amazon.com/datazone/latest/APIReference/API_S3PropertiesInput.html>, <https://docs.aws.amazon.com/datazone/latest/APIReference/API_GetConnection.html>. Read 2026-08-26.
...
- SageMaker Catalog discoverability and governance for S3 general-purpose buckets (AWS Big Data blog, 2025-08-21) — the end-to-end S3 publish/subscribe walk-through, with the S3 Access Grants boto3 plugin as the notebook consumption path: <https://aws.amazon.com/blogs/big-data/amazon-sagemaker-catalog-expands-discoverability-and-governance-for-amazon-s3-general-purpose-buckets/>. Read 2026-08-26.
...
- S3 Access Grants — the laptop→project-storage strategy (`s3-read-write/`, the
  `VendProjectStorageCredentials` statement); four facts, all read 2026-08-23:
  ...
  - *Why not the DataZone vending API* — AWS's own `sagemaker-studio` SDK (PyPI, no public repo)
    ...
    Strategy 1-B's anchor if 1-A falls through: <https://pypi.org/project/sagemaker-studio/> and
  ...
  - *The price*, written only in the Price List offer file — USD 0.03 per 1,000 non-delete AG
...
- JupyterLab lifecycle configurations — how the proxy variables reach a stock image (read 2026-09-05
...
- LF-Tag permissions — the grants that let a persona tag rather than read (Stage 5 pass 2, the
  ...
  permissions are themselves grantable, so only a data lake administrator can grant them until it
  delegates with the grant option. The LF-TBAC considerations page adds: *"You need to have `Grant
  with LF-Tag expressions` permission to grant data permissions on Data Catalog resources by using the
  LF-TBAC method. The data lake administrator and the LF-Tag creator implicitly receive this
  permission"* — so a persona holding only `ASSOCIATE` on the tags and `DESCRIBE` on the catalog,
  which is what the governance manager holds, can tag and cannot grant (decision 5). One ambiguity is
  Stage 6's: the same sentence gives the implicit permission to the LF-Tag *creator*, and the
  persona's IAM half carries `lakeformation:CreateLFTag`; whether that makes it a creator for tags it
  did not create is stated nowhere, and only a real governance-manager session answers it. These
  pages are JavaScript-rendered and return no body to a plain fetcher; a rendering browser reads them:
...
- Cross-account LF-TBAC sharing — the two statements Stage 5 pass 3 is built on (read 2026-08-19, for
  ...
  nothing inside it can use the share until that account's own data lake administrator passes it on.
  The receiving side, with the worked LF-Tag example, is on the regranting page; the bound is on the
  considerations page: a resource shared with an account may be granted only to principals in that
  account, never onward to another account, to an organization, or to `IAMAllowedPrincipals`. The
  Data Catalog resource-policy prerequisite is conditional: the `glue:ShareResource` statement is
  needed by an account already sharing through a Glue Data Catalog resource policy (the version 1/2
  path) and "is not required if your account has made no cross-account grants using the AWS Glue Data
  Catalog resource policy". Measured in Data Governance the same day: `glue:GetResourcePolicy` →
  `EntityNotFoundException`, so no policy is written:
  <https://docs.aws.amazon.com/lake-formation/latest/dg/cross-account-TBAC.html>,
  <https://docs.aws.amazon.com/lake-formation/latest/dg/regranting-shared-resources.html>.

- What the consumer side of a share requires (read 2026-08-19, the Prerequisites and LF-TBAC pages):
  *"at least one user in the consumer account must be a data lake administrator to view shared
  resources"*, and *"other principals can't access shared resources until the data lake administrator
  grants them permissions"*. Each Interactive account therefore owes a `DataLakeSettings` of its own
  before a link can resolve, with the two hazards the producer side met: the `Parameters` map a
...
- Amazon ECR pull-through cache (read 2026-09-05; under [D38](plan/decisions/D38-single-egress-hub.md)
  the estate has no route to the internet, and the page's considerations decide whether the cache is
  usable). *"When an image is pulled using the pull through cache rule for the first time a route to
  the internet may be required… if you've configured Amazon ECR to use an interface VPC endpoint using
  AWS PrivateLink then you need to ensure the first pull has a route to the internet. One way to do
  this is to create a public subnet in the same VPC, with an internet gateway"* — a route, which an
  explicit proxy is not; Stage 7 step 5.2 measures it, and the cache is D38's first named
  NAT-contingency candidate. *"For upstream repositories that do not require authentication… the image
  pulls are initiated by AWS IP addresses"*, so the requirement is conditional. *"Turning on image tag
  immutability for repositories using a pull through cache rule will prevent Amazon ECR from updating
  images using the same tag"*. Repository creation templates exist, and can set tag immutability,
  encryption, policies and lifecycle on the repositories ECR creates on your behalf:
...
- Amazon Inspector, supported operating systems and programming languages (read 2026-08-22, for the
  paid upgrade Stage 7 decision 2 defers). For ECR container images the supported languages are C#,
  Go (and the Go toolchain), Java (and the JDK), JavaScript, PHP, Python, Ruby and Rust — *"Amazon
  Inspector doesn't scan for toolchain vulnerabilities in Rust"* is the one carve-out named. Julia and
  R appear nowhere on the list, which makes the Julia/R half of the `dev-env` image an accepted
  residual (`institutional-delta.md`, the image-scanning row):
  <https://docs.aws.amazon.com/inspector/latest/user/supported.html>. The companion page: *"With basic
  scanning, you can configure your repositories to scan on push… With enhanced scanning, you scan for
  operating system and programming language package vulnerabilities"*; basic is billed through ECR,
  enhanced through Inspector: <https://docs.aws.amazon.com/inspector/latest/user/scanning-ecr.html>.

- ECR lifecycle policy properties (read 2026-08-22, for the image tag convention in
  [`SMUS.md`](SMUS.md) §Custom images). `tagPrefixList` matches a prefix only (*"if your images are
  tagged as `prod`, `prod1`, `prod2` … you would use the tag prefix `prod`"*); `tagPatternList` takes
  patterns with up to four `*` wildcards each, so it can match a suffix, and the page calls it *"best
  practice"*. The two are mutually exclusive in one rule — each is required only when the other is
  absent and `tagStatus` is `tagged` — and for both, *"if you specify multiple tags, only the images
  with all specified tags are selected"*: an AND, where a list reads like an OR:
...
- GitLab Linux package (Omnibus), SSL configuration (read 2026-09-05; Stage 7 step 1.3). *"Let's
  Encrypt is enabled by default if `external_url` is set with the HTTPS protocol and no other
  certificates are configured"*, and *"GitLab attempts to renew any Let's Encrypt certificate with
  every reconfigure"* — so on a `.internal` name behind a proxy, `letsencrypt['enable'] = false` is
  mandatory, or every reconfigure fails against an unreachable ACME server. The certificate pair is
  hostname-named (`/etc/gitlab/ssl/<host>.crt` at 644, `.key` at 600), and a custom CA root is
  trusted by dropping `.crt` files into `/etc/gitlab/trusted-certs/` followed by `gitlab-ctl
  reconfigure` — INT-19's fourth client surface: <https://docs.gitlab.com/omnibus/settings/ssl/>.

- GitLab Linux package, environment variables (read 2026-09-05). The proxy is configured per
  component: `gitlab_rails['env']`, `gitaly['env']`, `gitlab_workhorse['env']`, `gitlab_pages['env']`
  and `registry['env']`, each taking `http_proxy`/`https_proxy`. `no_proxy` takes wildcard syntax for
  an internal domain and must not carry a port — a port there breaks DNS resolution during repository
  mirroring: <https://docs.gitlab.com/omnibus/settings/environment-variables/>.

- GitLab Runner behind an HTTP proxy (read 2026-09-05; why Stage 7 step 6.2 has four sub-items). The
  runner service needs its own systemd drop-in
  (`/etc/systemd/system/gitlab-runner.service.d/http-proxy.conf`), the docker daemon needs a second
  one, and build containers are handed the values through `config.toml`'s `environment` — in both
  cases, because *"certain programs expect `HTTP_PROXY` and others `http_proxy`"*. `NO_PROXY`
  wildcards work only as suffixes — not as prefixes, and not as CIDR notation — so an RFC1918 block
  written there is silently ignored (6c step 5.6): <https://docs.gitlab.com/runner/configuration/proxy/>.

- GitLab Pages administration (read 2026-09-05; two Stage 7 questions). For a wildcard Pages domain
  served through the Omnibus nginx proxy no secondary IP address is required (a second address is
  needed only for custom domains); the Pages domain must not be a subdomain of `external_url` (D36's
  two-apex rule); and access control is available in the Free tier
  (`gitlab_pages['access_control'] = true`): <https://docs.gitlab.com/administration/pages/>.

- AWS CodeArtifact external connections (read 2026-09-05; what makes design B work for packages): the
  service fetches from the public repository, so a client with no internet still resolves a package
  that has never been cached. Each repository can have only one external connection, which is why
  `pypi` (`public:pypi`) and `crates` (`public:crates-io`) are two repositories rather than one; the
  supported set is enumerated on the page:
...
- Stage 7's documentation pass (read 2026-08-16):
  ...
  - *Prices measured for this pass* (Lesson 6, the bulk API): Secrets Manager
...
- MWAA Serverless networking — the page that decided D38's contingency had no candidate (read
  2026-09-05). It describes two VPC shapes, and the requirements lists read as contradictory until
  the section headings are read with them (Lesson 41). *Public routing over the internet*: the
  subnets *"must have a route table to a NAT device (gateway or instance)"* and two NAT gateways are
  listed as required infrastructure. *Private routing without internet access*: the subnets *"must
  not have a route table to a NAT device (gateway or instance), nor an internet gateway"*, and are
  served by VPC endpoints with private DNS enabled, associated to both private subnets and to the
  workflow's security group. Common to both: minimum 2, maximum 16 subnets, *"these subnets must be
  private"*, at least two in different Availability Zones, up to five security groups, and a security
  group with a self-referencing inbound rule and an outbound rule for all traffic. The companion page
  ships a CloudFormation template for the private shape whose only endpoints are `logs`, `monitoring`
  and `kms`. A claim to verify rather than to copy: the private shape's NACL is specified as inbound
  allow-all with an outbound deny-all, which a stateless ACL cannot mean literally without breaking
  the endpoints it fronts:
  <https://docs.aws.amazon.com/mwaa/latest/mwaa-serverless-userguide/networking.html> and
  <https://docs.aws.amazon.com/mwaa/latest/mwaa-serverless-userguide/networking-vpc.html>.
...
- Stage 10's documentation pass (read 2026-08-16):
...
- Managing Macie accounts with AWS Organizations, and the integration steps — the two facts Stage 11
  step 1 is written against: designating the delegated administrator *enables Macie in that account*,
  and auto-enable covers new accounts only — "Turning on this setting doesn't affect existing
  accounts in your organization", which must be added one by one (`create-member` takes the account's
  e-mail; the console flow does not): <https://docs.aws.amazon.com/macie/latest/user/macie-organizations.html>
  and <https://docs.aws.amazon.com/macie/latest/user/accounts-mgmt-ao-integrate.html>.

- Amazon GuardDuty: <https://docs.aws.amazon.com/guardduty/latest/ug/what-is-guardduty.html>.

- GuardDuty S3 Protection — monitors CloudTrail data events for S3 through GuardDuty's own stream
  ("you don't need to explicitly enable or configure S3 data event logging in AWS CloudTrail"), with
  its own 30-day free trial on first enablement; what keeps Stage 11 steps 4 and 5 complementary
...
- Logging CloudTrail data events — the advanced event selectors Stage 11 step 5 scopes its trails
  with (`resources.type = AWS::S3::Object`, `resources.ARN` starts-with, `readOnly`), and the
  statement that a trail logging *only* data events carries no management copy:
...
- Managing GuardDuty accounts with AWS Organizations — "For this administrator account, GuardDuty gets enabled automatically only in the current AWS Region", which is why the delegation travels with the enablement (Stage 15): <https://docs.aws.amazon.com/guardduty/latest/ug/guardduty_organizations.html>.

- Setting GuardDuty organization auto-enable preferences — `ALL` covers "all the accounts in an organization", including "those accounts that may have been suspended or removed" and "the delegated GuardDuty administrator account", with up to 24 h to propagate (Stage 15 step 2). Management's own coverage is answered by the members page below (Stage 15 verification (i)): <https://docs.aws.amazon.com/guardduty/latest/ug/set-guardduty-auto-enable-preferences.html>.

- Designating a delegated GuardDuty administrator account — the one-line CLI form
  (`enable-organization-admin-account --admin-account-id … --region us-west-2`), and the two
  properties that shape Stage 15 step 1: the designation is per-Region, and the *same* account must be
  the administrator in every Region where GuardDuty is enabled:
...
- Permissions required to designate a delegated GuardDuty administrator account — the management
  ...
  for Stage 15 step 5: `POLICIES.md`'s "carve out a named administration role" alternative has no
  candidate among GuardDuty's own creations:
...
- Monitoring GuardDuty usage and estimating costs — the 30-day free trial is per account and covers
  ...
  prices S3 Protection without paying for it, and at Stage 15 the estate is populated, so the window
  measures something (Lesson 7): <https://docs.aws.amazon.com/guardduty/latest/ug/monitoring_costs.html>.
...
- Amazon GuardDuty FAQs — "GuardDuty Runtime Monitoring is the only protection plan that is not
  ...
  default" answer given for S3 Protection, EKS Protection and Malware Protection. So the paid add-ons
  arrive on and must be switched off (Stage 15 step 3), and the switch is denied to Audit by
  `DenyGuardDutyTampering` (Stage 15 step 0, decision 1): <https://aws.amazon.com/guardduty/faqs/>.
...
- Compatibility for changing the instance type — *"AMIs are specific to the architecture of the
  processor, so you must select an instance type with the same processor architecture as the current
  instance type"*; an incompatible choice is not a resize at all, it means launching a new instance
  and migrating. Read 2026-08-20 when the VPN host's size became a slice parameter (`vpn.md` §S6):
  every value that parameter admits sits in one family, the one the module's pinned AL2023 AMI
  matches. The same sentence makes the user's move of the host to amd64 a replacement: the module's
  AMI changes (`-arm64` → `-x86_64`, `wireguard-v0.3.0`), the instance is destroyed and recreated, the
  user data re-runs, and the admitted values become `t3.*` where they were `t4g.*`:
  <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/resize-limitations.html>.

- Change the instance type for your Amazon EC2 instance — *"You must stop your instance before you
  can change its instance type"*, and the public-IPv4 warning that follows is scoped to addresses that
  are not Elastic. Switching the VPN host's size is therefore a stop/start the Terraform provider
  performs in one apply, in either direction, with the `[P]` Elastic IP and every client `.conf`
  untouched: <https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/change-instance-type-of-ebs-backed-instance.html>.

- AWS: Denies access to AWS based on the source IP — the deny-by-IP example, with the note that "the policy does not deny requests made by AWS services using forward access sessions, as the original requester's IP address is preserved". That scopes Stage 4 step 8.1's `aws:ViaAWSService` carve-out: FAS flows survive the bare `NotIpAddress`, and the carve-out defends the on-behalf calls that are *not* FAS: <https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_examples_aws_deny-ip.html>.
...
- The AL2023 core repository index for `us-west-2`, read through the repo bucket itself — the
  measurement (2026-08-16) that `wireguard-tools`, `amazon-cloudwatch-agent` and `iptables-nft` all
  exist in the repository the 9.3 allow-list admits, so Stage 4's user data installs everything
  through the S3 gateway endpoint (Stage 4 steps 1.2 and 7.2; the repo's `primary.xml.gz` was grepped
  from the mirror the bucket names). The URL carries the `/x86_64/` mirror path — the architecture the
  host moved to on 2026-08-20, so the amd64 host inherits the measurement:
  <https://al2023-repos-us-west-2-de612dc2.s3.dualstack.us-west-2.amazonaws.com/core/mirrors/latest/x86_64/mirror.list>.
...
- `iam-root-access-key-check` (AWS Config managed rule) — identifier `IAM_ROOT_ACCESS_KEY_CHECK`, trigger type Periodic, no parameters: it evaluates on a schedule against the account rather than on a recorded configuration item, so it is independent of whatever Stage 1d step 10.3 decides about the recorder's scope (Stage 1d step 10.4): <https://docs.aws.amazon.com/config/latest/developerguide/iam-root-access-key-check.html>.

- Managing AWS Config rules across an organization — why 10.4 creates the rule by hand in Management instead of deploying it as an organization Config rule from the Audit delegated administrator: AWS Config does not create the service-linked role in the management account by itself, and "if you do not have an SLR for your management account, you will not be able to deploy resources to that account from a delegated administrator account"; member accounts are unaffected (Stage 1d step 10.4): <https://docs.aws.amazon.com/config/latest/developerguide/config-rule-multi-account-deployment.html>.

- Region deny control applied to the OU (`CT.MULTISERVICE.PV.1`) — the *configurable*, OU-scoped Region deny, with its `AllowedRegions` / `ExemptedActions` / `ExemptedPrincipalARNs` parameters and the full default `NotAction` and principal-exemption lists in its SCP artifact; the control that can be exercised against the `Policy Test` OU first (Stage 1c step 7.7): <https://docs.aws.amazon.com/controltower/latest/userguide/ou-region-deny.html>.
...
- Modifications to CloudTrail event data of IAM Identity Center — the January 2025 changes to `userName`, `principalId`, `userIdentity` type and group `displayName`. The Stage 1b step 8.3 filter matches every membership and assignment event and keys on nothing, because a pattern keyed on identity-plane payload fields, name or GUID, has a hidden expiry (`docs/plan/institutional-delta.md` carries the row for when filtering becomes necessary again): <https://aws.amazon.com/blogs/security/modifications-to-aws-cloudtrail-event-data-of-iam-identity-center/>.

- AWS Organizations increases SCP quotas (May 2026) — 10 SCPs per node and 10 240 characters per policy, up from 5 and 5 120; RCP quotas are not part of the same increase — the sizing budget Stage 1c step 7.1 states: <https://aws.amazon.com/about-aws/whats-new/2026/05/aws-organizations-increased-scp-quotas/>.

- Establish permissions guardrails using data perimeters — the trusted-resources SCP shape Stage 1c step 7.5 uses for `aws:ResourceOrgID`: `StringNotEqualsIfExists` beside `BoolIfExists` on `aws:PrincipalIsAWSService`, so a call that does not populate the key is not denied and calls AWS makes on your behalf are not caught: <https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_data-perimeters.html> and <https://aws.amazon.com/blogs/security/establishing-a-data-perimeter-on-aws-allow-only-trusted-resources-from-my-organization/>.

- Notebooks in SageMaker Unified Studio — three facts for the 2026-08-13 objectives: notebooks run on JupyterLab spaces; the default Spark runtime is Amazon Athena for Apache Spark, which "doesn't support Virtual Private Cloud (VPC)", with EMR Serverless / EMR / Glue as the VPC-capable alternatives and a *Network isolation* procedure for disabling it; and notebooks do not support trusted identity propagation — in an Identity Center domain they use *compatibility permission mode* for data access (Stage 1c step 7.6's action table; `docs/plan/open-questions.md` items 12 and 13): <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/notebooks.html>.

- Spaces in SageMaker Studio / Unified Studio — a space is "a combination of a compute instance, storage and other runtime configurations" behind a JupyterLab or Code Editor application, created through `sagemaker:CreateSpace` / `sagemaker:CreateApp`. The evidence that settled decision 1 of Stage 1c: denying the *classic* `sagemaker:CreateNotebookInstance` costs none of the `CLAUDE.md` notebook objectives, because they are a different product surface: <https://docs.aws.amazon.com/sagemaker/latest/dg/studio-updated-jl-user-guide-create-space.html> and <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/jupyterlab.html>.

- Stage 6's documentation pass (read 2026-08-16) — the pages behind
  ...
  - *Network isolation in SMUS* — the required VPC-endpoint table (`datazone` included), the
    ...
    the third control is *"remove Amazon Athena Spark permissions from individual project IAM
    policies"*, a grant-shaped edit on blueprint-authored policies rather than the permissions boundary
    Stage 6 step 2.1 builds; and `DenyUserAccessFromUnauthorizedVPCs` carries three conditions, the
    third `StringLike aws:userid = *:user-*`, which confines it to portal users and spares the catalog
    service on the same role. The endpoint tables are split *required* (`athena`, `datazone`(+`-fips`),
    `ec2`, `ec2messages`, `q`, `s3`, `sagemaker.api`, `sagemaker.runtime`, `glue`, `kms`,
    `secretsmanager`, `sts`, `ssm`, `ssmmessages`) and *optional* (per blueprint); the `q` row pairs
    with `com.amazonaws.us-east-1.codewhisperer`, *"available only in us-east-1"*, which a `us-west-2`
    VPC cannot reach through an interface endpoint (Stage 6 step 4.2). Re-read 2026-08-24, attributing
    the on-VPN portal break: a third table, *Public internet access*, states that the portal's client
    assets, its client APIs (`agent.datazone.<region>.api.aws` among them) and the IdC sign-in endpoints
    *require the public internet* — so one page instructs the `datazone` endpoint and requires a name
    that endpoint's private zone shadows (authoritative for the subtree; `NETWORK.md` §5). The required
    tables are scoped by the page's own no-internet premise, never by `VpcOnly`: its troubleshooting
    table answers *"Private with NAT + Private with NAT: Works as expected, no action needed"* (Lesson
    41); where the two tables collide, the browser-facing one wins. Re-read 2026-08-25 against the live
    page: a *VPC troubleshooting* section carries the scenario table behind the "your domain may be
    using a public subnet" banner, with *Private with NAT* blessed twice and *"network calls … route
    over the public internet when that network path is available"* stated outright; the public-internet
    section's four tables (portal assets, portal client APIs, IdC sign-in, console web client) seed the
    Sandbox allow-list's portal/console block (`EXC-06`'s exit path). Required-table diff against this
    estate: the non-fips gaps are exactly `ec2`, `ec2messages`, `q`/`codewhisperer`, `secretsmanager`,
    `ssm`, `ssmmessages`, all NAT-served under design A and added only by measurement (verification
    (viii)). Re-read 2026-09-05 for the 6b-6d review, the optional table in full, because under design
    B a project whose blueprint's endpoints are missing has no path at all rather than a slower one; its
    scoping sentence is *"Create these Amazon VPC endpoints if you plan to deploy Amazon SageMaker
    Unified Studio projects that include blueprints using the services listed below"*: Amazon Bedrock
    (`bedrock-agent`, `bedrock-agent-runtime`, `bedrock-runtime`); Athena Spark (`athena.sessions`,
    `athena.dashboard`, `athena.persistent-dashboard`); Glue interactive sessions (`glue.sessions`);
    CloudWatch (`logs`); EMR (`elasticmapreduce`, `elasticmapreduce-services`, `emr-serverless`,
    `emr-serverless-services.livy`, `emr-serverless-services.sessions`, `emr-serverless.dashboard`,
    `emr-dashboard`, `elasticmapreduce-fips`); EMR on EKS (`emr-containers`); RDS; Redshift (six
    names); Portal Query Editors (`sqlworkbench`, `sqlworkbench-v2`); CodeCommit; and CodeConnections
    (`codeconnections.api`, `codestar-connections.api`), which open question 26's `gitConnectionArn`
    would need — Stage 7's input rather than Stage 6's. The same reading confirmed the `q` row still
    pairs with the `us-east-1`-only `codewhisperer`, and that *"If Amazon VPC endpoints are missing or
    misconfigured, network calls … route over the public internet when that network path is
    available"* stops being true under design B, which is why 6c step 5.3 turns the optional table into
    a costed decision.
  - *Manage Tooling blueprint parameters* — `sagemakerDomainNetworkType` (default `VpcOnly`),
    `lifecycleManagement` / `idleTimeoutInMinutes` / `maxIdleTimeoutInMinutes`, `maxEbsVolumeSize`, and
    the per-parameter *Editable* flag that turns a default into a control (Stage 6 steps 1.5, 8.1):
    <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/manage-tooling-blueprint.html>.
  - *Supported blueprints* — the real names (Tooling, LakeHouseDatabase/`DataLake`, LakehouseCatalog,
    RedshiftServerless, EMRServerless, EMRonEC2, Workflows, MLExperiments, PartnerApps, Quicksight,
    AmazonBedrockGenerativeAI); there is no "ML experience" blueprint:
    <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/supported-blueprints.html>.
    Re-read 2026-08-19 for the per-blueprint *Resources created* column: `LakehouseCatalog`
    *"provisions a new catalog in the Amazon SageMaker Lakehouse that is backed by Amazon Redshift
    Managed Storage"*; the Glue/Athena form is `LakeHouseDatabase`/`DataLake` (Glue databases, Lake
    Formation permissions, Athena workgroups) — Stage 6 decision 4. `Workflows` *provides the
    CloudFormation template* for the MWAA environment, which is born when a project uses the blueprint,
    not when it is enabled; `MLExperiments` is an OnDemand MLflow tracking server; the combined
    `AmazonBedrockGenerativeAI` blueprint carries seven `AmazonBedrock*` sub-blueprints. Measured
    against the live domain 2026-08-21: `datazone list-environment-blueprints --managed` returns 23
    where the console lists 13, and they reconcile exactly — `AmazonBedrockGenerativeAI` is a console
    grouping with no API identifier, expanding into the seven; `LakeHouseDatabase` is the API's
    `DataLake`; four are API-only, never offered by the console (`LakehouseAdmin`, `S3Bucket`,
    `S3TableCatalog`, `ToolingLite`); and three of the page's names do not resolve —
    `EMRServerless`→`EmrServerless`, `EMRonEC2`→`EmrOnEc2`, `Quicksight`→`QuickSight`. `US-3`
    enumerates the names rather than matching a prefix, which would silently admit anything AWS adds
    under the namespace (Lesson 38).
  - *Associated accounts* — the association is a console flow in which DataZone creates the RAM share
    on the domain's behalf (no public API), and the member account configures blueprints, the
    provisioning and manage-access roles and the VPC parameters (Stage 6 steps 1.3-1.4, INT-12):
    <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/associated-accounts.html>
    and <https://docs.aws.amazon.com/datazone/latest/userguide/working-with-associated-accounts.html>.
    Both pages re-read 2026-08-21, before 1.3 ran: the surface is
    `https://console.aws.amazon.com/datazone` on both sides — *View domains → the domain → Account
    associations tab → Request association* in the domain account, *View requests → Review request →
    Accept and configure AWS association → Accept new permissions* in the member — not the
    `dzd-*.sagemaker.<region>.on.aws` portal. The V1 page carries a *RAM Policy* selector
    (`AWSRAMPermissionDataZonePortalReadWrite`, DataZone APIs plus data portal access, against
    `AWSRAMPermissionDataZoneDefault`, APIs only) and a 7-day invitation expiry; the V2 page states
    neither. Both accept flows offer to build the environment inline — V2's *Next steps for your
    domain* Configure buttons, V1's *DefaultDataLake/DefaultDataWarehouse* checkboxes with their *"have
    Amazon DataZone create and use a new IAM role"* pickers — a console route that mints roles this
    repository owns by name and skips `environment_role_permission_boundary` (INT-15; Lesson 17).
    Measured 2026-08-21 by running the flow: there is no invitation and no expiry — with Stage 1d's
    org-wide RAM enablement the console creates the share organization-scoped and it auto-accepts,
    zero invitations in either member account, so the accept flows are never reached; and the RAM
    Policy selector does not exist in this console, which offers *AWS Organization-only RAM share* and
    *IAM users can access APIs only* instead, producing
    `AWSRAMPermissionsAmazonDatazoneDomainExtendedServiceAccess`, a name RAM publishes and the
    documentation does not. `ram list-permissions --resource-type datazone:Domain` returns six, and
    neither `AWSRAMPermissionDataZoneDefault` nor `AWSRAMPermissionDataZonePortalReadWrite` is among
    them. The APIs-only choice is the design's no-portal decision under a different name.
  - *Trusted identity propagation in SMUS* — supported since 2025-09 for Athena/Redshift/Glue/EMR
    through the Tooling parameter `enableTrustedIdentityPropagationPermissions`; JupyterLab and Visual
    ETL still resolve through the project role (open question 13, Stage 6 decision 2):
    <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/trusted-identity-propagation.html>
    and <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/using-project-tip.html>.
  - *Remote access to spaces* — the SMUS policy scoping `sagemaker:StartSession` by the
    `AmazonDataZoneProject`/`AmazonDataZoneUser` tags, the `sagemaker:RemoteAccess` condition key, the
    IAM-credential/12-hour residual, and the incompatibility with TIP (Stage 6 step 3.2):
    <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/configuring-sagemaker-unified-studio-remote-access.html>
    and <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/network-config-remote-access.html>.
  - *BYOI in SMUS* — a custom image attaches to the Tooling-provisioned SageMaker AI domain in the
    member account; the Dockerfile specification; the ECR same-Region requirement (Stage 6 steps
    5.0-5.1, INT-01/INT-17):
    <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/byoi-how-to.html>,
    <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/byoi-specifications.html>
    and <https://docs.aws.amazon.com/sagemaker/latest/dg/studio-updated-byoi-how-to-attach-to-domain.html>.
    Re-read 2026-08-21 to write the build code (`images/`): `jupyterlab/default` is the health-check
    *Base URL* (`jupyterlab/default/api/status`, port 8888, one application always named `default`);
    the required base image is `public.ecr.aws/sagemaker/sagemaker-distribution`, version ≥ `2.6-cpu`,
    because those images carry the extensions without which an image will not run in SMUS. Three
    constraints the `Dockerfile`s enforce: no `ENTRYPOINT` — adding one *"will not work as expected"*,
    the distribution's `_entrypoint.sh` must survive, and a custom one is a `ContainerConfig` setting;
    `/opt/ml`, `/opt/.sagemakerinternal` and `/var/log/studio` are AWS's; and the space's EBS volume
    mounts at `/home/sagemaker-user`, a path that cannot be changed, so anything a user must write goes
    there and everything baked goes read-only under `/opt`. `UID=1000`/`GID=100` are *supported
    values* and the ids are remapped at runtime — the live `4.4.1-cpu` image runs `sagemaker-user` at
    57439, so the AWS sample's `NB_UID`/`NB_GID` are a convention rather than a requirement. Measured
    against the public registry the same day (anonymous Docker Registry v2 API — no IAM action, so
    `DenyEcrPublicEntirely` does not reach it): `4.4.1-cpu` is
    `sha256:9c1ea89f6ae62261de895f51fceb9f084c526829d50e21bcead175e3cfcfeb40`, `linux/amd64`, Ubuntu
    24.04, micromamba at `/opt/conda`, 24 layers ≈ 4.1 GB compressed; the repository publishes
    `cpu`/`gpu` suffixes only, with no `arm64` variant, which makes an Apple Silicon build an emulated
    one. The health check and the `jupyter-activity-monitor-extension` package name are stated on the
    JupyterLab custom-images pages:
    <https://docs.aws.amazon.com/sagemaker/latest/dg/studio-updated-byoi-specs.html> and
    <https://docs.aws.amazon.com/sagemaker/latest/dg/studio-updated-jl-admin-guide-custom-images.html>.
  - *Idle shutdown mechanics* — per-app idle settings with `MaxIdleTimeoutInMinutes` as the admin
    ceiling, requiring SageMaker Distribution v2+ (Stage 6 step 8.1):
    <https://docs.aws.amazon.com/sagemaker/latest/dg/studio-updated-idle-shutdown.html> and
    <https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_IdleSettings.html>.
  - *`sagemaker:InstanceTypes` condition key* — listed for `CreateApp`, `CreateSpace`, `UpdateSpace`
    and `CreateTrainingJob`, which is what makes Stage 6 step 3's instance ceiling reach spaces as well
    as jobs: <https://docs.aws.amazon.com/service-authorization/latest/reference/list_sagemaker.html>.
  - *The `aws-ia` module, measured* — registry name `aws-ia/sagemaker-unified-studio/aws`, v0.2.0
    (2026-07-02), providers `aws ≥ 6.51.0` / `awscc ≥ 1.89.0` / `random` / `time`; the domain is
    `aws_datazone_domain` with `domain_version = "V2"`, and the root requires `vpc_id`/`subnet_ids`
    and enables Tooling in the domain account — the single-account assumption Stage 6 verification
    (ii) is about: <https://registry.terraform.io/modules/aws-ia/sagemaker-unified-studio/aws/latest>.
  - *The associated-account half in Terraform* — the aws provider's
    `aws_datazone_environment_blueprint_configuration` takes `environment_blueprint_id`, an id its
    example resolves through a data source, and carries no `environment_role_permission_boundary`,
    which is why the module uses `awscc_datazone_environment_blueprint_configuration` instead (INT-15):
    <https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/datazone_environment_blueprint_configuration>.
    Measured 2026-08-21, when step 1.4's first apply failed twelve for twelve: the awscc resource rides
    CloudFormation's contract, whose `EnvironmentBlueprintIdentifier` is the blueprint name — the awscc
    example passes the literal `"DefaultDataLake"`, and the CFN page's separate
    `EnvironmentBlueprintId` GetAtt is the resolved id — so feeding it the data-source id made the
    handler hunt for a blueprint named like an id (`Managed Environment Blueprint with <id> doesn't
    exist`) while `get-environment-blueprint` answered for those same ids from the same profile, and a
    plain-CLI `put-environment-blueprint-configuration` for `Tooling` (id, provisioning role, boundary,
    two-subnet regional parameters) succeeded and was deleted (Lessons 30, 32). The CFN page still says
    *"only `DefaultDataLake` and `DefaultDataWarehouse` are supported"*, V1-era names a V2 domain does
    not carry. The live type schema (`describe-type`) marks `EnvironmentBlueprintIdentifier` createOnly
    and writeOnly, so name-passing is diff-safe, and `EnvironmentRolePermissionBoundary` writeOnly
    too, so boundary drift is invisible to `plan` and `./aws/studio.py` US-8 is the sentinel:
    <https://registry.terraform.io/providers/hashicorp/awscc/latest/docs/resources/datazone_environment_blueprint_configuration>
    and <https://docs.aws.amazon.com/AWSCloudFormation/latest/TemplateReference/aws-resource-datazone-environmentblueprintconfiguration.html>.
  - *SMUS multi-Region IdC* (2026-04) — a domain may sit outside IdC's Region only with an external
    IdP, and TIP does not cross Regions; with the native directory the same-Region rule stands (Stage
    6 step 1.1): <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/manage-users-idc-based-domains.html>.
  - *Disabling the JupyterLab download UI* — the official, bypassable lifecycle-configuration
    mitigation open question 6 records: <https://github.com/aws-samples/sample-disable-sagemaker-jupyterlab-download>.
  - *Account pools* — CLI-only, account-agnostic project profiles; noted for Stage 14, not adopted at
    N=1: <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/account-pools-create.html>.

- Athena Spark and AWS PrivateLink (2026-04-21) — the announcement that does not retire Stage 6
  decision 3 (read 2026-08-19, before the deny was written). It moves the client → session path:
  three interface endpoints — Spark Connect (`com.amazonaws.<region>.athena.sessions`), Live UI
  (`athena.dashboard`) and the Spark History Server (`athena.persistent-dashboard`), on workgroups
  running Apache Spark 3.5, the version SMUS notebooks use. It does not move where the session runs:
  there is no `NetworkConfiguration`, no subnet and no security group anywhere in the Athena Spark
  API, and the SMUS network-isolation page, current after the release, still answers VPC connectivity
  with *"use Amazon EMR or AWS Glue instead"* — the executor stays outside the customer VPC (open
  question 12). Two details argue for the deny: *"VPC endpoint policies are not supported on Athena
  Spark Connect, Live UI, or Persistent UI endpoints"* (the workaround is a policy on the Athena API
  endpoint governing `GetSessionEndpoint`/`GetResourceDashboard`), and a session URL generated inside
  a VPC *"can be accessed from that same VPC or from the public internet, but not from a different
  VPC"*, by design, for the open-the-dashboard-locally workflow. The negative control for the compute
  claim is the considerations-and-limitations page, which mentions no VPC, subnet or network
  configuration at any release version:
  <https://aws.amazon.com/about-aws/whats-new/2026/04/amazon-athena-spark-aws-privatelink/>,
  <https://docs.aws.amazon.com/athena/latest/ug/athena-spark-vpc-endpoint.html> and
  <https://docs.aws.amazon.com/athena/latest/ug/notebooks-spark-considerations-and-limitations.html>.

- The Spark-runtime block behind Stage 6 decision 1 (read 2026-08-19) — the pages that corrected the
  decision's number and added the FGAC counter-axis (the decision row carries the consequences):
...
- Remote access to SageMaker spaces from a local IDE — the `sagemaker:StartSession` API behind "connect your local VS Code to a Unified Studio space", the three connection methods (deep link, AWS Toolkit, SSH), and AWS's own recommendation to scope `StartSession` by tag to a user's *own* private applications. The action matches neither `sagemaker:Create*` nor `datazone:*`, which is why Stage 1c step 7.6 names it explicitly and why `docs/plan/open-questions.md` item 14 treats it as an egress channel: <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access.html> and <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/adminguide/local-ide-support.html>.
  - The pages 6d step 7.1 consumed (read 2026-09-07):
    - *Set up remote access* (the SageMaker AI guide) — the three methods' permissions, each on a different principal: the deep link on the space or domain execution role (the portal makes the call), the Toolkit on the IdC permission set, SSH on the local credentials; the `RemoteAccess` toggle on an existing space; remote connections authenticate with IAM credentials, not IdC, and persist up to 12 h: <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access-remote-setup.html>.
    - *Set up Remote IDE* — the SSH method's `ProxyCommand` script (`aws sagemaker start-session` → `session-manager-plugin … StartSession`) and the `~/.ssh/config` entry whose `HostName` is the space ARN: <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access-local-ide-setup.html>.
    - *Connect your Remote IDE to SageMaker spaces* — the local prerequisites (Remote-SSH, the Session Manager plugin, an SSH client), the laptop's endpoint names (`ssm`, `ssm…api.aws`, `ssmmessages`, `ec2messages`), the ≥ 8 GB instance rule that names `ml.t3.medium` unsupported, and the BYOI dependencies (`curl`/`wget`, `unzip`, `tar`, `gzip`): <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access.html>.
    - *Set up Studio to run with subnets without internet access* — the space-side pair `ssm` + `ssmmessages` "in addition to the standard set", the recommended endpoint policies on `aws:PrincipalIsAWSService`, and the two private-subnet shapes: an HTTP proxy with the IDE's names allowed, or a pre-packaged VS Code server installed by a lifecycle configuration; Cursor unsupported without internet: <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access-remote-setup-vpc-subnets-without-internet-access.html> and its client half, with `remote.SSH.httpProxy` / `remote.SSH.httpsProxy`: <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access-local-ide-setup-vpc-no-internet.html>.
    - *Advanced access control* — `sagemaker:RemoteAccess` is a condition key on `CreateSpace`/`UpdateSpace` only (a kill-switch, never a per-method control), and the tag-based example keyed on `IDC_UserName`, the principal tag a persona session can carry through Identity Center's attributes for access control: <https://docs.aws.amazon.com/sagemaker/latest/dg/remote-access-remote-setup-abac.html>.
    - *VS Code remote development FAQ* — the names the remote host downloads the server from (`update.code.visualstudio.com`, `vscode.download.prss.microsoft.com`) and the marketplace pair (`marketplace.visualstudio.com`, `*.gallerycdn.vsassets.io`), and `remote.SSH.localServerDownload`, the setting that moves the download to the local machine: <https://code.visualstudio.com/docs/remote/faq>.
    - *AWS Systems Manager endpoints and quotas* — the per-Region list the SageMaker guide points the laptop at (`ssm`, `ssmmessages`, `ec2messages`, each with its `amazonaws.com` and, for `ssm`, an `api.aws` spelling): <https://docs.aws.amazon.com/general/latest/gr/ssm.html>.
    - *Attributes for access control* (IAM Identity Center) — the mechanism decision due 4's `IDC_UserName` rests on: attributes selected on the instance's *Settings → Attributes for access control* page (API `CreateInstanceAccessControlAttributeConfiguration`) arrive in the account as session tags, readable as `aws:PrincipalTag/<key>` in every IAM policy type; with the Identity Center directory as the source, the values come from the user's own attributes: <https://docs.aws.amazon.com/singlesignon/latest/userguide/attributesforaccesscontrol.html> and <https://docs.aws.amazon.com/singlesignon/latest/userguide/configure-abac.html>.
    - *`session-manager-plugin`, `websocketutil.go`* — the plugin dials with gorilla's `websocket.DefaultDialer`, whose `Proxy` is `http.ProxyFromEnvironment`: the data channel honours `HTTPS_PROXY` if the variable reaches the process, which is 7.5's measurement rather than a documented claim: <https://github.com/aws/session-manager-plugin/blob/mainline/src/websocketutil/websocketutil.go>.

- Workflows in SageMaker Unified Studio — the Workflows tool is Amazon MWAA, in both a *serverless* and a *provisioned* form, and existing MWAA environments can be connected to a project. It confirms D7/D28: the `CLAUDE.md` "workflows" feature and Stage 10's orchestration comparison are the same surface (`docs/plan/open-questions.md` item 15): <https://docs.aws.amazon.com/sagemaker-unified-studio/latest/userguide/workflow-orchestration.html>.

- AWS Control Tower RCP controls — the artifacts of the RCP-based controls, and `CT.STS.PV.1` in particular: the page that diagnosed the 2026-08-14 SSO lockout and sets the scope of `EnforceOrgIdentitiesOnRoleAssumption`. AWS's own trusted-identities control for STS names only `sts:AssumeRole` and `sts:SetContext`, and states why the other four are out of scope — *"the respective STS operations do not use AWS security credentials, and therefore do not include the `aws:PrincipalOrgID` condition key value in the request context"* — with `sts:SetSourceIdentity` and `sts:TagSession` excluded as well so that `AssumeRoleWithSAML` and `AssumeRoleWithWebIdentity` are not denied. The page is also the reference shape for the other five statements of the RCP — `CT.S3.PV.4`, `CT.SQS.PV.1`, `CT.KMS.PV.7` and `CT.SECRETSMANAGER.PV.1` are the same `BoolIfExists` + `StringNotEqualsIfExists` pair — and carries the `ExemptedPrincipalArns` parameter, the supported way to add a carve-out (Stage 1c step 7.8): <https://docs.aws.amazon.com/controltower/latest/controlreference/list-of-rcp-controls.html>.
...
- Restrictions on SSM Parameter Store parameter names — the rule Stage 2's Validation hit at
  ...
  policy problem and is a naming one. This project's `awsds` prefix begins with `aws`, so Parameter
  Store is the one service where the convention of `docs/plan/conventions.md` cannot be used verbatim
  (the rule is in that file's naming section):
  <https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-parameter-name-constraints.html>.

- `aws_organizations_organizational_unit_descendant_organizational_units` (Terraform AWS provider) —
  the data source Stage 2 verification (iv) is about: given a parent, it returns organizational units
  at any depth, unlike `aws_organizations_organizational_units`, which returns one level of children.
  That is what makes `make check-ou` see `Sandboxes` at depth 2 under `Interactive` (D23), confirmed
...
- GitLab Dependency Scanning is Ultimate — not available to this CE instance, so Stage 8's dependency
  gate is `pip-audit` in an ordinary job rather than the built-in analyzer (Lesson 12; Stage 8 step
  5.2): <https://docs.gitlab.com/user/application_security/dependency_scanning/>.

- GitLab pipeline Secret Detection is Free — the one built-in scanner the CE instance gets, as a
  template include (`Jobs/Secret-Detection.gitlab-ci.yml`); the dashboards above it are Ultimate
  (Stage 8 step 5.3): <https://docs.gitlab.com/user/application_security/secret_detection/pipeline/>.

- GitLab protected runners are Free — a runner marked *Protected* runs only jobs on protected
  branches/tags, in every tier, and a project runner is scoped to the projects it is registered to.
  This pair makes Stage 8's deploy runner enforceable on CE: an ordinary CI job cannot schedule onto
  the deploy credential (Stage 8 step 4.3): <https://docs.gitlab.com/ci/runners/configure_runners/>.
...
- `pip-audit` — PyPA's dependency auditor, the open tool carrying Stage 8's dependency gate; consumes
  a requirements export (`uv export --format requirements-txt`) and queries the OSV/PyPI advisory
  databases (Stage 8 step 5.2): <https://pypi.org/project/pip-audit/>.

- ECR encryption at rest — who decrypts a pull. ECR creates two grants on the repository's KMS key
  for itself at repository creation, makes the `Decrypt` call on pulls on the caller's behalf, and the
  KMS permissions it documents (`kms:CreateGrant`, `RetireGrant`, `DescribeKey`) belong to the
  principal *creating or deleting the repository* — a pulling principal, cross-account included, needs
  the repository policy and no `kms:Decrypt` (Stage 9 step 7.3 records the non-grant):
...
- `PutModelPackageGroupPolicy` — the resource policy on a model package group (≤ 20 480 bytes), the
  API behind D28 item 6; Terraform exposes it as `aws_sagemaker_model_package_group_policy` (Stage 9
  step 3.2):
...
- Athena workgroups — override client-side settings; the behaviour Stage 9's workgroup boundary
  stands on: with the override on, *"Athena uses the workgroup's settings for all queries that run in
  the workgroup, including the settings for query results location, expected bucket owner,
  encryption, and control of objects written"*, and a query that asks for its own location *runs,
  with the workgroup's settings used*. One location per workgroup, which is why the per-principal
  result prefix became an enforced results zone (Stage 9 steps 1.2/5.4, verification (vi)):
...
- Lake Formation cross-account data sharing — the mechanism of Stage 9's producer path, and the
  ...
  immediately, with no invitation to accept; integrated services (Athena) require resource links on
  the consumer side; and the consumer's own admin regrants received permissions to local principals
...
- CloudWatch agent configuration file, the `logs` section — every field of the config the Stage 4
  WireGuard host writes in its user data: `agent.run_as_user` (optional, root when absent), and
  `logs.logs_collected.files.collect_list[]` with `file_path`, `log_group_name`, `log_stream_name` —
  where `{instance_id}`, `{hostname}`, `{local_hostname}` and `{ip_address}` are the substitutions
  allowed in the stream name — and `timezone`, whose only valid values are `UTC` and `Local`. On start
  the agent copies each configuration into `…/etc/amazon-cloudwatch-agent.d/` prefixed `file_` for a
  local source and `ssm_` for a Parameter Store one, so the prefix says where the running
  configuration came from:
  <https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Agent-Configuration-File-Details.html>.
  Unverified: the `amazon-cloudwatch-agent-ctl -a fetch-config -m ec2 -s -c file:…` invocation
  itself, whose page did not render on three fetch attempts (2026-08-16); the user data reports that
  command's exit status and follows it with `-a status`, so the first boot answers it.

- AWS Price List bulk API, the Amazon Bedrock offer files for `us-west-2` and `sa-east-1` (read
  2026-08-21, both published `2026-08-20`): the measured source of `docs/PRICING.md` §5's Bedrock
  rows, owed before the Stage 6 step 1.4 apply because decision 5 put `AmazonBedrockGenerativeAI` in
  category 1. Two readings from the files themselves: the `us-west-2` file carries no `output-tokens`
  usagetype for any Claude model (the current models are reached through cross-region inference
  profiles, whose SKUs publish under the profile's home region), and the `sa-east-1` file carries no
...
- KMS pricing and key rotation — the unit billed is a key *version*, not a key (read 2026-08-21, from
  ...
  KMS key version"*, and the rotation page turns that into a multi-year number: a rotation-enabled CMK
  bills 1 version in its first year, 2 after its first rotation, 3 after its second, and is capped
  there. Every CMK in this design sets `enable_key_rotation = true` with no `rotation_period_in_days`
  — the 365-day default, measured live as `True 365` on every key the same day — so
  `docs/PRICING.md` §2's count cell is a year-one figure and `docs/plan/cost-model.md`'s Floor row
  carries the consequence. Recorded as a rule rather than as an API rate, so §0's claim that every
  number in that file came from the bulk API stays true:
  <https://aws.amazon.com/kms/pricing/> and
  <https://docs.aws.amazon.com/kms/latest/developerguide/rotate-keys.html>.

- `uv` in the `base` image — the five sources behind its layer (read 2026-08-21, to answer *"install
  the latest uv"*). The latest is `0.12.5`, published 2026-08-14, the version this repository's own
  tooling runs on (`CLAUDE.md`) — a coincidence, not a constraint:
  <https://github.com/astral-sh/uv/releases/tag/0.12.5>. The image was never without uv: the
  distribution's own resolved environment — the `@EXPLICIT` list the `4.3.0-cpu` tag is built from —
  pins `uv-0.11.28` from conda-forge at line 913, so this is a version move inside an existing package
  and not an install, and a second binary under `/opt` would have left two `uv` whose answers depend
  on which PATH asks (Lesson 33):
  ...
  `libstdcxx >=15`, all satisfied at 15.2.0 in this image, so the move costs nothing else:
  <https://anaconda.org/conda-forge/uv>. `--freeze-installed` cannot express what it looks like it
  expresses: `create_install_request` emits a `Freeze` job for *every* package in the prefix with no
  exemption for the specs on the command line, so the flag would have frozen `uv` itself — an
  unsolvable request or a silent no-op, indistinguishable afterwards from protection; the guarantee is
  a before/after reading of `$MAMBA_ROOT_PREFIX/conda-meta` instead:
  <https://github.com/mamba-org/mamba/blob/main/libmamba/src/api/install.cpp>. Two readings from uv's
  own source: `system_config_file` tries `XDG_CONFIG_DIRS` and falls back to `/etc/uv/uv.toml`, which
  is why the CodeArtifact default is written there to be *discovered* rather than forced through
  `UV_CONFIG_FILE` (`--config-file` replaces discovery and would silence a data scientist's own project
  config); and `uv-static`'s `EnvVars` confirms `UV_SYSTEM_CERTS` and `UV_PYTHON_DOWNLOADS` while
  containing no `UV_VERSION`, which is what makes the build argument of that name safe:
  <https://github.com/astral-sh/uv/blob/0.12.5/crates/uv-dirs/src/lib.rs> and
...
A reading list added at the user's request, not authorities. None of these has been read against this
design; none is cited by a decision, a stage or an invariant; where a book and a measurement disagree
the measurement wins (Lesson 30). The rejected titles are recorded so the same search is not run
twice. Kindle availability was checked 2026-08-29.
...
- **AWS Certified Advanced Networking Study Guide: Specialty (ANS-C01), 2nd ed.** — Todd Montgomery
  ...
  peering, Route 53 including Resolver and private hosted zones. An exam book, thin on *why*; the most
  recent one that covers everything `docs/NETWORK.md` describes.
...
- **AWS Advanced Networking Exam Guide: ANS-C01** — Kam Agahian et al. (independently published, 2022),
  ...
  career network engineers (several CCIE/CCDE contributors): reference architectures and topology
  reasoning rather than exam recall. A complement to the Sybex, and older.
...
- **AWS Networking Fundamentals: A Practical Guide to Understand How to Build a Virtual Datacenter into
  ...
  datacenter piece by piece and follows a packet through it, the mental model the two egress paths and
  the VPN host's NAT role need. Self-published and unedited — reviews report wrong figure captions, so
  read the diagrams against the text.
...
Rejected: *Practical AWS Networking* (Mitesh Soni, Packt 2017) and *AWS Networking Cookbook* (Das and
Modi, Packt 2017) predate Transit Gateway and most of PrivateLink — the titles match what one would
search for, and the content does not. A crop of 2025 Kindle titles with generic names (*AWS Certified
Advanced Networking – Specialty 2025*; *Mastering AWS VPC*) are self-published with no editorial track
record; check the sample before buying.
