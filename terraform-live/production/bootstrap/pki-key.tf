...
#
# Production has two keys because D36 puts the internal CA's root private key in a Terraform state
...
#
# The key is created here and not by the pki/ slice. A backend is configured at `init`, before
...
# one slice.
#
# What makes this a control rather than a folder, since the key alone is not one: the key
# policy below is the same IAM delegation as the account state key, so today
...
#
# The file is separate so that main.tf stays byte-identical across the five bootstrap
