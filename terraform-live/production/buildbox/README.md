...
| **out** | **As a client of the explicit proxy** (D38, 6c step 5.8), and there is **no default route in this tier at all**. Three paths leave this host and reading them apart is what makes a failure diagnosable: the **internet** through `proxy.awsds.internal:3128` over the SharedServices ↔ Networking peering, bounded by the `production-foundation` plane of the proxy's allow-list; **AWS APIs** through this VPC's interface endpoints, which is why `no_proxy` is *generated* (5.6) and not written; **S3 and DynamoDB** by route, through the `[P]` gateway endpoints — free, and the reason an ECR pull is cheap, since the layers come from S3 |
