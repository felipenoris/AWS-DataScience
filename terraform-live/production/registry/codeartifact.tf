# CodeArtifact (Stage 7 step 5.3, the 5.a half) - the package path design B reads from when there
# is no NAT at all (D5(B), INT-02's consumer half, proven at Stage 6 pass 4).
#
# One domain, two repositories, one external connection each. A CodeArtifact repository may carry
# at most one external connection, which is why `pypi` and `crates` are two repositories rather
# than one with two upstreams.
#
...
#
# Julia and R are uncovered. CodeArtifact has no external connection for either, so under design B
# they arrive baked into the dev-env image (docs/plan/architecture.md 4.3) - a constraint on the
# image rather than a gap in this slice, and Stage 6's comparison prices it.
#
# The first fetch of any package reaches the public internet from CodeArtifact's side, not from the
# consumer's, and Production's NAT is not involved either: the external connection is the service's
# own egress. What the consumer needs is the interface endpoints (Stage 3 step 8.4) and the grants
# below.
...
#
# Both layers are needed (Lesson 28):
#
...
#
# Both are read-only. Nothing outside this account publishes a package: the promotion chain
# publishes from a pipeline in Production (Stage 8), and an Interactive account that could push
# into the shared repository would be a supply-chain write from the least-governed side of the
# estate.
