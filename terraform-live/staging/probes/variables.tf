# Inputs. Everything but the probe defaults and the tag defaults arrives from the generated,
# untracked terraform.auto.tfvars (./scripts/gen-tfvars.py staging probes) - region and env for
# Stage 2's standing reasons, zone_ids because the AZ choice lives in
# scripts/tfhygiene/backend.py (D9), account_folder because the remote-state key is keyed by the
# account folder, and peer_cidrs because an address range written in a .tf file is a copy of the
# allocation table that nothing keeps in step (Lesson 14).
#
# The target's addresses are not an input. This slice reaches Production by name, in a private
# zone pass 2 associated with this VPC, so no id crosses the account boundary.
