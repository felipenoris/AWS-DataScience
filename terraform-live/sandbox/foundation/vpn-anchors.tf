...
#
# The other anchors left at 6c step 6.5, once no slice read them through remote state: the security
# group - world-open UDP/51820 guarding no listener since 4.13's first half, and the estate's second
# world-open rule while it stood (VP-3 reads every account since, Lesson 31) - and the host-key secret
# container, whose value was copied by hand into the Production secret at 4.3 and which keeps Secrets
# Manager's 30-day recovery window here. `sandbox.internal` went in the same apply (zones.tf).
removed {
