...
make**, not a hole this slice can close ([Lesson 1](../../../docs/plan/lessons.md)); the stage buys the
permanence with named compensations rather than with a control.

## The apply order across `sandbox/data/` and `sandbox/lake/`

The first apply is three acts over two slices. **KMS validates the principals in a key policy**, so the
statement admitting the access role cannot be written before the role exists.
...
**No single file in this repository answers "what can this group do"**
([Lesson 28](../../../docs/plan/lessons.md)). The estate passed through the state where two of the three
were in place and nothing worked, on 2026-08-26 between the two applies, and that window is
**unmeasured**: the persona held no SSO token, the tunnel was down, and a probe off VPN would have been
denied by `DenyControlPlaneOffVpn` instead (Lesson 24). Step 2.3 is owed as a **contrast** rather than a
...
| **no `expiration_days`** | **the one bucket in this tree with no current-object expiry.** The module's variable exists so "the shadow lake does not silently become permanent"; here permanence is the requirement. The trade is priced in the stage file's compensations and in [`institutional-delta.md`](../../../docs/plan/institutional-delta.md) |
| noncurrent versions | 90 days, multipart abort 7 days — the module's unconditional hygiene |
| **no `additional_policy_statements`** | the emptiness **is** the access model. Nothing reaches this bucket by naming it in a bucket policy; the only principal with a statement over its objects is the access role, and the only way to become it is a vended session. `SL-5` measures the identity side |
...
The project statements are separate because they are separate actions: a trust admitting `sts:AssumeRole`
alone **rejects** an assume that also sets tags or a source identity.
...
test project); wiring and unwiring are §W's and §R's. **Never a wildcard principal**: `SL-2` fails on
one, and the enumeration *is* the register that makes §R's second half auditable.

**Absent:** `sts:SetContext` — the directory-grantee path, which decision 2 declined so that the
Identity Center association stays [open question 13](../../../docs/plan/open-questions.md)'s own
decision. **Left unpinned:** `AmazonDataZoneDomain`. The principal is already one project's role and the
project id is already pinned, so pinning the domain would buy nothing with a cross-account remote-state
read. Revision trigger: a second domain in this estate.

### The permissions

| `Sid` | What |
|---|---|
| `ReadWriteLakeObjects` | `GetObject`, `GetObjectVersion`, `PutObject`, `DeleteObject`, `AbortMultipartUpload`, `ListMultipartUploadParts` on `<bucket>/*`. `DeleteObject` is in because a permanent store needs correction; **versioning is what makes that safe** |
...
**Not carried, a recorded deviation from AWS's documented "option 1" location role:** the `S3AG*`
location/grant-management statements and the `iam:PassRole` to the Access Grants service. They exist so a
connection can register its *own* location; here the location is pre-registered. **Verification (ii) is
the gate** — if creating a connection demands them, they join as a measured amendment with a date.

**`permissions_boundary = null` is a decision** (the module makes the argument required so it has to be
one): the identity policy is already one bucket and one key, and every vended session is narrowed again
by the grant's scope-down. A boundary would be a third copy of the same narrowing, and where three
documents deny one call only one is proven ([Lesson 20](../../../docs/plan/lessons.md)). **Revision
trigger:** the first statement added to this role naming anything other than this bucket or this
account's data key.
...
table over it, which makes an unexpected top-level prefix a finding (`SL-4`).
...
this bucket, not a tenant of it. So the table is this slice's decision rather than a copy of
`identity/sso/`'s — the two answer different questions. What keeps it from drifting is `one()`: a row
naming a permission set not provisioned here **fails the plan**, so an invented tenant cannot be applied
and a withdrawn one fails by name on the next plan.

The inert rows are written now because the prefix contract is the bucket's layout.
