...
console as `AWS Control Tower Admin`, and **imported into Terraform at Stage 2 step 5.5** — which is why
the documents live in a file at all rather than only in the console: an import compares a document
against itself instead of against a re-typing.

**This folder is also a Terraform slice.** The documents did not move; what was added is the code that
adopts them. The import ran on 2026-08-16 (Stage 2 step 5.5): ten `aws_organizations_policy` and ten
`aws_organizations_policy_attachment` resources adopted, `0 to add / 10 to change / 0 to destroy`, second
plan `No changes`. A document change is now an `apply` in this slice, never a console paste.
...
| [`POLICIES.md`](POLICIES.md) | **The statement-level index for every document in `policies/`, of all four policy types**: what each entry does and why it exists. JSON carries no comments, so that file is where the reasoning lives, and it is **updated in the same sitting as any policy change** |

**The check that guards `POLICIES.md` is [`scripts/check-index.py`](../../../scripts/check-index.py)**, one
of the gates `make check` and `pre-commit` run. It verifies that `POLICIES.md` still lists exactly what
each document in `policies/` contains, in order, and names both directions of a mismatch. It is
type-aware: `Sid`s for an SCP or RCP, tag keys for a tag policy, `ec2_attributes` names for a declarative
policy, and an unrecognised document stops the run rather than being skipped. No AWS session, no side
effects, exits non-zero when it drifts. `render.py` stays in this folder: it is not a gate.

## The templates carry placeholders

Paste the *rendered* files, never the templates:
...
Organizations API. The last one is the one whose omission bites hardest: it sits inside the D27 crawler
carve-out in `awsds-org-scp-ou-data.json`, so a document shipped with it unsubstituted has an
`ArnNotEquals` comparing against a literal string and a carve-out that matches nothing, silently. It is
derived **from the OU, never from an account name** (the rule `aws/import-ids.py` states), and the result
lands in `aws/output/rendered-policies/`, which is untracked, so **no identifier enters a tracked file**
(`aws/INDEX.md` rule 1). `render.py` also refuses to leave a placeholder unsubstituted, checks that the
JSON parses, and prints each document's size against **5 120 characters** — the RCP limit, the tighter of
the two since SCPs went to 10 240 in May 2026, and the right one to check when the same folder holds both
kinds.

The ids are not baked in for two reasons: the organization id has to appear in four documents, and a value
typed four times is eventually wrong in one of them, in the silent direction (Lesson 14); and at Stage 2
the id comes from `data.aws_organizations_organization.this.id`, so a template is already the right shape
while a baked-in literal is an un-baking nobody remembers is pending.
...
hand in Stage 1c step 7 — and every resource here is imported. The slice derives everything it can from
the two files already authored, so nothing is typed twice:
...
**The policy `type` and the `description` have no home in a tracked file, so `locals.tf` writes them
out.** Both are checked against the document set in *both* directions: a missing entry and a stale one
fail differently, and only the first is loud on its own.
...
### The first apply is not a no-op

Step 5.5's gate — *after the import, `terraform plan` must come back empty* — applies to `content` and to
`type`. Two other things do change on the first apply, both measured on 2026-08-16:
...
**Anything else in that plan is a finding.** A diff on `content` means the document differs from what is
attached — a control that does not say what it appears to say.
...
ten documents; every other single-action statement here was already a scalar, so the fix moved the file
toward the folder's own convention.
...
**The price:** retiring a document or detaching one deliberately is a two-commit operation — remove the
`lifecycle` block, apply, then remove the entry. That friction is the same one the state buckets carry.
...
**It carries no CloudTrail and no Config statement.** Control Tower's guardrails already deny
the Config recorder on every registered OU, with the `AWSControlTowerExecution` carve-out that keeps the
landing zone able to update itself — measured by `aws/org-policy-baseline.py` section 4. For
...
The `aws:ResourceOrgID` write deny, S3 and ECR only. It is a separate document **because it is the one
...
Three properties of its shape, invisible in review and obvious in the canary:
...
- **`BoolIfExists: aws:PrincipalIsAWSService = false` is load-bearing.** Without it the deny reaches calls
  AWS services make on your behalf, and service *principals* — unlike service-linked roles — are not
  exempt.

### The per-OU documents (step 7.6) → one OU each, never the root

One tier per OU policy set (D23), on top of the root set above. They are separate documents because they
are attached to separate targets: a per-OU document is detached from that OU alone, so a mistake in one of
them costs one OU rather than the organization.
...
**Four more documents sit on the organization root, and three of them are not SCPs** (step 7.8) — read
[`POLICIES.md`](POLICIES.md) for the statement-level reasoning, which covers all four policy types:
...
| `awsds-org-scp-tag-enforcement.json` | `SERVICE_CONTROL_POLICY` | Decision 5: `Environment` and `Project` required on `ec2:RunInstances`, scoped to `instance/*` and split one statement per key — both correctness requirements, not style |
...
actions — the mirror of the `Data` document, where the wildcard *is* correct because nothing is supposed
to run there at all. **`datazone:*` is denied as a whole namespace** (D25's near miss): DataZone gains
APIs, and an enumerated list of them goes stale in the direction of the false negative.

**`Data` carves the catalog-maintenance role out of the crawler *runs*, and not out of their
...
**None of the four carves out `AWSControlTowerExecution`.** Control Tower's own guardrails exempt it from
their Config deny, so the landing zone can update itself; these documents deny compute *creation*, which
no landing-zone operation in this account map performs today. An unnecessary carve-out is a hole rather
than a safety margin, so the exemption is not pre-written. **Revision trigger:** a landing-zone update, an
account update or a re-enrollment that fails on a compute-creation call in `Data` or `Identity`. Carve
that one action then, with the reason recorded.
...
**What it does not prove**: that a write to a genuinely external bucket is denied. That rests on the real
document being the complement of the tested one, a one-character review, and it is the limit of a lab with
