...
# policies.
#
# The policy ids are useful: the detach command is the entire recovery path and it needs one.
# They live in two places already - `docs/log/log-stage-01c-preventive-policies.md`, recorded as
# each was attached, and `./aws/import-ids.py`, which regenerates them from the organization into
# untracked aws/output/. A third copy that appears on every apply is the one that gets pasted
...
output "policy_document_bytes" {
  description = <<-EOT
    ...
    An output rather than a comment because the number moves. Stage 6 and Stage 9 both come back
    to awsds-org-scp-perimeter.json, and every stage that adds a deny adds characters; the
    precondition in policies.tf is what fails, and this lets somebody watch the margin shrink
    before it does. Measured 2026-08-16: 201 characters at the smallest and 1 651 at the
    largest, against 5 120.
  EOT
