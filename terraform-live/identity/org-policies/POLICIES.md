...
| `DenyBedrockRetentionModeOtherThanNone` | Denies `bedrock:PutAccountDataRetention` where `bedrock:DataRetentionMode` is not `none` (Stage 6e decision 4). The mode is an account setting with **no Terraform resource and no CloudFormation type**, set by hand and readable by anyone with the permission — so without this it is a setting, not a control, and this estate could be opted into prompt retention by accident in an account nobody watches. **The enum has four values**: `default`, `none`, `provider_data_share`, `inherit`; the condition catches the other three, and `provider_data_share` is the one the requirement is about. **No Region condition, deliberately** — the mode is per account *and* per Region (`INV-18`), so a Region-scoped deny would freeze one Region and leave the rest free. **Effect: exercised in both directions on 2026-09-12, before attachment**, as a throwaway document on the `Policy Test` OU rather than by amending this one — this document already reaches `Policy Canary`, so a statement that turned out to be a blanket deny would have stranded every retention write in the organization at once. *Baseline:* `--mode none` with nothing attached succeeded. *Positive:* the same call with the document attached succeeded, with a fresh `updatedAt` — a write, not a no-op. *Negative:* `--mode inherit` returned `AccessDeniedException … explicit deny in a service control policy`, naming the throwaway's own id. So **`PutAccountDataRetention` publishes the condition key at request time** — the claim `accessanalyzer validate-policy` could not settle, since it answers only that a key is in the service's catalogue. A `StringNotEquals` whose key is absent evaluates TRUE, which is the failure this pair ruled out |
| `DenyInvokingModelsThatRetain` | Denies `bedrock:InvokeModel` and `bedrock:InvokeModelWithResponseStream` on the models the vendor names as retaining all traffic for up to 30 days with human review — `anthropic.claude-fable-5` and `anthropic.claude-fable-5-1`, read 2026-09-11 and **present and available in `us-west-2`**. It is the half of the pair a probe can prove: nothing in the control plane reads a retention mode's *effect*, so the mode ceiling above is verifiable only in its own terms while this one is verifiable by a call. **Six resource ARNs, not two** — the two foundation-model ids plus a `us.` **and** a `global.` inference profile for each (all four `ACTIVE`, measured 2026-09-12). A deny naming only the model ids would leave the profile routes open, which is how the model gets invoked in practice. **Written as a deny on the retaining models, never as an allow-list of the scoped ones**: an allow-list refuses the next model this estate adopts, from a document nobody thinks to open. It goes stale in the safe direction — a new retaining model is not on this list and is caught by the mode ceiling instead |
| `DenyRedshiftProvisionedClusters` | Denies `redshift:CreateCluster`, `RestoreFromClusterSnapshot` and `RestoreTableFromClusterSnapshot` on the organization root (Stage 5b decision 3). A provisioned cluster is the always-on shape [D12](../../../docs/plan/decisions/D12-budget-ceiling.md) rules out — the argument that refused always-on GitLab and provisioned MWAA — and nothing in `objectives.md` asks for one. **This is the cheap half**: a capability nobody has asked for is cheaper to refuse than to fence. **One known collision, in a stage four files away**: a Lake Formation **federated catalog** with *"Access this catalog from Iceberg compatible engines"* enabled makes **AWS Glue create a managed Amazon Redshift cluster**, which is a path [Stage 9](../../../docs/plan/stages/stage-09-deployment-targets.md) 9.7 may take; 9.5a owns the resolution and its recommendation is the mechanism that needs no cluster. **Exercised in the canary on 2026-09-20, both directions**: before the attach, `RestoreFromClusterSnapshot` read `ClusterSnapshotNotFound` and `RestoreTableFromClusterSnapshot` read `ClusterNotFound`; after it, both read `AccessDenied … with an explicit deny in a service control policy`, naming the policy id. `CreateCluster` is **attached and not exercised** (Lesson 20) and deliberately unprobed — nothing blocks it, so a probe would be the `creates` class and a provisioned cluster that really appeared in `Policy Canary` would be the estate's most expensive probe by orders of magnitude |
| `DenyRedshiftCostGuardTamperingExceptInfrastructure` | Denies `redshift-serverless:DeleteUsageLimit`, `UpdateUsageLimit` and `UpdateWorkgroup` to every principal except `InfrastructureAccess`'s reserved-SSO role, which is also the account's Terraform principal (Stage 5b decision 3). **What it cannot do, and the plan does not pretend otherwise: there is no IAM condition key for `baseCapacity`**, so no policy can say *"never above 4"*. The ceiling is the workgroup's own `max_capacity` plus the usage limit, and this statement's whole job is to stop those two being edited away — Lesson 18 in full, since the identity that authors it is outside it, and what watches that identity is CloudTrail. **`Resource: "*"` is forced rather than lazy**: a workgroup's ARN carries a service-minted UUID that changes on every destroy and re-create under the same name (measured, 5b 1.9: `workgroup/75b926c8-…` → `workgroup/4b0577a2-…`), and the compute slice is `[E]`, so an ARN-scoped statement would stop matching after the first `make down`. The same reading is why the usage limit is not named by ARN either. **Exercised on 2026-09-20, in both directions and on both sides.** *Denied side:* `DeleteUsageLimit` in `Policy Canary` read `ResourceNotFoundException` before the attach and `AccessDeniedException … with an explicit deny in a service control policy` after it. *Exempt side:* `UpdateUsageLimit` on the real Sandbox limit, to the amount it already held, **succeeded** as `InfrastructureAccess` — so the `ArnNotLike` carve-out admits the principal it names, which is what keeps `make up` able to apply the ceiling it enforces. **`UpdateWorkgroup` and `UpdateUsageLimit` are attached and not exercised on the denied side** (Lesson 20): `UpdateWorkgroup` **validates before it authorizes** (measured — it still answers `ResourceNotFoundException` in the canary with the statement live), and Lesson 21's remedy of retrying with a real id has nowhere to go, because this statement exists so that `Policy Canary` never holds a workgroup |
...
- **Glue table optimizers** (found 2026-08-19). No statement in this document names a table-optimizer
  ...
  only optimizer configured, at Stage 5a pass 1, runs under the carve-out role itself; but the deny does no
...
Athena was an exception to `DenyUserCompute` until 2026-08-20 (Stage 5a pass 4e). The allow existed for
Stage 5a's Iceberg maintenance, expected to run `OPTIMIZE`/`VACUUM` through Athena, a reason decision 4
...
| `DenyBedrockInvocationOutsideTheScopedModels` | Denies `bedrock:InvokeModel` and `bedrock:InvokeModelWithResponseStream` on **`NotResource`** — everything but the three scoped `us.` inference profiles and the three foundation models behind them (Stage 6e decision 15, 2026-09-12). **Why an SCP and not the IAM grant:** `sandbox/bedrock/` is a floor, this is the ceiling, and the two are different intents that happen to share one list (Lesson 51). The blueprint attaches AWS-managed policies to every project role and can widen what they allow without this repository being asked, which is exactly the shape a ceiling exists for. **Why `NotResource`:** a cross-Region inference profile is authorized once per destination Region, so the evaluation sees the profile ARN on one pass and a per-Region `foundation-model` ARN on the others; all six spellings are listed and anything else is denied on whichever pass names it. The profile ARNs carry a **wildcard account** because an account id may not appear in a tracked file (`aws/INDEX.md` rule 1) and because an inference profile ARN is always the caller's own account — within an account there is nothing else for it to match. The foundation-model ARNs carry **no region**: the profiles route to three, and a region-pinned ARN would deny two thirds of the requests by geography. **What it does not do:** it says nothing about which principal may invoke, which is §P's job. **Revision trigger:** the scoped set changing — this is one of the four consumers of that one list, and a model added to the grant and not here is refused at the ceiling with no denial naming the grant |
| `DenyBedrockReadsOutsideTheRoutedRegions` | Denies the four Bedrock actions exempted from Control Tower's Region ceiling — `InvokeModel`, `InvokeModelWithResponseStream`, `GetFoundationModelAvailability`, `GetUseCaseForModelAccess` — where `aws:RequestedRegion` is none of `us-east-1`, `us-east-2`, `us-west-2` (Stage 6e decision 15, 2026-09-12). **It exists because a hole was opened deliberately.** `CT.MULTISERVICE.PV.1` on this OU pins every service to `us-west-2`, and a cross-Region inference profile cannot be invoked under that condition (`AWS_STATE.md` `INV-12`), so five `bedrock:` actions were added to the control's `NotAction` list by hand. That lever has no resource and no region axis: it lifts the ceiling for those actions **everywhere**. This statement puts the region axis back, bounded by the profiles' actual routing rather than by the whole partition. **`bedrock:GetAccountDataRetention` and `PutAccountDataRetention` are deliberately absent from the list**: the retention mode is per Region and is declared in every enabled Region, so a Region condition here would forbid the very calls that keep it declared; what guards them instead is `awsds-org-scp-baseline.json`'s mode deny, on the mode axis, in every Region at once. **Revision trigger:** the routed-Region list changing — AWS owns that routing and can add a Region under a pinned model id, and `./aws/bedrock.py` is what reports it (`claude-code-sagemaker.md` M0) |
...
| `serial_console_access` | `disabled`. Beyond what step 7.8 listed. The EC2 serial console is console-based access to an instance that traverses no network, so it is a path into an instance around the private-network boundary the VPN guards (D39) and around every security group and endpoint policy Stages 3, 4 and 9 write. It is off by default per account, which makes pinning it cheap and forgetting it easy. Drop this row if the trade is unwanted: the cost is that recovering a mis-networked instance then needs a rebuild rather than a serial session |
