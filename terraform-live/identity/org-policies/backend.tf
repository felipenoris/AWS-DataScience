...
# reader may not see.
#
# This state file holds the full text of every preventive document in the organization, plus the
# root and OU ids they are attached to. The documents are tracked in policies/, but the state is
# a map of the ceiling, which is why the bucket is SSE-KMS under an account-local key whose
# policy is the read control (bootstrap/).
