#!/usr/bin/env bash
#
# check-iam-wildcards.sh - Stage 2 step 9.2. No wildcard-account ARN in the identity plane.
#
#   run:      ./scripts/check-iam-wildcards.sh            # terraform-live/identity/
#             ./scripts/check-iam-wildcards.sh <path>...  # anything else, for testing the check
...
TARGETS=("$@")
[ ${#TARGETS[@]} -eq 0 ] && TARGETS=(terraform-live/identity)
...
WHITELIST = {
    ("awsds-org-scp-baseline.json", "DenyAccountBpaChangeExceptInfrastructure"):
...
for t in targets:
    ...
    for root, dirs, names in os.walk(t):
        ...
        for n in sorted(names):
            if n.endswith(".json") or n.endswith(".tf"):
...
for path in files:
    ...
    if path.endswith(".json"):
        try:
            ...
            if isinstance(parsed, dict) and isinstance(parsed.get("Statement"), list):
                doc = parsed
    ...
    if doc is not None:
        ...
        for stmt in doc["Statement"]:
            ...
            if not PATTERN.search(json.dumps(stmt)):
                continue
            key = (base, sid)
            if key in WHITELIST:
                ...
                print(f"  FAIL  {path}  Sid={sid}: wildcard-account ARN in a statement that is not whitelisted")
                bad += 1
        continue
...
# Judged only when the document was actually in scope: run against some other path - which is
# how this check is itself tested - the entry is neither satisfied nor stale, it was not read.
for key, why in WHITELIST.items():
    if key in allowed_seen:
        continue
    if key[0] not in scanned:
        print(f"  note  whitelist entry {key[0]} Sid={key[1]} not judged: the document is "
              f"outside the scanned path")
        continue
    print(f"  FAIL  stale whitelist entry: {key[0]} Sid={key[1]} no longer carries a "
          f"wildcard-account ARN - remove it from this script")
    bad += 1
