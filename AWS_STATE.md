...
| **The root carries this project's two SCPs and four OUs carry one each** — 7.5 and 7.6 are done and both were exercised (INV-11). **7.7 is done: `CT.MULTISERVICE.PV.1` and the two root-user controls on the same five OUs** — `Policy Test`, `Workloads`, `Data`, `Interactive`, `Identity`; **not `Sandboxes` (by rule) and not `Security` (never a target)**, so **Log Archive and Audit are the only governed accounts with no Region ceiling** | **Stage 1d step 12** decides whether that stays true (open question 16). Everything else here is closed: 7.8 attached the RCP, the tag policy, the tag-forcing SCP and the declarative EC2 policy on 2026-08-14, and INV-11's root line is restated accordingly |
| **`awsds-org-scp-baseline` carries 8 statements, not 7** — `DenyImageAndSnapshotExport` was added 2026-08-13 (7.5a) — and `DenyUserCompute` carries **18 actions** in both `awsds-org-scp-ou-data` and `awsds-org-scp-ou-identity` (7.6a). All three read back from Organizations after upload and were re-probed | Nothing pending. **A future amendment repeats the same order**: upload, read back, re-probe — phase 4b for an OU document, phases 1-3 on the canary for a root one ([`plan/runbooks/scp-battery.md`](plan/runbooks/scp-battery.md)) |
| **`Policy Canary` holds nothing** — no bucket, no ECR repository, no IAM user — and `Policy Test` carries no policy of this project's | It is emptied at the end of **every** battery ([`plan/runbooks/scp-battery.md`](plan/runbooks/scp-battery.md)). A throwaway resource still there means a battery was interrupted; read the log before deleting it, because a battery in progress is the one time it is expected |
