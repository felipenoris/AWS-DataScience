...
| **The root carries this project's two SCPs and no OU carries one yet** — 7.5 is done (INV-11); every OU-level SCP shown is still `FullAWSAccess` or Control Tower's | **Stage 1c sitting B**: 7.6 one document per OU, 7.7 one Region control per OU, 7.8 the RCP, tag and declarative policies. Restate INV-11 as each lands |
