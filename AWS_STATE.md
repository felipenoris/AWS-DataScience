...
| **The root carries this project's two SCPs and four OUs carry one each** — 7.5 and 7.6 are done and both were exercised (INV-11). No Region control is enabled anywhere, and no RCP, tag or declarative policy is attached | **Stage 1c sitting B, what is left**: 7.7 one Region control per OU, 7.8 the RCP, tag and declarative policies. Restate INV-11 as each lands |
| **Three deployed documents are behind the repository**, amended 2026-08-13 and not yet uploaded: `awsds-org-scp-ou-data` and `awsds-org-scp-ou-identity` (7.6a — four EC2 launch actions; `BoolIfExists: aws:PrincipalIsAWSService=false` on the D27 carve-out) and `awsds-org-scp-baseline` (7.5a — five GuardDuty actions, and a new `DenyImageAndSnapshotExport` statement, taking it from 7 `Sid`s to 8). **Reading any of the three from AWS and finding the old content is expected, not drift** | **Closed by the upload plus the re-probe, in that order** — phase 4b of [`plan/runbooks/scp-battery.md`](plan/runbooks/scp-battery.md) for the OU pair, **phases 1-3 on the canary** for the root document, which reaches `Policy Canary` and can therefore be measured there. Delete this row then |
