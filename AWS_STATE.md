...
| No account-level S3 Block Public Access anywhere — **measured 2026-08-13, `NOT SET` in all six accounts with a profile**; the three without one are unread. And no RCP, tag or declarative policy is attached, because those types are not enabled | **Stage 1c**: 7.4 step 1 sets BPA in every account, 7.5 then denies changing it with one carve-out, and 7.8 attaches the rest. Until then, a bucket created outside a module is the exposure |
