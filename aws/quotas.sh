#!/usr/bin/env bash
#
# quotas.sh - the AWS Organizations quotas, and whether the account-cap increase has landed.
#
#   needs:    CloudShell on the MANAGEMENT account, as `AWS Control Tower Admin`.
#             There is no laptop path - see IDENTITY below.
#
#   run:      ./aws/quotas.sh                    # ambient credentials (CloudShell)
#             ./aws/quotas.sh <profile>          # if a profile ever reaches Management
...
# So a run from awsds-infra-identity does not return a small number, it returns a MEANINGLESS
...
OUT="$OUT_DIR/quotas.txt"
mkdir -p "$OUT_DIR"

export AWS_PAGER=""

TMP=$(mktemp -d "${TMPDIR:-/tmp}/quotas.XXXXXX")
trap 'rm -rf "$TMP"' EXIT
...
run service-quotas list-service-quotas --service-code organizations \
...
APPLIED=""
if [ -n "${CAP_CODE:-}" ]; then
  run service-quotas get-service-quota --service-code organizations --quota-code "$CAP_CODE" \
...
printf 'profile   : %s\n' "$PROFILE_LABEL"
printf 'caller    : %s\n' "$CALLER"
printf 'produced  : aws/quotas.sh   (index: aws/INDEX.md)\n'
printf '\n'
printf 'SECTIONS\n'
...
if [ -n "${CAP_CODE:-}" ]; then
  show service-quotas list-requested-service-quota-change-history-by-quota \
...
if [ -s "$ERRORS" ]; then
  ...
  printf '"wrong account" - see section 1.\n'
else
  printf 'None. Every call returned successfully.\n'
fi

printf '\nRegenerate with:  ./aws/quotas.sh          (CloudShell on Management)\n'
