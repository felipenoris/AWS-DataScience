...
| `warehouse.py` — **not written yet**; [Stage 5b](../docs/plan/stages/stage-05b-redshift-serverless.md) writes it before its pass 1, so the checks below are a specification and not a description | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | two profiles once both halves exist, `awsds-infra-sandbox-1` and `awsds-infra-prod`: [D40](../docs/plan/decisions/D40-redshift-warehouse.md) puts the `sbx_*` class in Sandbox and the `gov_*` class in Production, so neither account answers for the warehouse alone | `output/warehouse.txt` | **The Redshift Serverless warehouse and its cost ceiling** ([Stage 5b](../docs/plan/stages/stage-05b-redshift-serverless.md), extended by [6h](../docs/plan/stages/stage-06h-redshift-connection.md) and [Stage 9](../docs/plan/stages/stage-09-deployment-targets.md) step 9). `WH-1` the workgroup's shape — `base_capacity = 4`, a capped `max_capacity`, not publicly accessible, no Enhanced VPC Routing, and the subnets with their AZ `zone_id`s. `WH-2` the four config parameters (`require_ssl`, `enable_user_activity_logging`, `max_query_execution_time`, the price-performance target — whose **default is `Balanced`, which AWS does not recommend at 4 base RPUs**, so a reading of `Balanced` is a finding rather than a blank). **`WH-3` is the load-bearing one**: a `serverless-compute` usage limit exists with `breach_action = deactivate` and **no looser second limit beside it** — the default action is `log`, which refuses nothing, so a limit whose action was left alone reads as a ceiling and is not one. `WH-4` all three audit log groups carry a retention period that is not `Never Expire`: Redshift creates them itself at `Never Expire` if nobody owns them first, and this estate keeps exactly one such group as a dated exception (`EXC-10`). `WH-5` the admin credential is Secrets-Manager-managed and **no password string appears in the slice's state** — a `grep`, not a matter of trust. `WH-6` every database carries a class prefix and no class is in the wrong account (the prefix is a convention nothing enforces, so this check is the whole enforcement). `WH-7` the per-project tags match the authored map on **both** workgroup and namespace, and `for-use-with-all-datazone-projects` appears on neither — and on the Production namespace it reads *no project tag at all*, since D26 keeps a deployment target out of the domain. `WH-8` enumerates namespaces and workgroups in every profiled account, so a hand-made one is a diff. From 6h: `WH-9` the connection, `WH-10` the tag pair against the live projects, `WH-11` the project role's reach still inside the D13 boundary (`get-role` per role — `list-roles` omits it), `WH-12` the databases and their `GRANT`s, **which is the one check that needs a database session** rather than an AWS API: a Redshift grant has no `list-permissions` and is readable only from `SVV_*`. It also prints the **hourly burn**, which is 0.00 while no query runs and 1.44 while one does. Exits 2 when a check fails |
