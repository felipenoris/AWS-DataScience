...
| Question | Section |
|---|---|
| Which VPCs exist here, and which of them did **nobody in this project create**? | 2 — the DEFAULT column, plus `NT-1` in 11: every vended account carries an **Account Factory VPC** (`docs/AWS_STATE.md` §C), which is not the default VPC and not ours |
...
| Which accounts is nobody measuring? | 12 — **Staging above all**, whose "peering list is empty" deliverable is unrunnable until the vend |
