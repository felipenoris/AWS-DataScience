...
| [`account-bpa.py`](account-bpa.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user); [`AWS Control Tower Admin`](../docs/ORGANIZATION.md#aws-control-tower-admin-d33) for the `-` runs in Management, Log Archive and Audit | every `awsds-*` profile, the ones named as arguments, or `-` inside CloudShell for the accounts without a profile (multi-profile) | `output/account-bpa.txt` | The account-level S3 Block Public Access configuration of each account, the four flags side by side, and which accounts nothing is measuring. Read before 7.4, after 7.4 and before 7.5, and at every vend. |
| [`bedrock.py`](bedrock.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | every `awsds-*` profile in `~/.aws/config`, or the ones named as arguments (multi-profile) | `output/bedrock.txt` | What Amazon Bedrock is enabled for, per account, in three layers the console blurs: the **account gates** (the use-case form, the data retention mode, model invocation logging), the **catalogue** (providers, the scoped model set with its lifecycle and inference types, the inference profiles the account holds and how many models each routes to), and the **resources that would be billing** (provisioned throughput, custom, imported and marketplace models, guardrails). It decodes the use-case form, which is a blob on both sides of the API and double base64 over flat JSON inside. `BR-7` is an instrument control rather than a finding: it compares the availability reading in an account that submitted the form against one that did not, and reports whether the call distinguishes them. `BR-8` reads **both sides of the per-project grant** — what `terraform-live/sandbox/bedrock/variables.tf` declares against what IAM reports — because the two divergences are different faults: a declared attachment that is gone was removed by something (INT-15's open half), and an attachment nothing declares is undone by the next apply (Lesson 35). The **scoped set is read from the SCP** (`awslib/bedrockscope.py`), never spelled here: this file carried a literal until 2026-09-12 and it still named the generation Stage 6e decision 14 had abandoned the day before, so `BR-4` reported `pass` about three profiles nobody had scoped (Lesson 33). With the document unreadable — standalone in CloudShell — `BR-4` fails naming why, rather than passing on an empty set. |
| [`bedrock-scope.py`](bedrock-scope.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | one profile (`awsds-infra-sandbox-1` by default; the argument picks another) | `output/bedrock-scope.txt` | **The Bedrock scope this repository declares, against what AWS actually does.** The declaration is read out of [`awsds-org-scp-ou-interactive.json`](../terraform-live/identity/org-policies/policies/awsds-org-scp-ou-interactive.json) — by `awslib/bedrockscope.py`, which `bedrock.py` reads too — rather than carried here — the models from `DenyBedrockInvocationOutsideTheScopedModels`' `NotResource`, the Regions from `DenyBedrockReadsOutsideTheRoutedRegions`' condition (Stage 6e decision 15) — because a script with its own copy is one more consumer of a list that already has four (Lesson 33). It watches the two facts that are **AWS's and not ours**: a `us.` profile's routing, which AWS can change under a pinned model id with no diff anywhere here, and whether a model still holds an agreement. `BS-2` every declared profile is `ACTIVE`. **`BS-3` is the one that costs a session**: a Region AWS routes to and the SCP does not name is an invocation denied by our own statement, in a Region where the retention mode is not declared either. `BS-4` is its harmless mirror — a named Region nothing routes to is merely wider than needed. `BS-5` every scoped model holds an agreement; without one that model refuses at the first prompt naming neither the policy nor the grant (`EXC-08`). **`BS-6` fails silently in production**: it reads `get-account-data-retention` in **every enabled Region** rather than the routed three, because the routing can move (decision 16), and a Region reading `inherit` retains the prompt under the vendor default with nothing refusing. `BS-7` compares the SCP against the grant slice's `models` map and the image's pins. Exits 2 when a check fails. What it cannot see: whether a principal can invoke — that is an intersection (Lesson 28) and only a call answers it |
| [`bedrock-usage.py`](bedrock-usage.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | one profile (`awsds-infra-sandbox-1` by default; `--profile` picks another) | `output/bedrock-usage.txt` | **What Bedrock consumed in a window, per model, and what that costs.** With **no argument it reads this month to now** (UTC); `YYYY-MM` reads one calendar month with a per-day table; two ISO-8601 instants read one window, and **a time with no offset is refused** — this machine's CLI renders timestamps in local time while the API takes UTC, and the reading this script replaced confused the two once. Tokens and invocations come from CloudWatch `AWS/Bedrock` in `us-west-2`, which carries a cross-region `us.` profile's tokens **with invocation logging off** (the Control Tower Region ceiling refuses the other two Regions, and nothing is lost: measured 2026-09-12). Models are the union of `ListMetrics` — which names only series active in the last two weeks — and the scoped set read from the SCP through `awslib/bedrockscope.py`. `BU-1` CloudWatch still holds the window at the resolution asked. **`BU-2` every token is attributed to a model**: the dimensionless series against the per-model sum, the check that catches a model neither listed nor declared. `BU-3` invocations equal the per-model sum plus `InvocationClientErrors`, which AWS publishes only without a `ModelId` (66 = 60 + 6 on 2026-09-12). **`BU-4` CloudTrail agrees per model**: events naming a `modelId` with no `errorCode`, which counts a streaming invocation once despite its sibling event. Refusals are printed from both channels and not asserted, because they do not reconcile (18 `AccessDenied` against 6 client errors in one hour). `BU-5` every model with tokens has a row in `docs/PRICING.md`. **An empty window reports `BU-2`..`BU-4` as notes, never as agreement** — an identity over nothing holds whatever the instrument is doing. Verified both ways on 2026-09-12: the evening window read 22/20/3 in both channels, and with `amazon.nova-lite-v1:0` hidden from discovery `BU-2` failed on its 1 input and 16 output tokens and `BU-4` on its one invocation. The dollars are **computed, not billed**: cache writes at the 5-minute rate, which a 1-hour write exceeds; Cost Explorer carries the bill a day late. What it cannot see: which space, user or session spent the tokens — every space invokes as its project role |
...
| [`proxy.py`](proxy.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) on Production | `ec2:DescribeInstances`, `DescribeAddresses`, `DescribeSecurityGroups`, `ssm:GetParameter`, `logs:DescribeLogGroups`, `DescribeSubscriptionFilters` reads. `--on-host` adds `ssm:SendCommand`, a write API carrying only reads, fenced as `vpn.py`'s is | `output/proxy.txt` | The instrument for D38's single internet exit, shaped after `vpn.py` (one way in, one way out). `PX-1`: only the spokes and the tunnel may reach 3128. `PX-2`: no `http_access allow` precedes the private-destination deny; the order is the security property, since one allow above it makes the proxy an L7 bridge between VPCs peering keeps apart. The check reads the committed template with no session and the running file with `--on-host`, both counting toward the verdict. `PX-3`: the running allow-list equals the committed one, the second of two links (`DN-3` in `dns-allowlist.py` is the first, and needs no write API). `PX-4`: the access log group; a missing Log Archive export is a `note`, not a `fail`, because 4.11's second half is an open decision. `PX-5` is retired, not renumbered: no policy names the proxy's address since D39. Exits 2 when a check fails. What it cannot see: whether a request succeeds. Every check reads configuration; the behavioural proof is 6c step 6.3's four probe readings |
| [`vpn.py`](vpn.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user); [`AWS Control Tower Admin`](../docs/ORGANIZATION.md#aws-control-tower-admin-d33) for `-` runs | two profiles by default, the VPN home and Identity (the host lives in the VPN home, the permission sets `VP-7` reads in Identity); the ones named, or `-` | `output/vpn.txt` | [Stage 4](../docs/plan/stages/stage-04-vpn.md)'s evidence. The WireGuard host (`[D]`: type, root volume size and type, state, IMDSv2; the disk needs a second call, `DescribeVolumes`, because `DescribeInstances` names the volume and not its capacity; both halves of the shape are slice parameters, so `VP-1` reports them and passes), the `[P]` Elastic IP (printed as `WG_EIP=`, the `Endpoint` every client `.conf` pins), the one world-open SG rule (UDP/51820 and nothing else, never port 22), the handshake log and alarm, and `VP-7`, the guard that no permission set, `InfrastructureAccess` included, tests the caller's network in its inline policy (D39; read through the Identity Center delegated admin). Checks `VP-1`–`VP-9` less `VP-8`, retired when the GuardDuty reading moved to `guardduty.py` and not renumbered, so the Stage 4 log's readings stay unambiguous. Exits 2 when a check fails |
...
| [`datalake.py`](datalake.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user); [`AWS Control Tower Admin`](../docs/ORGANIZATION.md#aws-control-tower-admin-d33) for `-` runs | the same multi-profile shape: the lake and its policy live in Data Governance, every legitimate reader in a consumer account (D22), and a pending RAM invitation is visible only from the consumer | `output/datalake.txt` | [Stage 5a](../docs/plan/stages/stage-05a-data-foundation.md)'s evidence, producer and consumers side by side. The lake buckets with their policy branches (vpce/via/sigage/prin; presence, never sufficiency; an `ip` branch fails, and a principal branch must name `s3:PutObject` alone, D39), KMS aliases, the Glue catalog with resource links flagged, crawlers (never scheduled, never at an Iceberg target), the `awsds-data-catalog-maintenance` role and its trust, the Lake Formation `Parameters` reading that defends INT-11 (`DL-5`, the check to read after any apply in `data-governance/data/`), RAM shares and pending invitations, the absence of any consumer `awsds-*` workgroup or `*-derived` bucket (both removed 2026-08-26, D19 revised; `DL-8`/`DL-9` are absence checks), the absence of any EFS beyond a Studio domain's own tagged home (the NFS requirement was withdrawn 2026-08-17; conventions §5.1 rule 2 expects the domain's), Security Hub per account, and the persona's identity half of the drop-box write, read off the provisioned role in each consumer (`DL-12`; `DL-2` measures the resource half, and a cross-account permission is the AND of the two). Checks `DL-1`–`DL-14` (`DL-13` the data lake administrator list per account, `DL-14` the Lake Formation registrations in each consumer); exits 2 when a check fails |
...
| [`catalog.py`](catalog.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | two fixed profiles, `awsds-infra-data` and `awsds-infra-sandbox-1`: the domain lives in the registry account and the grants SMUS writes for its projects live in the consumer account, so neither side answers alone | `output/catalog.txt` | **The SageMaker Catalog over the governed lake, and the two roads an access act can take** ([Stage 6f](../docs/plan/stages/stage-06f-data-governance.md) step 1.3). On the catalog road a subscription or a direct Share is approved in the portal and DataZone writes a Lake Formation grant; on the register road a grant is written by Terraform and carries a row in [`docs/AWS_STATE.md`](../docs/AWS_STATE.md)'s grant register. A grant on the first road with no row on the second is invisible to every review this estate performs, and `CT-7` is that comparison: **no SMUS-managed principal may hold a grant on `raw`, `curated`, `dropbox` or an LF-Tag**, because the design reads the lake through TBAC re-grants to a persona. `CT-1` reads **`managedAccountId`, never the listing**: `list-domains` in an associated member account returns the shared domain, so the listing alone cannot say where a domain lives — the first draft failed on exactly that and reported the association as a second domain. `CT-2` every listing names a live owning project. `CT-3` the access acts per listing, reported. `CT-4` no data source publishes on import, which is the only switch that creates a listing nobody reviewed. `CT-5` the AI business-name generation, a note until 6f decision 3. **`CT-6` reads the run history, not the status field**: the `Tooling` blueprint creates a SageMaker data source on a daily cron nobody wrote (Lesson 17), and that source has failed **every day since 2026-08-24** — invisible in the data source's own `status`, which reads `RUNNING` because each run stops only when its successor starts. It reports the one source `FAILING_BY_EXCEPTION` names as a note citing `EXC-11` and **fails on any other**, so the check does not sit red while a recorded exception stands (Lesson 50). `CT-8` every `IAMPrincipals` grant carries `context.datazone.projectId` — without the condition that principal is the whole account. `CT-9` both override policy types are empty, so the owner project's approval stays the control on the catalog road. The three classifiers are exercised against fabricated grants, and no account id reaches the report. Exits 2 when a check fails. What it cannot see: DataZone refuses `GetAsset` and `GetDataProduct` to `InfrastructureAccess` (6f 0.6), so a refused `ListPolicyGrants` cell reads `DENIED` rather than empty |
| [`remote-ide.py`](remote-ide.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | three fixed profiles — `awsds-infra-sandbox-1`, `awsds-infra-identity`, `awsds-infra-prod`; `--days N` sets `RI-5`'s window (default 7) | `output/remote-ide.txt` | **What the remote-IDE channel rests on, and who opened a session from where** (Stage 6d step 7.9). `sagemaker:StartSession` is called by the client **as the project role**, so the persona sets' deny pair never evaluates on it and what scopes the call is **AWS's** managed policy — versioned, and moving without this repository. `RI-1` the seven endpoints the space side needs exist while `sandbox/egress` is up (an empty set is the slice down, a partial one the failure). `RI-2` every space with `RemoteAccess` enabled is at least 8 GiB, its type, memory and the rate `docs/PRICING.md` records printed. **`RI-3` every `StartSession` Allow reaching a project role is conditioned on both the project tag and the user tag** — measured 2026-09-12, AWS's `AllowStartSessionForSpaceRemoteConnection` in `SageMakerStudioProjectRoleMachineLearningPolicy` v43. **`RI-4` the six persona sets carry both deny Sids on the same resource-tag keys the Allow reads**, so the day AWS renames a key the divergence is reported rather than inherited. `RI-5`, reported and not asserted, the `StartSession` calls by caller and source: the proxy's Elastic IP, the VPN host's, or a laptop's own uplink, off the VPN or on the split-tunnel profile. The channel carries no network requirement (D39). The RI-3 and RI-4 rules were exercised against fabricated statements (an unconditioned Allow, a one-tag wildcard, a renamed tag — each fails). What it cannot see: whether a session is live, whether it outlived the tunnel (step 7.7), and which user a project-role call stands for |
...
| [`warehouse.py`](warehouse.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | two profiles, `awsds-infra-sandbox-1` and `awsds-infra-prod`: [D40](../docs/plan/decisions/D40-redshift-warehouse.md) puts the sandbox class in Sandbox and the governed class in Production, so neither account answers for the warehouse alone. `--sql` adds the database half | `output/warehouse.txt` | **The Redshift Serverless warehouse and its cost ceiling** ([Stage 5b](../docs/plan/stages/stage-05b-redshift-serverless.md), extended by [6h](../docs/plan/stages/stage-06h-redshift-connection.md) and [Stage 9](../docs/plan/stages/stage-09-deployment-targets.md) step 9). `WH-1` the workgroup's shape — base 4, a capped max, not publicly accessible, no Enhanced VPC Routing, two subnets; `WH-1b` the namespace role holds **no** policy, which is where D13 meets the new principal. `WH-2` the config parameters, **including that the price-performance target reads `DISABLED`**: AWS enables it by default at `Balanced` and does not recommend it at 4 base RPUs, so `ENABLED` is a finding rather than a blank. **`WH-3` is the load-bearing one**: a `serverless-compute` usage limit with `breach_action = deactivate` and no looser second limit beside it — the API's default is `log`, which refuses nothing, so a limit whose action was left alone reads as a ceiling and is not one. `WH-4` all three audit groups carry a retention period (Redshift creates them at `Never Expire` if nobody owns them first, and this estate keeps exactly one such group as a dated exception, `EXC-10`); `WH-4b` both alarms exist. `WH-5` the admin credential is Secrets-Manager-managed. **`WH-6` cannot classify by name and does not try**: a schema's name is thematic by requirement, so it reads instead that each account holds its own class container and not the other's, plus the two inert databases the service creates — `warehouse` *and* `dev`, because naming `db_name` adds a database rather than replacing `dev`. `WH-7` the `AmazonDataZoneProject` tag on **both** workgroup and namespace against the authored map, and `for-use-with-all-datazone-projects` on neither; the tag is **single-valued**, so layer 1 admits one project. `WH-8` enumerates namespaces and workgroups, so a hand-made one is a diff. `WH-11` layer 2, per admitted project: the slice's policy attached to the project role, and that the role **still carries the D13 boundary** - `iam.tf`'s precondition covers the boundary when the map is edited, this covers the role being recreated afterwards. `WH-15` the meter: an idle workgroup's most recent `ComputeSeconds` interval, which is where D40's "serverless costs nothing idle" stops being a quotation. `WH-12` the schemas, roles and grants, **which needs a database session**: a Redshift grant has no `list-permissions` and is readable only from `SVV_*`. **`WH-13` is a note and always will be**: `SVV_SCHEMA_QUOTA_STATE` and its `STV_` sibling are *refused to a superuser* through the Data API (measured 2026-09-20), so a schema's quota is unreadable on this platform and is proven by breach instead. It also prints the **burn** from `ComputeSeconds`, the only cost reading there is while the free trial hides usage from the billing console. **A missing workgroup is a `note`, never a failure**: the compute slice is `[E]` and its absence is the powered-off state. Exits 2 when a check fails |
| [`devenv.py`](devenv.py) | [Infrastructure](../docs/ORGANIZATION.md#infrastructure-user) | one profile, the account folder's own (`awsds-infra-sandbox-1` by default; the argument picks another) | `output/devenv.txt` | The dev-env image's proxy contract against the account that generates it. Since [6d decision 8](../docs/plan/stages/stage-06d-unified-studio-remainder.md) the house image carries `NO_PROXY` as a literal in [`images/dev-env/Dockerfile`](../images/dev-env/Dockerfile), and the list is generated by `<account>/egress`: the copy is dated the moment it is written and a change on the generating side leaves an image that still works. `DE-1`: the Dockerfile declares one `ARG NO_PROXY_LIST`, on one line, with no whitespace inside an entry. `DE-2`: the account's list reads from the egress slice's state object (`s3:GetObject` + `kms:Decrypt`, no terraform binary and no initialised directory) - an **empty** `no_proxy` output is the session being down, reported as that and not as a divergence. `DE-3`: the two lists are equal, and the two directions are different faults - a name in the account and not in the image leaves through the proxy and **succeeds**, without `aws:SourceVpce`; a name in the image and not in the account **times out** against an absent endpoint (Lesson 42). `DE-4`: the proxy URL agrees with the repository's other copy. **`DE-5`: the image the registry actually holds carries the account's list** - `DE-3` compares the *recipe*, so an image built from an older checkout or registered from an older tag passes it while the domain serves something else. `DE-5` follows the latest image version's `ContainerImage` digest to its manifest and config blob (`sagemaker:DescribeImageVersion`, `ecr:BatchGetImage`, `ecr:GetDownloadUrlForLayer`, one HTTPS GET, no layer pulled), **checks the blob against its own digest before believing it**, and reads the six variables out of its `Env`: none of Docker, the buildbox, or the buildbox client's own `noProxy` injected over the reading. Verified in both directions on 2026-09-12: `default-v0.4.0` bakes all six with the 52-entry list, `default-v0.1.0` - which predates the proxy `ENV` - bakes none. Exits 2 when a check fails. What it cannot see: **which image a space runs**. A space keeps its own version number and can name AWS's SageMaker Distribution from the same portal picker, which bakes no proxy at all (`EXC-09`, and a space created that way on 2026-09-12) - `describe-app`'s `ResourceSpec` is that reading, or `/opt/awsds-proxy.txt` from inside |
...
Log Archive and Audit, the accounts where no persona holds an assignment by design and that standing
...
| What may Stage 5a's bucket policies anchor on (INT-05)? | 5 — the gateway endpoint IDs, and only those; the interface endpoints of `egress.txt` are `[E]` and may anchor nothing (Lesson 3) |
...
| Does any private door exist for the SMUS portal's browser surfaces? | 10 — the regional endpoint-service catalog filtered to the SMUS families, each row read against the VPC configuration it fits; `NT-9` holds the 2026-08-24 absence as a baseline, so a door appearing is a red check pointing at the client plane's design (D38, D39), never a silent widening |
...
| What is the **Elastic IP** every client `.conf` pins as its `Endpoint`? | 3 — printed as `WG_EIP=`; also whether it is associated with the host |
...
| Does any permission set test the caller's network — `aws:SourceIp`, `aws:SourceVpc` or `aws:SourceVpce` — in its inline policy? | 5, decided by `VP-7` — any hit fails (D39) |
...
| Which S3 locations does Lake Formation govern in a consumer account, and who registered them? | 6, decided by `DL-14`: a note per SMUS project `dev/` scope under its own project role, a fail for anything else |
...
| Do the drop-box statement **and** its key name exactly `awsds-prod-job-exec` (the Stage 5a contract, INT-10)? | 6, decided by `DT-7` |
...
(**`awsds-prod-job-exec`**, the same name Stage 5a's drop-box statement and key grant carry), the workgroups
...
[Stage 5a](../docs/plan/stages/stage-05a-data-foundation.md) roteiros, before either stage runs. Until each
...
state Stage 5a step 5.4 must carry through its first apply — and confirmed the `IAMAllowedPrincipals`
create-defaults are still in place, which step 5.2 removes. Each script names its contracts (the
`awsds-*-vpn` Name tag, the network-origin keys `VP-7` refuses, the `awsds-data-catalog-maintenance` role
name)
...
- **One profile per script**, named at the top, with the reason that profile can see what it sees, and
  **name the SSO user behind it in the table above**: a profile is an *(account, permission set)* pair and
  ...
  right in every account but one is that account's hole. All three print the caller ARN
