...
| Are the IAM-fallback defaults still granting `IAMAllowedPrincipals` (step 5.2)? | 6, decided by `DL-6` once databases exist — **scoped to Data Governance today, and that is a gap from Stage 5 pass 4 on**: the defaults act at *creation* time in every account that has a `DataLakeSettings`, so the consumer accounts need the same reading before their first resource link exists. Extend it in the sitting that writes their settings, not after |
