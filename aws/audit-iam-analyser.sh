...
#
#                 ./aws/audit-iam-analyser.sh <profile>
#                 AWSDS_AUDIT_PROFILE=<profile> ./aws/audit-iam-analyser.sh
#
#   writes:   aws/output/audit-iam-analyser.txt   (untracked - see .gitignore)
...
#     ./aws/org-trusted-access-services.sh, which reads it from the Identity account.
...
# repository to land in. Writing to `aws/output/` relative to a home directory that is not
...
printf 'produced  : aws/audit-iam-analyser.sh   (index: aws/INDEX.md)\n'
[ "$STANDALONE" -eq 1 ] && printf 'note      : run outside the repository - this report is beside the script, not in aws/output/\n'
printf '\n'
printf 'SECTIONS\n'
...
printf '\nRegenerate with:  ./aws/audit-iam-analyser.sh   (CloudShell, on Audit)\n'
