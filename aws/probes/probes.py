...
#
#   probe(phase, account, expect, allowed_regex_or_None, safety, label, [aws args...])
#
...
#
# This file keeps a compact one-probe-per-block layout and is excluded from `ruff format`
# (pyproject.toml): it is edited as a table, not as code.

PROBES = []


def probe(phase, account, expect, allowed, safety, label, argv):
    """Collect one probe. Pure data - nothing here talks to AWS."""
    PROBES.append({
        "phase": phase,
        "account": account,
        "expect": expect,
...
probe("root", "canary", "deny", None, "blocked", "guardduty:UpdateDetector",
      ["guardduty", "update-detector",
       "--detector-id", "00000000000000000000000000000000", "--no-enable",
       "--region", "us-west-2"])

probe("root", "canary", "deny", None, "blocked", "guardduty:StopMonitoringMembers",
...
probe("root", "canary", "deny", None, "blocked", "ec2:ExportImage",
      ["ec2", "export-image", "--image-id", "ami-0123456789abcdef0",
       ...
       "--s3-export-location", "S3Bucket=awsds-canary-does-not-exist",
       "--region", "us-west-2"])

probe("root", "canary", "deny", None, "blocked", "ec2:CreateInstanceExportTask",
      ["ec2", "create-instance-export-task", "--instance-id", "i-1234abcd",
       "--target-environment", "vmware",
       "--export-to-s3-task",
       '{"S3Bucket":"awsds-canary-does-not-exist","DiskImageFormat":"VMDK",'
       '"ContainerFormat":"ova"}',
       "--region", "us-west-2"])
...
probe("root", "canary", "deny", None, "blocked",
      ...
      ["ec2", "create-store-image-task", "--image-id", "@AMI@",
       "--bucket", "awsds-canary-does-not-exist", "--region", "us-west-2"])

probe("root", "canary", "deny", None, "blocked", "rds:StartExportTask",
...
probe("root", "canary", "allow", None, "ro", "floor: s3:ListAllMyBuckets",
      ["s3api", "list-buckets"])
probe("root", "canary", "allow", None, "ro", "floor: ec2:DescribeVpcs",
      ["ec2", "describe-vpcs", "--region", "us-west-2"])
probe("root", "canary", "allow", None, "ro", "floor: iam:ListRoles (global, us-east-1)",
...
probe("ou", "prod", "deny", None, "ro", "workloads: datazone:ListDomains",
      ["datazone", "list-domains", "--region", "us-west-2"])
probe("ou", "prod", "deny", "ValidationException|ResourceNotFound|does not exist", "blocked",
      "workloads: sagemaker:CreateSpace (no such domain)",
      ["sagemaker", "create-space", "--domain-id", "d-0000000000000",
       "--space-name", "awsds-canary-probe", "--region", "us-west-2"])
probe("ou", "prod", "deny", "ValidationException|does not exist", "blocked",
      "workloads: sagemaker:CreateNotebookInstance (no such role)",
...
probe("ou", "data", "deny", None, "dryrun", "data: ec2:RunInstances",
      ["ec2", "run-instances", "--dry-run", "--image-id", "@AMI@",
       ...
       "--region", "us-west-2"])
probe("ou", "data", "deny", None, "dryrun", "data: ec2:CreateFleet (7.6a)",
      ["ec2", "create-fleet", "--dry-run",
       "--launch-template-configs",
       '[{"LaunchTemplateSpecification":{"LaunchTemplateName":"awsds-canary-probe",'
       '"Version":"1"}}]',
       "--target-capacity-specification",
       '{"TotalTargetCapacity":1,"DefaultTargetCapacityType":"on-demand"}',
       "--region", "us-west-2"])
probe("ou", "data", "deny", None, "dryrun", "data: ec2:RequestSpotInstances (7.6a)",
      ["ec2", "request-spot-instances", "--dry-run", "--instance-count", "1",
       "--launch-specification",
       '{"ImageId":"@AMI@","InstanceType":"t3.micro","SubnetId":"@SUBNET@"}',
       "--region", "us-west-2"])
probe("ou", "data", "deny", "EntityNotFoundException", "blocked",
      "data: glue:StartJobRun (no such job)",
      ["glue", "start-job-run", "--job-name", "awsds-canary-probe",
       "--region", "us-west-2"])
probe("ou", "data", "deny", "EntityNotFoundException", "blocked",
      "data: glue:StartCrawler (D27 negative half)",
      ["glue", "start-crawler", "--name", "awsds-canary-probe", "--region", "us-west-2"])
probe("ou", "data", "deny", "ValidationException|ResourceNotFound|does not exist", "blocked",
      "data: sagemaker:CreateSpace (no such domain)",
      ["sagemaker", "create-space", "--domain-id", "d-0000000000000",
       "--space-name", "awsds-canary-probe", "--region", "us-west-2"])
probe("ou", "data", "deny", "EntityNotFoundException", "blocked",
      "data: lakeformation:DeregisterResource (no such resource)",
...
probe("ou", "identity", "deny", None, "dryrun", "identity: ec2:RunInstances",
      ["ec2", "run-instances", "--dry-run", "--image-id", "@AMI@",
       "--instance-type", "t3.micro", "--subnet-id", "@SUBNET@",
       "--region", "us-west-2"])
probe("ou", "identity", "deny", None, "dryrun", "identity: ec2:CreateFleet (7.6a)",
      ["ec2", "create-fleet", "--dry-run",
       "--launch-template-configs",
       '[{"LaunchTemplateSpecification":{"LaunchTemplateName":"awsds-canary-probe",'
       '"Version":"1"}}]',
       ...
       '{"TotalTargetCapacity":1,"DefaultTargetCapacityType":"on-demand"}',
       "--region", "us-west-2"])
probe("ou", "identity", "deny", "EntityNotFoundException", "blocked",
      "identity: glue:StartJobRun (no such job)",
      ["glue", "start-job-run", "--job-name", "awsds-canary-probe",
       "--region", "us-west-2"])
probe("ou", "identity", "allow", "EntityNotFoundException", "blocked",
      "identity: glue:StartCrawler is ALLOWED here",
      ["glue", "start-crawler", "--name", "awsds-canary-probe", "--region", "us-west-2"])
probe("ou", "identity", "allow", "EntityNotFoundException", "blocked",
      "identity: lakeformation:DeregisterResource is ALLOWED here",
      ["lakeformation", "deregister-resource",
       "--resource-arn", "arn:aws:s3:::awsds-canary-does-not-exist",
       "--region", "us-west-2"])
...
probe("ou", "data", "allow", None, "ro", "floor: s3:ListAllMyBuckets",
      ["s3api", "list-buckets"])
probe("ou", "data", "allow", None, "ro", "floor: glue:GetDatabases",
      ["glue", "get-databases", "--region", "us-west-2"])
probe("ou", "identity", "allow", None, "ro", "floor: s3:ListAllMyBuckets",
      ["s3api", "list-buckets"])
...
probe("region", "canary", "deny", None, "dryrun", "region: ec2 in us-east-1 must be denied",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-east-1"])
probe("region", "canary", "allow", None, "dryrun",
      "region: ec2 in us-west-2 must still work",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-west-2"])
...
probe("region", "data", "deny", None, "dryrun", "region: us-east-1 denied in Data Governance",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-east-1"])
...
probe("region", "sandbox1", "deny", None, "dryrun",
      "region: us-east-1 denied in Sandbox 1 (nested OU)",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-east-1"])
probe("region", "prod", "deny", None, "dryrun", "region: us-east-1 denied in Production",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-east-1"])

probe("region", "data", "allow", None, "dryrun",
      "region: us-west-2 still works in Data Governance",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-west-2"])
probe("region", "identity", "allow", None, "dryrun",
...
probe("region", "sandbox1", "allow", None, "dryrun",
      "region: us-west-2 still works in Sandbox 1",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-west-2"])
probe("region", "prod", "allow", None, "dryrun",
      "region: us-west-2 still works in Production",
      ["ec2", "create-key-pair", "--key-name", "awsds-canary-probe", "--dry-run",
       "--region", "us-west-2"])
...
probe("region", "data", "allow", None, "ro", "region floor: iam:ListRoles in Data Governance",
      ["iam", "list-roles", "--max-items", "1"])
...
probe("region", "sandbox1", "allow", None, "ro", "region floor: iam:ListRoles in Sandbox 1",
      ["iam", "list-roles", "--max-items", "1"])
probe("region", "prod", "allow", None, "ro", "region floor: iam:ListRoles in Production",
      ["iam", "list-roles", "--max-items", "1"])

probe("region", "canary", "allow", None, "ro", "region floor: iam:ListRoles",
      ["iam", "list-roles", "--max-items", "1"])
probe("region", "canary", "allow", None, "ro", "region floor: budgets:DescribeBudgets",
      ["budgets", "describe-budgets", "--account-id", "@ACCT@"])
probe("region", "canary", "allow", None, "ro", "region floor: ce:GetCostAndUsage",
      ["ce", "get-cost-and-usage",
       "--time-period", "Start=2026-08-01,End=2026-08-02",
       "--granularity", "DAILY", "--metrics", "UnblendedCost"])
probe("region", "canary", "allow", None, "ro",
      "region floor: organizations:DescribeOrganization",
...
probe("rcp", "data", "allow", None, "ro",
      ...
      ["sts", "get-caller-identity"])
probe("rcp", "identity", "allow", None, "ro",
      "rcp floor: credentials still vend in Identity",
      ["sts", "get-caller-identity"])
probe("rcp", "sandbox1", "allow", None, "ro",
      "rcp floor: credentials still vend in Sandbox 1",
      ["sts", "get-caller-identity"])
probe("rcp", "prod", "allow", None, "ro",
...
probe("rcp", "canary", "allow", None, "ro", "rcp floor: dynamodb reachable",
      ["dynamodb", "list-tables", "--region", "us-west-2"])
probe("rcp", "canary", "allow", None, "ro", "rcp floor: sqs reachable",
      ["sqs", "list-queues", "--region", "us-west-2"])
probe("rcp", "canary", "allow", None, "ro", "rcp floor: kms reachable",
      ["kms", "list-aliases", "--limit", "1", "--region", "us-west-2"])
probe("rcp", "canary", "allow", None, "ro", "rcp floor: secretsmanager reachable",
      ["secretsmanager", "list-secrets", "--max-results", "1", "--region", "us-west-2"])
probe("rcp", "canary", "allow", None, "ro", "rcp floor: ecr reachable",
...
probe("rcp", "data", "allow", None, "ro", "rcp floor: s3 reachable in Data Governance",
      ["s3api", "list-buckets"])
probe("rcp", "data", "allow", None, "ro", "rcp floor: kms reachable in Data Governance",
      ["kms", "list-aliases", "--limit", "1", "--region", "us-west-2"])
probe("rcp", "data", "allow", None, "ro", "rcp floor: ecr reachable in Data Governance",
      ["ecr", "describe-repositories", "--max-results", "1", "--region", "us-west-2"])
probe("rcp", "data", "allow", None, "ro", "rcp floor: glue still reads the catalog",
      ["glue", "get-databases", "--region", "us-west-2"])
...
probe("tags", "canary", "deny", None, "dryrun", "tags: RunInstances with NO tags",
      ["ec2", "run-instances", "--dry-run", "--image-id", "@AMI@",
       "--instance-type", "t3.micro", "--subnet-id", "@SUBNET@",
       "--region", "us-west-2"])

# `Environment=org` is the enumerated value for every account that is not a workload
# environment - Management, Log Archive, Audit, Identity and this one. The SCP tests
# presence and not value, so this row would pass with any string; using the tag policy's
# own value keeps the two documents from drifting into disagreement about what a legal tag
# looks like.
probe("tags", "canary", "deny", None, "dryrun",
      "tags: RunInstances with Environment ONLY (catches the AND bug)",
      ["ec2", "run-instances", "--dry-run", "--image-id", "@AMI@",
       "--instance-type", "t3.micro", "--subnet-id", "@SUBNET@",
       "--region", "us-west-2",
       "--tag-specifications",
...
probe("tags", "canary", "allow", None, "dryrun",
      "tags: RunInstances with BOTH tags must still work",
      ["ec2", "run-instances", "--dry-run", "--image-id", "@AMI@",
       "--instance-type", "t3.micro", "--subnet-id", "@SUBNET@",
       "--region", "us-west-2",
       "--tag-specifications",
...
probe("decl", "canary", "deny", None, "creates", "decl: ec2:DisableImageBlockPublicAccess",
...
probe("decl", "canary", "allow", None, "ro", "decl floor: read snapshot BPA state",
      ["ec2", "get-snapshot-block-public-access-state", "--region", "us-west-2"])
probe("decl", "canary", "allow", None, "ro", "decl floor: read serial console status",
