...
printf '    packs per ENABLEMENT and does it inconsistently across OUs - the root-user\n'
printf '    controls sit in the original guardrail on three OUs and in the REGION document\n'
printf '    on two. "The region policy" is not a stable way to refer to anything.\n'
...
printf '\n'
printf 'READ THE GAPS, NOT THE YESES. Two are expected and neither is drift:\n'
...
printf '  - `Security` has no region ceiling and no root controls: it was never a target of\n'
printf '    Stage 1c step 7.7, which leaves `Log Archive` and `Audit` as the two accounts\n'
printf '    with no Region restriction. That is open question 16, addressed to Stage 1d.\n'
